...
class kang_goldModule extends WeModule
{
    ...
    public function fieldsFormDisplay($rid = 0)
    {
        ...
        if (!empty($rid)) {
            ...
            $share = pdo_fetchall("select * from " . tablename('kang_gold_share') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
            $prize = pdo_fetchall("select * from " . tablename('kang_gold_prize') . " where rid = :rid order by `id` asc", array(':rid' => $rid));
        }
        if (!$reply) {
            $now = time();
            $reply = array(
                "title" => "幸运猜涨跌活动开始了!",
                "start_picurl" => "../addons/kang_gold/template/images/activity-lottery-start.jpg",
                "description" => "欢迎参加幸运猜涨跌活动",
                "repeat_lottery_reply" => "亲，继续努力哦~~",
                "ticket_information" => "兑奖请联系我们,电话: 13512341234",
                "starttime" => $now,
                "endtime" => strtotime(date("Y-m-d H:i", $now + 7 * 24 * 3600)),
                "end_theme" => "幸运猜涨跌活动已经结束了",
                "end_instruction" => "亲，活动已经结束，请继续关注我们的后续活动哦~",
                "end_picurl" => "../addons/kang_gold/template/images/activity-lottery-end.jpg",
                "most_num_times" => 0,
                "number_times" => 0,
                "probability" => 0,
                "turntable" => 1,
                ...
                "bigwheelpic" => "../addons/kang_gold/template/images/activity-lottery-6.png",
                "bigwheelimg" => "../addons/kang_gold/template/images/activity-inner.png",
                "bigwheelimgan" => "../addons/kang_gold/template/images/activity.png",
                "bigwheelimgbg" => "../addons/kang_gold/template/images/activity_bg.png",
        ...
        if (!$share) {
            ...
            foreach ($ids as $acid => $idlists) {
                $share[$acid] = array(
                    ...
                    "share_desc" => "亲，欢迎参加猜涨跌抽奖活动，祝您好运哦！！ 亲，需要绑定账号才可以参加哦",
                    "share_picurl" => "../addons/kang_gold/template/images/share.png",
                    "share_pic" => "../addons/kang_gold/template/images/img_share.png",
                    "sharenumtype" => 0,
                    "sharenum" => 0,
                    "most_share_times" => 0,
                    "sharetype" => 1,
    ...
    public function fieldsFormSubmit($rid)
    {
        ...
        if (empty($id)) {
            ...
            if ($token['config']) {
                $id = pdo_insert('kang_gold_reply', $insert);
            }
        } else {
            if ($token['config']) {
                pdo_update('kang_gold_reply', $insert, array('id' => $id));
            }
        }
        if ($token['config']) {
            //查询规则
        ...
        for ($i = 0; $i <= 11; $i++) {
            ...
            if ($token['config']) {
                if (empty($_GPC['prize_id_' . $i])) {
                    pdo_insert('kang_gold_prize', $insertprize);
                } else {
                    pdo_update('kang_gold_prize', $insertprize, array('id' => $_GPC['prize_id_' . $i]));
                }
            }
        }
        pdo_update('kang_gold_reply', $updata, array('id' => $id));
        //奖品配置
        //查询子公众号信息必保存分享设置
        $acid_arr = uni_accounts();
        $ids = array();
        ...
        foreach ($ids as $acid => $idlists) {
            ...
            if ($token['config']) {
                if (empty($_GPC['acid_' . $acid])) {
                    pdo_insert('kang_gold_share', $insertshare);
                } else {
                    pdo_update('kang_gold_share', $insertshare, array('id' => $_GPC['acid_' . $acid]));
                }
            }
        }
        //查询子公众号信息必保存分享设置
    ...
    {
        pdo_delete('kang_gold_award', array('rid' => $rid));
        pdo_delete('kang_gold_reply', array('rid' => $rid));
        pdo_delete('kang_gold_fans', array('rid' => $rid));
        pdo_delete('kang_gold_data', array('rid' => $rid));
        pdo_delete('kang_gold_prize', array('rid' => $rid));
        pdo_delete('kang_gold_share', array('rid' => $rid));
    }
    ...
    public function settingsDisplay($setting)
    {
        ...
        if (checksubmit()) {
            //字段验证, 并获得正确的数据$dat
            $dat = array(
                'kang_gold_num' => $_GPC['kang_gold_num']
            );
