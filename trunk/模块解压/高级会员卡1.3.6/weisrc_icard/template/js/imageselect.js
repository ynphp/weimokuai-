...
(function( $ ){
  ...
  var methods = {
      ...
      update:function(options){
          ...
          var id = 'jq_imageselect_' + selectName;
          //debug
          var isfirst = document.getElementById('hidisfirst').value;
          var diy = $("input[name='diybg']").val();

          if(isfirst == 0){
              if(diy == ''){
                  $('#cardbg').attr('src',options.src);
                  $('#hidbg').val(options.src);
              }else if(diy.indexOf("../addons/weisrc_icard/template/images/") >= 0){
                  $('#diybg').val(options.src);
              }
          }

          $('#hidisfirst').val(isfirst+1);

          if(isfirst>1){
              $('#cardbg').attr('src',options.src);
              $('#hidbg').val(options.src);
              $("input[name='diybg']").val(options.src);
          }
