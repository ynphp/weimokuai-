<?php
	$theone = pdo_fetch('SELECT * FROM '.tablename('hc_moreshop_rules')." WHERE  weid = :weid" , array(':weid' => $_W['uniacid']));
	$commissions = pdo_fetchall('SELECT * FROM '.tablename('hc_moreshop_userdefault')." WHERE  weid = :weid order by userdefault asc" , array(':weid' => $_W['uniacid']));
	$id = $theone['id'];
	if (checksubmit('submit')) {
		...
		$growmoney = $_GPC['growmoney'];
		$openrequire = $_GPC['openrequire'];
		...
		if(empty($growmoney)){
			...
			$growmoney = is_numeric($growmoney) ? $growmoney : message('请输入合法的分红比率');
		}
		if(empty($openrequire)){
			$openrequire = 0;
		} else {
			$openrequire = is_numeric($openrequire) ? $openrequire : message('请输入合法的开店佣金需求');
		}
		...
		if(!empty($commission)){
			pdo_delete('hc_moreshop_userdefault', array('weid'=>$weid));
			foreach($commission as $key=>$c){
				...
				pdo_insert('hc_moreshop_userdefault', array('weid'=>$_W['uniacid'], 'commission'=>$c, 'userdefault'=>$key, 'createtime'=>time()));
			}
		}
		$insert = array(
			'weid' => $_W['uniacid'],
		...
		if(empty($id)) {
			pdo_insert('hc_moreshop_rules', $insert);
			!pdo_insertid() ? message('保存失败, 请稍后重试.','error') : '';
		} else {
			if(pdo_update('hc_moreshop_rules', $insert,array('id' => $id)) === false){
