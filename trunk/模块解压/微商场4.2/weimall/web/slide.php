<?php
	if(checksubmit('submit')) {
		...
		if (!empty($_GPC['attachment'])) {
			foreach ($_GPC['attachment'] as $index => $row) {
				...
				pdo_update('weimall_slide', $data, array('id' => $index));
			}
		}
	...
	if($op=='delete'){
		$id = intval($_GPC['id']);
		if (!empty($id)) {
			$item = pdo_fetch("SELECT * FROM " . tablename('weimall_slide') . " WHERE id = :id", array(':id' => $id));
			if (empty($item)) {
				message('图片不存在或是已经被删除！');
			}
			pdo_delete('weimall_slide', array('id' => $item['id']));
		} else {
	...
	$photos = pdo_fetchall("SELECT * FROM " . tablename('weimall_slide') . " WHERE weid = :weid ORDER BY listorder DESC", array(':weid' => $weid));
