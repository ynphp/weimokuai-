<?php
	...
	if($op=='display'){
		$activities = pdo_fetchall("select * from ".tablename('weimall_activity')." where weid = ".$weid."  order by displayorder desc");
	...
	if($_GPC['id']==''){
		...
		$activity = pdo_fetch('select * from '.tablename('weimall_activity')." where id = ".$_GPC['id']);
	}
	...
	if(checksubmit('submit') && $_GPC['op']=='posts'){
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('weimall_activity', array('displayorder' => $displayorder), array('id' => $id));
		}
		message('排序更新成功！', $this->createWebUrl('activity', array('op' => 'display', 'storeid'=>$storeid)), 'success');
	}
	if($op=='delete'){
		$temp = pdo_delete('weimall_activity', array('id'=>$_GPC['id'], 'storeid'=>$storeid));
		if($temp){
