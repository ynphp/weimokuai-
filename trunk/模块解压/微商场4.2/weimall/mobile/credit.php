<?php
	...
	if (!empty($_GPC['award_id']))
	{
		...
		$award_info = pdo_fetch("SELECT * FROM ".tablename('weimall_list')." WHERE award_id = $award_id AND weid = '{$weid}'");
		if ($fans['credit1'] >= $award_info['credit_cost'] && $award_info['amount'] > 0)
		{
			$data = array(
				'amount' => $award_info['amount'] - 1
			);
			pdo_update('weimall_list', $data, array('weid' => $weid, 'award_id' => $award_id));
			...
			pdo_insert('weimall_request', $data);
			...
			message('积分兑换成功！', create_url('mobile/module/mycredit', array('weid' => $weid, 'name' => 'weimall', 'do' => 'mycredit','op' => 'display')), 'success');
		}
		else
		{
			message('积分不足或商品已经兑空，请重新选择商品！<br>当前商品所需积分:'.$award_info['credit_cost'].'<br>您的积分:'.$fans['credit1']
				...
				create_url('mobile/module/award', array('weid' => $weid, 'name' => 'weimall')), 'error');
		}
	}
	else
	{
		message('请选择要兑换的商品！', create_url('mobile/module/award', array('weid' => $weid, 'name' => 'weimall')), 'error');
	}
?>