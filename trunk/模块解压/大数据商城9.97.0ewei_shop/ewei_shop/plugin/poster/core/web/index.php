...
if ($operation == "display") {
    ...
    foreach ($list as &$row) {
        $row["times"]   = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_poster_scan") . " where posterid=:posterid and uniacid=:uniacid", array(
            ":posterid" => $row["id"],
            ":uniacid" => $_W["uniacid"]
        ));
        $row["follows"] = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_poster_log") . " where posterid=:posterid and uniacid=:uniacid", array(
            ":posterid" => $row["id"],
            ":uniacid" => $_W["uniacid"]
        ));
    }
