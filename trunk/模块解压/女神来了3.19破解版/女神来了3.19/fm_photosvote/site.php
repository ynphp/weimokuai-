...
class Fm_photosvoteModuleSite extends WeModuleSite
{
		...
		if (!empty($rid)) {
			...
			if ($_GPC['do'] == 'photosvote' || $_GPC['do'] == 'tuser' || $_GPC['do'] == 'tuserphotos' || $_GPC['do'] == 'des' || $_GPC['do'] == 'reg' || $_GPC['do'] == 'paihang') {
				...
				if (!empty($tfrom_user)) {
					...
					pdo_update($this->table_reply_display, array('hits' => $reply['hits'] + 1), array('rid' => $rid));
				}
			}
	...
	public function gettiles($keyword = '')
	{
		...
		if (!empty($list)) {
			foreach ($list as $row) {
				...
				$urls[] = array('title' => $reply['title'], 'url' => $_W['siteroot'] . 'app/' . $this->createMobileUrl('photosvote', array('rid' => $row['id'])));
			}
		}
		return $urls;
	}
	...
	{
		$fmurl = 'http://api.zheyitianshi.com/api/qiniu/api.php?';
	...
	function get_share($uniacid, $rid, $from_user, $title)
	{
		...
		if (!empty($from_user)) {
			$userinfo = pdo_fetch("SELECT uid, nickname,realname FROM " . tablename($this->table_users) . " WHERE uniacid= :uniacid AND rid= :rid AND from_user= :from_user", array(':uniacid' => $uniacid, ':rid' => $rid, ':from_user' => $from_user));
	...
	{
		if ($isfm == 1) {
			$photo = pdo_fetch("SELECT photos FROM " . tablename($this->table_users_picarr) . " WHERE uniacid = :uniacid AND from_user = :from_user AND rid = :rid AND isfm = :isfm LIMIT 1", array(':uniacid' => $uniacid, ':from_user' => $from_user, ':rid' => $rid, ':isfm' => $isfm));
		} else {
			$photo = pdo_fetch("SELECT photos FROM " . tablename($this->table_users_picarr) . " WHERE uniacid = :uniacid AND from_user = :from_user AND rid = :rid ORDER BY createtime DESC LIMIT 1", array(':uniacid' => $uniacid, ':from_user' => $from_user, ':rid' => $rid));
		}
		return $photo;
	}
	...
	private function sendMobileVoteMsg($tuservote, $tousers, $template_id = '')
	{
		...
		$u = pdo_fetch("SELECT uid,realname, nickname, from_user, photosnum, xnphotosnum FROM " . tablename($this->table_users) . " WHERE uniacid = :uniacid  AND from_user = :from_user AND rid = :rid", array(':uniacid' => $uniacid, ':from_user' => $tuservote['tfrom_user'], ':rid' => $rid));
	...
	private function sendMobileMsgtx($from_user, $rid, $uniacid)
	{
		...
		$msgs = pdo_fetch('SELECT from_user,tfrom_user, content,createtime FROM ' . tablename($this->table_bbsreply) . ' WHERE uniacid= :uniacid and rid = :rid AND from_user = :from_user  LIMIT 1', array(':uniacid' => $uniacid, ':rid' => $rid, ':from_user' => $from_user));
	...
	public function doWebSettuijian()
	{
		...
		if (in_array($type, array('tuijian'))) {
			$data = ($data == 1 ? '0' : '1');
			pdo_update($this->table_users, array('istuijian' => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
			die(json_encode(array('result' => 1, 'data' => $data)));
		}
		if (in_array($type, array('limitsd'))) {
			pdo_update($this->table_users, array('limitsd' => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
			die(json_encode(array('result' => 1, 'data' => $data)));
		}
	...
	public function getMember($from_user)
	{
		...
		return pdo_fetch('SELECT * FROM ' . tablename($this->table_users) . " WHERE uniacid = :uniacid and  from_user = :from_user ORDER BY id DESC LIMIT 1", array(':uniacid' => $uniacid, ':from_user' => $from_user));
	}
	...
	{
		$str = array('(#呵呵)' => '<span class="smile_popo" style="background-position-y: -0px;display: inline-block;  width: 30px"></span>', '(#哈哈)' => '<span class="smile_popo" style="background-position-y: -30px;display: inline-block;  width: 30px"></span>', '(#吐舌)' => '<span class="smile_popo" style="background-position-y: -60px;display: inline-block;  width: 30px"></span>', '(#啊)' => '<span class="smile_popo" style="background-position-y: -90px;display: inline-block;  width: 30px"></span>', '(#酷)' => '<span class="smile_popo" style="background-position-y: -120px;display: inline-block;  width: 30px"></span>', '(#怒)' => '<span class="smile_popo" style="background-position-y: -150px;display: inline-block;  width: 30px"></span>', '(#开心)' => '<span class="smile_popo" style="background-position-y: -180px;display: inline-block;  width: 30px"></span>', '(#汗)' => '<span class="smile_popo" style="background-position-y: -210px;display: inline-block;  width: 30px"></span>', '(#泪)' => '<span class="smile_popo" style="background-position-y: -240px;display: inline-block;  width: 30px"></span>', '(#黑线)' => '<span class="smile_popo" style="background-position-y: -270px;display: inline-block;  width: 30px"></span>', '(#鄙视)' => '<span class="smile_popo" style="background-position-y: -300px;display: inline-block;  width: 30px"></span>', '(#不高兴)' => '<span class="smile_popo" style="background-position-y: -330px;display: inline-block;  width: 30px"></span>', '(#真棒)' => '<span class="smile_popo" style="background-position-y: -360px;display: inline-block;  width: 30px"></span>', '(#钱)' => '<span class="smile_popo" style="background-position-y: -390px;display: inline-block;  width: 30px"></span>', '(#疑问)' => '<span class="smile_popo" style="background-position-y: -420px;display: inline-block;  width: 30px"></span>', '(#阴险)' => '<span class="smile_popo" style="background-position-y: -450px;display: inline-block;  width: 30px"></span>', '(#吐)' => '<span class="smile_popo" style="background-position-y: -480px;display: inline-block;  width: 30px"></span>', '(#咦)' => '<span class="smile_popo" style="background-position-y: -510px;display: inline-block;  width: 30px"></span>', '(#委屈)' => '<span class="smile_popo" style="background-position-y: -540px;display: inline-block;  width: 30px"></span>', '(#花心)' => '<span class="smile_popo" style="background-position-y: -570px;display: inline-block;  width: 30px"></span>', '(#呼~)' => '<span class="smile_popo" style="background-position-y: -600px;display: inline-block;  width: 30px"></span>', '(#笑眼)' => '<span class="smile_popo" style="background-position-y: -630px;display: inline-block;  width: 30px"></span>', '(#冷)' => '<span class="smile_popo" style="background-position-y: -660px;display: inline-block;  width: 30px"></span>', '(#太开心)' => '<span class="smile_popo" style="background-position-y: -690px;display: inline-block;  width: 30px"></span>', '(#滑稽)' => '<span class="smile_popo" style="background-position-y: -720px;display: inline-block;  width: 30px"></span>', '(#勉强)' => '<span class="smile_popo" style="background-position-y: -750px;display: inline-block;  width: 30px"></span>', '(#狂汗)' => '<span class="smile_popo" style="background-position-y: -780px;display: inline-block;  width: 30px"></span>', '(#乖)' => '<span class="smile_popo" style="background-position-y: -810px;display: inline-block;  width: 30px"></span>', '(#睡觉)' => '<span class="smile_popo" style="background-position-y: -840px;display: inline-block;  width: 30px"></span>', '(#惊哭)' => '<span class="smile_popo" style="background-position-y: -870px;display: inline-block;  width: 30px"></span>', '(#升起)' => '<span class="smile_popo" style="background-position-y: -900px;display: inline-block;  width: 30px"></span>', '(#惊讶)' => '<span class="smile_popo" style="background-position-y: -930px;display: inline-block;  width: 30px"></span>', '(#喷)' => '<span class="smile_popo" style="background-position-y: -960px;display: inline-block;  width: 30px"></span>', '(#爱心)' => '<span class="smile_popo" style="background-position-y: -990px;display: inline-block;  width: 30px"></span>', '(#心碎)' => '<span class="smile_popo" style="background-position-y: -1020px;display: inline-block;  width: 30px"></span>', '(#玫瑰)' => '<span class="smile_popo" style="background-position-y: -1050px;display: inline-block;  width: 30px"></span>', '(#礼物)' => '<span class="smile_popo" style="background-position-y: -1080px;display: inline-block;  width: 30px"></span>', '(#彩虹)' => '<span class="smile_popo" style="background-position-y: -1110px;display: inline-block;  width: 30px"></span>', '(#星星月亮)' => '<span class="smile_popo" style="background-position-y: -1140px;display: inline-block;  width: 30px"></span>', '(#太阳)' => '<span class="smile_popo" style="background-position-y: -1170px;display: inline-block;  width: 30px"></span>', '(#钱币)' => '<span class="smile_popo" style="background-position-y: -1200px;display: inline-block;  width: 30px"></span>', '(#灯泡)' => '<span class="smile_popo" style="background-position-y: -1230px;display: inline-block;  width: 30px"></span>', '(#茶杯)' => '<span class="smile_popo" style="background-position-y: -1260px;display: inline-block;  width: 30px"></span>', '(#蛋糕)' => '<span class="smile_popo" style="background-position-y: -1290px;display: inline-block;  width: 30px"></span>', '(#音乐)' => '<span class="smile_popo" style="background-position-y: -1320px;display: inline-block;  width: 30px"></span>', '(#haha)' => '<span class="smile_popo" style="background-position-y: -1350px;display: inline-block;  width: 30px"></span>', '(#胜利)' => '<span class="smile_popo" style="background-position-y: -1380px;display: inline-block;  width: 30px"></span>', '(#大拇指)' => '<span class="smile_popo" style="background-position-y: -1410px;display: inline-block;  width: 30px"></span>', '(#弱)' => '<span class="smile_popo" style="background-position-y: -1440px;display: inline-block;  width: 30px"></span>', '(#OK)' => '<span class="smile_popo" style="background-position-y: -1470px;display: inline-block;  width: 30px"></span>',);
