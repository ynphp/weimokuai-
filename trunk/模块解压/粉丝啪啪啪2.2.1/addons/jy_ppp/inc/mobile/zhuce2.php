...
global $_W,$_GPC;
		...
		else
		{
			...
			{
				echo "<script>
					window.location.href = '".$this->createMobileUrl('userinfo',array('op'=>'zhuce2','id'=>$_GPC['sex']))."';
				</script>";
			}
			else
			{
		...
		if($op=='wechat')
		{
			...
			if(!empty($sex))
			{
				if(!empty($from_user) && $weixin==1)
				{
					...
					$wechat_member_temp=pdo_fetch("SELECT id FROM ".tablename('jy_ppp_member')." WHERE weid=".$weid." AND from_user='".$from_user."' AND type=1 ");
