<?php
	  ...
	  $psize = 20;
        ...
        $where = " weid = :weid AND rid = :rid AND actid = :actid";
				...
				if(!empty($_GPC['nickname'])){
					$openid = pdo_fetchcolumn("SELECT `openid` FROM".tablename($user_table)." WHERE rid = '{$id}' AND actid ='{$actid}' AND nickname LIKE '{$_GPC['nickname']}'");
					
					$where .= " AND openid = '{$openid}' ";
			  }
			  
				$sql = "SELECT * FROM ".tablename($cj_table)." WHERE {$where} ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
				$lists = pdo_fetchall($sql,$params);
				foreach($lists as &$row){
						$it = pdo_fetch("SELECT `nickname`,`headimgurl` FROM".tablename($user_table)." WHERE openid = :openid AND rid = :rid AND actid = :actid",array(':openid'=>$row['openid'],':rid'=>$id,':actid'=>$actid));
						$row['nickname'] = $it['nickname'];
						$row['headimgurl'] = $it['headimgurl'];
				}
				unset($row);
				$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($cj_table) . " WHERE {$where} ", $params);
			  $pager = pagination($total, $pindex, $psize);
		   }elseif($op == 'delete'){
					$luckid = intval($_GPC['luckid']);
					if(empty($luckid)){
		           message('删除项目不存在',$this->createWebUrl("luckuser",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"error");
					}
					pdo_delete($cj_table,array('id'=>$luckid,'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					message('删除成功',$this->createWebUrl("luckuser",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"success");
			 }else{
			   message('访问错误');
			 }
				...
				if(checksubmit('delete')){
					...
					$select = $_GPC['select'];
					if(empty($select)){
						message('请选择删除项',$this->createWebUrl("luckuser",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"error");
					}
					foreach ($select as $se) {
						pdo_delete($cj_table,array('id'=>$se,'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					}
					message('批量删除成功',$this->createWebUrl("luckuser",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"success");
				}
    ...
    include $this->template('luckuser');
