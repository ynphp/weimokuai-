...
class Lianhu_schoolModuleSite extends WeModuleSite
{
    ...
    {
        $school_id   = $_SESSION['school_id'];
        $line_status = pdo_fetchcolumn("select line_status from  " . $this->table_pe . "lianhu_school where  school_id=:sid", array(
            ":sid" => $school_id
        ));
        return $line_status;
    }
    ...
    {
        return pdo_fetch("select * from " . $this->table_pe . "lianhu_student where student_id=:sid ", array(
            ":sid" => $student_id
        ));
    }
    ...
    {
        $info = pdo_fetch("select * from " . $this->table_pe . "lianhu_jilv_class where class_id=:cid ", array(
            ':cid' => $class_id
        ));
        return $info;
    }
    ...
    public function classTeacher($class_id)
    {
        ...
        $result      = pdo_fetchall("select * from " . $this->table_pe . "lianhu_teacher where 
                     teacher_other_power like :power and status=1  {$other_where}", array(
            ':power' => "%{$class_id}%"
        ));
        return $result;
    }
    ...
    {
        $other_where = " and " . $this->where_uniacid_school;
        $course_id   = pdo_fetchcolumn("select course_id from " . $this->table_pe . "lianhu_course where course_name=:name {$other_where}", array(
            ':name' => $course_name
        ));
        if (!$course_id)
            return 'no';
    ...
    public function teacher_standard($model = '')
    {
        ...
        if ($group_name == '教师组') {
            ...
            if ($teacher_result['status'] == 0)
                message('该教师账号已经在智慧学校注销，不能登陆', '', 'error');
    ...
    public function mobile_from_find_student($to_bd = true)
    {
        ...
        if ($_SESSION['student_id'] || $_GPC['student_id'])
            ...
            $where = '';
        $result = pdo_fetch("select stu.*, class.class_name ,grade.grade_name, tea.teacher_realname,tea.teacher_id from       
                           " . $this->table_pe . "lianhu_student stu 
			               left join " . $this->table_pe . "lianhu_class class 
                                on class.class_id=stu.class_id left join " . $this->table_pe . "lianhu_grade grade on grade.grade_id=class.grade_id
			                left join  " . $this->table_pe . "lianhu_teacher tea 
                                on tea.teacher_id=class.teacher_id
    ...
    {
        global $_W;
        $list = pdo_fetchall("select ta.*,tea.teacher_realname,tea.teacher_id from " . $this->table_pe . "lianhu_work  ta left join " . $this->table_pe . "lianhu_teacher tea on tea.fanid=ta.teacher_id where student_id=:sid and jilv_class=:cid order by addtime desc", array(
            ':sid' => $sid,
            ':cid' => $jilv_class_id
        ));
        return $list;
    ...
    public function send_record_msg($sid, $type, $intro = '', $url, $mobile = true)
    {
        global $_W;
        if ($mobile) {
            $teacher_info = $this->teacher_mobile_qx();
            $tea_result   = pdo_fetch("select tea.* ,users.username,users.uid from " . $this->table_pe . "lianhu_teacher tea left join " . tablename('users') . " users on  users.uid=tea.fanid where tea.teacher_id=:tid", array(
                ":tid" => $teacher_info['teacher_id']
            ));
        } else {
            $teacher = $this->teacher_qx('no');
            if ($teacher == 'teacher')
                $tea_result = pdo_fetch("select * from " . $this->table_pe . "lianhu_teacher where fanid=:uid", $_W['uid']);
        }
        if (!$tea_result)
            $admin_name = '管理员';
        else
        ...
        foreach ($openids as $key => $value) {
            if ($value) {
                ...
                $first   = '您好，' . $type . '更新了，请查看';
                $key2    = $admin_name;
                $key4    = $type . '更新了';
                $remark  = "敬请留意";
                $mu_info = $this->decodeMsg1($first, $key2, $key4, $remark);
                $accObj->sendTplNotice($value, $mu_info['mu_id'], $mu_info['data'], $url);
            }
        }
    }
    public function sendLeaveMsg($leave_id)
    {
        $leave_result = pdo_fetch("select * from " . $this->table_pe . "lianhu_leave where leave_id=:id ", array(
            ':id' => $leave_id
        ));
        $stu_result   = pdo_fetch("select * from " . $this->table_pe . "lianhu_student where student_id=:sid ", array(
            ':sid' => $leave_result['student_id']
        ));
        $class_name   = $this->className($stu_result['class_id']);
        $tea_result   = pdo_fetch("select tea.* ,users.username,users.uid from " . $this->table_pe . "lianhu_teacher tea left join " . tablename('users') . " users on  users.uid=tea.fanid where tea.teacher_id=:tid", array(
            ":tid" => $leave_result['teacher_id']
        ));
        if (!$tea_result)
            $admin_name = '管理员';
        else
            $admin_name = $tea_result['teacher_realname'];
        $openids = $this->returnEfficeOpenid($stu_result, 3);
        load()->classs('weixin.account');
        $accObj = WeixinAccount::create($acid);
        ...
        foreach ($openids as $key => $value) {
            if ($value) {
                $first   = '家长您好，您的孩子请假申请，老师回复了';
                $key2    = $admin_name;
                $key4    = $word;
                $remark  = '备注：' . $leave_result['teacher_text'];
                $mu_info = $this->decodeMsg1($first, $key2, $key4, $remark);
                $accObj->sendTplNotice($value, $mu_info['mu_id'], $mu_info['data'], $url);
            }
        }
    ...
    public function send_class_msg($hid, $que_num)
    {
        ...
        foreach ($student_list as $key => $value) {
            ...
            $first                         = $value['student_name'] . '的家长您好,发布了新的作业';
            $key2                          = $homework_info['teacher_name'];
            $key4                          = '课程：' . $homework_info['course_name'] . '的作业发布了';
            $remark                        = "请督促您的孩子完成作业！";
            $mu_info                       = $this->decodeMsg1($first, $key2, $key4, $remark);
            ...
            $que_num                       = $this->insertMsgQueueMu($openid, $mu_info['data'], $mu_info['mu_id'], $url, $que_num);
        }
        return $que_num;
    }
    ...
    public function getLineList($page = 1, $page_size = 10, $class_id)
    {
        ...
        $list = pdo_fetchall("select " . $this->table_pe . "lianhu_send.*,mc_members.nickname,mc_members.avatar from " . $this->table_pe . "lianhu_send left join " . tableName('mc_members') . " mc_members 
         on mc_members.uid=" . $this->table_pe . "lianhu_send.send_uid where 
		  {$and_where}
		 and class_id=:cid order by add_time desc {$limit} ", array(
            ':cid' => $class_id
        ));
        return $list;
    }
    ...
    public function getJiaToken($fouce_get = false)
    {
        global $_W;
        $config = $this->module['config'];
        if (!$config['jia_passport'] || !$config['jia_password']) {
            $data['err_type']  = 1;
            ...
            $db_re      = pdo_fetch("select * from {$this->table_pe}lianhu_wechat where type=3 and addtime >{$time_begin} order by wechat_id desc ");
            $token      = $db_re['code'];
            if ($fouce_get || !$token) {
                $data['passport'] = $config['jia_passport'];
                $data['password'] = md5($config['jia_password']);
    ...
    /*public function checkJiaToken()
    {
        ...
        if ($token) {
            $result = $this->curlPost($this->jia_yun . 'api/checkTokenEff', array(
                "token" => $token
            ));
    ...
    {
        $list = pdo_fetchall("select * from " . $this->table_pe . "lianhu_jilv_class where class_status =1 ");
        return $list;
    }
    public function getTodayCourse($teacher_id)
    {
        $class_list = $this->getTeacherClass($teacher_id, true);
    ...
    public function getTeacherOneClass($teacher_id, $class_id)
    {
        ...
        $result       = pdo_fetch("select * from " . $this->table_pe . "lianhu_syllabus where class_id=:cid order by addtime desc", array(
            ":cid" => $class_id
        ));
    ...
    public function findCourseTime($key_word, $i)
    {
        ...
        $pm_much        = $this->module['config']['pm_much'][$_SESSION['school_id']];
        $ye_much        = $this->module['config']['ye_much'][$_SESSION['school_id']];
        $time_re        = pdo_fetch("select * from " . $this->table_pe . "lianhu_set  where uniacid=:uid and school_id=:sid order by set_id desc ", array(
            ":uid" => $_W['uniacid'],
            ':sid' => $_SESSION['school_id']
        ));
    ...
    public function decodeMsg($first, $key2, $key4, $remark)
    {
        ...
        $mu_id01 = $this->module['config']['msg01'];
        if ($mu_id) {
            $data = array(
                'first' => array(
                    'value' => $first,
            ...
            $data  = array(
                ...
                'keyword2' => array(
                    'value' => $key2 . "在" . date("Y-m-d H:i:s", TIMESTAMP) . "发布了" . $key4,
                    'color' => '#173177'
                ),
                'remark' => array(
                    'value' => $remark,
    ...
    public function decodeMsg1($first, $key2, $key4, $remark)
    {
        $mu_id1  = $this->module['config']['msg1'];
        $mu_id11 = $this->module['config']['msg11'];
        if ($mu_id1) {
            $data = array(
                'first' => array(
                    'value' => $first,
