<?php
 class Fans{
    ...
    public function batchGet($weid, $conds, $key, $pindex, $psize, $fields = array('openid as from_user', 'b.nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime')){
        ...
        $list = pdo_fetchall("SELECT {$fields_str} FROM " . tablename(self :: $t_sys_fans) . " a, " . tablename('mc_members') . " b WHERE acid = :acid AND a.uid = b.uid {$condition} ORDER BY a.updatetime DESC, b.createtime DESC " . " LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':acid' => $weid), $key);
    ...
    private function sysAddCredit($from_user, $credit_value, $credittype, $tag){
        ...
        mc_credit_update($uid, 'credit' . $credittype, $credit_value, $tag);
    }
