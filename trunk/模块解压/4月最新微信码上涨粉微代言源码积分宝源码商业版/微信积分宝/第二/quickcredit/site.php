    ...
    public function doWebGoods(){
        ...
        if ($operation == 'post'){
            ...
            if (checksubmit('submit')){
                ...
                $data = array('weid' => $_W['weid'], 'title' => $_GPC['title'], 'logo' => $_GPC['logo'], 'deadline' => $_GPC['deadline'], 'amount' => $amount, 'per_user_limit' => $per_user_limit, 'vip_require' => $vip_require, 'cost' => $cost, 'price' => $price, 'type' => $type, 'content' => $_GPC['content'], 'createtime' => TIMESTAMP,);
                if (!empty($goods_id)){
            ...
            $condition = '';
            $list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_goods) . " WHERE weid = '{$_W['weid']}' $condition ORDER BY createtime DESC");
        }
    ...
    public function doMobileExchange(){
        ...
        if (!empty($_GPC['goods_id'])){
            ...
            if (!empty($replicated)){
                ...
                message("您在{$last_time}已经成功兑换【{$goods_info['title']}】。5分钟内不能重复兑换相同物品", $this -> createMobileUrl("MyRequest"), "error");
            }
