<?php
 class Order{
    ...
    public function getDetailedGoods($orderid, $key = null){
        $goods = pdo_fetchall("SELECT g.support_delivery, g.goodstype, g.maxbuy, g.id, g.totalcnf, g.status, g.sales, g.title, g.thumb, g.unit, g.marketprice,g.productprice, g.costprice, g.total goodstotal, o.total,o.optionid,o.total ordertotal, o.optionname, o.price FROM " . tablename(self :: $t_order_goods) . " o " . " LEFT JOIN " . tablename(self :: $t_goods) . " g " . " ON o.goodsid=g.id " . " WHERE o.orderid='{$orderid}'");
        return $goods;
    }
    ...
    public function batchGetOrderGoodsByOpenIds($weid, $openids, $conds = array(), $key = null, $pindex = 1, $psize = 9999999){
        ...
        $condition .= " AND a.from_user IN ('" . join("','", $openids) . "')";
        $orders = pdo_fetchall("SELECT c.*, b.*, a.* FROM " . tablename(self :: $t_order) . " a " . " LEFT JOIN " . tablename(self :: $t_order_goods) . " c ON a.weid=c.weid AND a.id =c.orderid " . " LEFT JOIN " . tablename(self :: $t_sys_fans) . " b ON a.weid=b.uniacid AND a.from_user=b.openid" . " WHERE a.weid = $weid $condition ORDER BY a.id DESC " . " LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(), $key);
        return $orders;
    }
    ...
    public static function getPayTypeName($paytype){
        ...
        case self :: $PAY_CREDIT: return '积分支付';
        default: return '未知';
        }
    }
    public static function getOrderStatusName($status, $goodstype = 1){
        if ($goodstype == 2){
            ...
            case self :: $ORDER_PAYED: return '后台确认中';
