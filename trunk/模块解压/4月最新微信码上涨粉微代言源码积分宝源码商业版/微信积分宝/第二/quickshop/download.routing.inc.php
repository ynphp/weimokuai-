...
$pindex = 1;
$psize = 1024000;
list($list, $total) = $_order -> batchGet($_W['weid'], array('status' => $status, 'allstatus' => !isset($status), 'sendtype' => $sendtype), null, $pindex, $psize);
if (!empty($list)){
    foreach ($list as & $row){
...
$cmap = array('id' => 'A', 'ordersn' => 'B', 'status' => 'C', 'realname' => 'D', 'mobile' => 'E', 'openid' => 'F', 'address' => 'G', 'price' => 'H', 'createtime' => 'I', 'detail' => 'J', 'remark' => 'K', 'paytype' => 'L');
$objPHPExcel -> setActiveSheetIndex(0) -> setCellValue($cmap['id'] . '1', 'ID') -> setCellValue($cmap['ordersn'] . '1', '订单号') -> setCellValue($cmap['status'] . '1', '状态') -> setCellValue($cmap['realname'] . '1', '姓名') -> setCellValue($cmap['mobile'] . '1', '手机') -> setCellValue($cmap['openid'] . '1', 'OPENID') -> setCellValue($cmap['address'] . '1', '快递地址') -> setCellValue($cmap['price'] . '1', '总价') -> setCellValue($cmap['createtime'] . '1', '下单时间') -> setCellValue($cmap['detail'] . '1', '订单详情') -> setCellValue($cmap['remark'] . '1', '备注') -> setCellValue($cmap['paytype'] . '1', '付款方式') ;
$i = 2;
