...
class QuickLinkModuleProcessor extends WeModuleProcessor{
    ...
    private function respondScan($fans, $rule){
        ...
        if ($_follow -> isNewUser($_W['weid'], $follower)){
            ...
            $_follow -> processSubscribe($_W['weid'], $leader, $follower, $qr['channel']);
            $this -> notifyUpLevelFollow($_weapi, $_follow, $_fans, $_W['weid'], $leader, $channel);
            $this -> notifyLeaderFollow($_weapi, $_fans, $_W['weid'], $leader, $follower, $channel);
        }
        $this -> notifyLeaderScan($_weapi, $_fans, $_W['weid'], $leader, $follower, $channel);
        $response = array();
        $channel['title'] = $_parser -> parse($fans, $channel['title']);
        $channel['desc'] = $_parser -> parse($fans, $channel['desc']);
