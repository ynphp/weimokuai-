...
if($originCredit<=0){
	exit(json_encode(array("status"=>2,"info"=>"原有积分不足哦，快去赚取积分吧")));
}else if($originCredit<$changenum){
	exit(json_encode(array("status"=>3,"info"=>"兑换积分不能大于原有积分哦")));
}
load()->model('account');
...
if(!empty($account)&&!empty($account['payment']['wechat'])){
	...
	if(empty($cfg['nbfwxpaypath'])){
		exit(json_encode(array("status"=>4,"info"=>"公众号未开启支付，请联系管理员处理")));
	}
	...
	try {
		...
		pdo_insert($this->paylogtable,array("uniacid"=>$_W["uniacid"],"dwnick"=>$_W['fans']['tag']['nickname'],"dopenid"=>$_W['fans']['from_user'],"dtime"=>time(),
				"dcredit"=>$changenum,"dtotal_amount"=>$dtotal_amount,"dmch_billno"=>$mch_billno,"dissuccess"=>$dissuccess,"dresult"=>$dresult));
		pdo_commit();
		if(IS_DEBUG){
		...
		pdo_rollback();
		exit(json_encode(array("status"=>5,"info"=>$e->getMessage())));
	}
}else{
	exit(json_encode(array("status"=>4,"info"=>"公众号未开启支付，请联系管理员")));
}
