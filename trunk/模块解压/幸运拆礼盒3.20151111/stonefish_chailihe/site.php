...
class Stonefish_chailiheModuleSite extends WeModuleSite {	
	...
	function Message_tips($rid,$msg,$url=''){
        global $_W;
		$reply = pdo_fetch("select msgadpictime,msgadpic from ".tablename("stonefish_chailihe_reply")." where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
	...
	function Get_openid() {   
		...
		if($_W['account']['level']==4){
		    $fans_member = pdo_fetch("select follow,uid from " . tablename('mc_mapping_fans') . " where uniacid = :uniacid and acid = :acid and openid = :openid order by `fanid` asc", array(':uniacid' => $_W['uniacid'], ':acid' => $_W['acid'], ':openid' => $from_user['openid']));
		    if(!empty($fans_member)){
			    $_W['fans']['follow'] = $fans_member['follow'];
		        $_W['member']['uid'] = $fans_member['uid'];
		    }
		}elseif(!empty($_SESSION['openid'])){
			$fans_member = pdo_fetch("select follow,uid from " . tablename('mc_mapping_fans') . " where uniacid = :uniacid and acid = :acid and openid = :openid order by `fanid` asc", array(':uniacid' => $_W['uniacid'], ':acid' => $_W['acid'], ':openid' => $_SESSION['openid']));
		    if(!empty($fans_member)){
			    $_W['fans']['follow'] = $fans_member['follow'];
		        $_W['member']['uid'] = $fans_member['uid'];
		    }
	...
	function Get_UserInfo($power,$rid,$iid,$page_fromuser,$entrytype) {   
		...
		}elseif($setting['stonefish_chailihe_oauth']>=1 || $_W['account']['level']==4){
			$appUrl=$this->createMobileUrl('entry', array('rid' => $rid,'iid' => $iid,'from_user' => $page_fromuser,'entrytype' => $entrytype),true);
			$appUrl=substr($appUrl,2);
			$url = $_W['siteroot'] ."app/".$appUrl;
			header("location: $url");
	...
	//活动状态
	//会员以上类型验证
	function Check_fans($reply,$typeurl,$from_user='',$iid='') {
		global $_W;
		...
		if(!empty($members) && $members['status']==0) {
			$this->message_tips($reply['rid'],'会员已被锁定，请联系管理员');
		}
		if($reply['issubscribe']==3 && empty($mobile['mobile'])) {
			$this->message_tips($reply['rid'],'请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'mobile','url'=>url('entry//'.$typeurl,array('m'=>'stonefish_chailihe','rid'=>$reply['rid'],'fromuser' => $from_user,'iid' => $iid)))));
		}
		if($reply['issubscribe']==4 && empty($members['groupid'])) {
			$this->message_tips($reply['rid'],'请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'groupid','url'=>url('entry//'.$typeurl,array('m'=>'stonefish_chailihe','rid'=>$reply['rid'],'fromuser' => $from_user,'iid' => $iid)))));
		}
		if($reply['issubscribe']==5 && empty($members['districtid'])) {
			$this->message_tips($reply['rid'],'请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'districtid','url'=>url('entry//'.$typeurl,array('m'=>'stonefish_chailihe','rid'=>$reply['rid'],'fromuser' => $from_user,'iid' => $iid)))));
		}
		if(empty($members)) {
			$this->message_tips($reply['rid'],'请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//'.$typeurl,array('m'=>'stonefish_chailihe','rid'=>$reply['rid'],'fromuser' => $from_user,'iid' => $iid)))));
		}
		return true;
    }
	//会员以上类型验证
	function Fans_times($reply,$profile) {   
		...
		if($reply['opportunity']==1){
			...
			if(empty($profile['mobile'])){
				$this->message_tips($reply['rid'],'还没有注册成为会员，无法进入领取礼盒', url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$reply['rid'])))));
			}
			...
			if(!empty($doings)){
				if ($doings['status']<2) {
					$this->message_tips($reply['rid'],'抱歉，您的领取礼盒资格正在审核中',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
                }else{
					if($doings['awardcount'] == 0){
					    $this->message_tips($reply['rid'],'抱歉，您的领取礼盒机会已用完了或没有获得领取礼盒机会',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
					}else{
				...
				$this->message_tips($reply['rid'],'抱歉，您的还未获得领取礼盒资格',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
			}
			...
			$credit_times = intval($credit[$reply['credit_type']]/$reply['credit_value']);
			if($credit_times<1){
				$this->message_tips($reply['rid'],'没有足够的!'.$creditnames.'兑换领取礼盒机会了！',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
			}
    ...
    public function doMobileauth2(){
        ...
        if ($entrytype == "index") { // 粉丝参与活动
            ...
            $appUrl=$this->createMobileUrl('shareview', array('rid' => $rid,"fromuser" => $_GPC['from_user'],"iid" => $iid),true);
			$appUrl=substr($appUrl,2);
			$url = $_W['siteroot'] ."app/".$appUrl;
        }
        header("location: $url");
	...
	public function doMobileShareview() {
		...
		$acid = $_W['acid'];
		//礼盒访问
		$showlihe = pdo_fetch("select a.id,a.sharenum,a.liheid,a.prizeid,b.thumb1,b.thumb2,b.thumb3,b.shangjialogo,c.break,c.share_title,c.share_desc,c.share_img,c.viewnum,c.xuninum_time,c.xuninumtime,c.xuninum,c.xuninuminitial,c.xuninumending from " . tablename('stonefish_chailihe_fansaward') . " as a," . tablename('stonefish_chailihe_lihestyle') . " as b," . tablename('stonefish_chailihe_prize') . " as c where a.liheid = b.liheid and a.prizeid = c.id and a.id = :liheid", array(':liheid' => $liheid));
		$now = time();
		if($now-$showlihe['xuninum_time']>$showlihe['xuninumtime']){
		    pdo_update('stonefish_chailihe_prize', array('viewnum' => $showlihe['viewnum']+1,'xuninum_time' => $now,'xuninum' => $showlihe['xuninum']+mt_rand($showlihe['xuninuminitial'],$showlihe['xuninumending'])), array('id' => $showlihe['prizeid']));
		}else{
			pdo_update('stonefish_chailihe_prize', array('viewnum' => $showlihe['viewnum']+1), array('id' => $showlihe['prizeid']));
		}
		//礼盒访问
		...
		if(!empty($fans)){
			...
			$this->message_tips($rid,'抱歉，您的朋友没有参与本活动！请告之你的朋友，3秒后自动进入活动页！',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//参与分享人信息
		if($reply['endtime'] < time()){
            $this->message_tips($rid,$reply['end_description'].'不能再给你的朋友〖'.$realname.'〗，再助力加油了！by【'.$_W['account']['name'].'】');
        }
		//活动状态
		//增加人数，和浏览次数
		...
		if(empty($from_user)) {
			...
			//获取粉丝信息
			if($reply['power']==2){
			    $userinfo = $this->get_userinfo($reply['power'],$rid,$liheid,$page_fromuser,'shareview');
		    }
			//获取粉丝信息
		}
		if($from_user!=$fromuser){
			...
			if(empty($liheid)){
				if($reply['surprises']){
					$lihe = pdo_fetchall('SELECT id,prizeid FROM '.tablename('stonefish_chailihe_fansaward').'  WHERE rid = :rid and from_user = :from_user order by `openstatus` asc', array(':rid' => $rid,':from_user' => $fromuser));
					foreach ($lihe as $row) {
						$break = pdo_fetchcolumn("SELECT break FROM ".tablename('stonefish_chailihe_prize')." WHERE id = :id",array(':id'=>$row['prizeid']));
						if($break>$row['sharenum']){
							$liheid = $row['id'];
							break;
						}
					}
					if(empty($liheid)){
						$liheid = pdo_fetchcolumn('SELECT id FROM '.tablename('stonefish_chailihe_fansaward').'  WHERE rid = :rid and from_user = :from_user order by rand()', array(':rid' => $rid,':from_user' => $fromuser));
					}
				}else{
					$this->message_tips($rid,'抱歉，您的朋友没有从礼盒页分享给你，小微也不知道他要你帮他拆哪个礼盒呀！请微信告之他（她）吧！3秒后自动进入活动页！',url('entry//entry',array('m'=>'stonefish_chailihe','rid'=>$rid,'entrytype' => 'index')));
				}
			}			
			$isok = 1;//默认拆过礼盒		    
			if($reply['helpfans']==0) {
		...
		//分享信息
        $gohome = $_W['siteroot'] .'app/'.substr($this->createMobileUrl('index', array('rid' => $rid)),2);
		$openlihe = $_W['siteroot']."app/".substr($this->createMobileUrl('shareview', array('rid' => $rid,'iid' => $liheid,'fromuser' => $page_fromuser,'powertype' => 1),true),2);//点击打开礼盒
		$sharelink = $_W['siteroot'] .'app/'.substr($this->createMobileUrl('entry', array('rid' => $rid,'iid' => $liheid,'from_user' => $page_fromuser,'entrytype' => 'shareview')),2);
        $sharetitle = empty($showlihe['share_title']) ? '我参与了拆礼盒活动，您来帮我加油、助威吧！' : $showlihe['share_title'];
        $sharedesc = empty($showlihe['share_desc']) ? '亲，我参与了拆礼盒活动，您来帮我加油、助威吧！他们都要来了，就等你了！' : str_replace("\r\n"," ", $showlihe['share_desc']);
		$sharetitle = $this->get_share($rid,$from_user,$sharetitle);
		$sharedesc = $this->get_share($rid,$from_user,$sharedesc);
		if(!empty($showlihe['share_img'])){
		    $shareimg = toimage($showlihe['share_img']);
		}else{
	...
	public function doMobileindex() {
        ...
        $reply = pdo_fetch("select * from " . tablename('stonefish_chailihe_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
		$banner = pdo_fetchall("select * from " . tablename('stonefish_chailihe_banner') . " where rid = :rid order by displayorder desc,`id` asc", array(':rid' => $rid));
		$template = pdo_fetch("select * from " . tablename('stonefish_chailihe_template') . " where id = :id", array(':id' => $reply['templateid']));
		$share = pdo_fetch("select * from " . tablename('stonefish_chailihe_share') . " where rid = :rid and acid = :acid", array(':rid' => $rid,':acid' => $acid));
        //活动状态
		$this->check_reply($reply);
		...
		if(!empty($fans)){
			...
			//更新头像和昵称
			if($reply['power']==2){
				pdo_update('stonefish_chailihe_fans', array('avatar' => $userinfo['headimgurl'], 'nickname' => $userinfo['nickname']), array('id' => $fans['id']));
			}
			//更新头像和昵称
		}
		//查询是否参与活动并更新头像和昵称
		...
		//中奖名单
		//整理数据进行页面显示
		$regurl= $_W['siteroot']."app/".substr($this->createMobileUrl('reglihe', array('rid' => $rid,'headimgurl' => $userinfo['headimgurl'],'nickname' => $userinfo['nickname']),true),2);
	...
	public function doMobilereglihe() {
		...
		//是否结束
		if ($reply['endtime'] < time()) {
			$this->message_tips($rid,$reply['end_description'],url('entry//myaward',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//是否结束
		if($reply['issubscribe']>=1 && intval($_W['fans']['follow'])==0){
			//没有关注粉丝跳转至引导页
		...
		if($reply['issubscribe']==6){
			...
			if(!in_array($profile['groupid'], $grouparr)) {
				$this->message_tips($rid,$reply['sys_users_tips'],url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
			}
		}
		//验证系统会员组
		//获取粉丝信息
		...
		//判断总次数超过限制
        if ($fans['totalnum'] >= $reply['number_times'] && $reply['number_times'] > 0) {
           $this->message_tips($rid,'您已领取足够多的礼盒了！',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
        }
        //判断当日是否超过限制
        if ($fans['todaynum'] >= $reply['day_number_times'] && $reply['day_number_times'] > 0) {
            $this->message_tips($rid,'今天您已领取足够多的礼盒了,明天再来吧!',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
        }
		//判断是否中奖限制
		if($fans['awardnum']>=$reply['award_num']&&$reply['award_num']!=0){				
			$this->message_tips($rid,'您已中过大奖了，本活动仅限中奖'.$reply['award_num'].'次，谢谢！',url('entry//myaward',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//整理数据进行页面显示
		$regurl= $_W['siteroot']."app/".substr($this->createMobileUrl('reguser', array('rid' => $rid,'avatar' => $userinfo['headimgurl'],'nickname' => $userinfo['nickname']),true),2);
		$mylihe= $_W['siteroot']."app/".substr($this->createMobileUrl('mylihe', array('rid' => $rid),true),2);
		$gohome= $_W['siteroot']."app/".substr($this->createMobileUrl('entry', array('rid' => $rid,'entrytype' => 'index'),true),2);
		//整理数据进行页面显示
        //分享信息
	...
	public function doMobilereguser() {
		...
		//是否结束
		if ($reply['endtime'] < time()) {
			$this->Json_encode(array('errno'=>1,'error'=>$reply['end_description']));
		}
		//是否结束
		if($reply['issubscribe']>=1 && intval($_W['fans']['follow'])==0){
		...
		if($reply['issubscribe']==6){
			$grouparr = (array)iunserializer($reply['sys_users']);
			if(!in_array($profile['groupid'], $grouparr)) {
				$this->Json_encode(array('errno'=>1,'error'=>$reply['sys_users_tips']));
			}
		}
		//验证系统会员组
		//查询是活动定义的次数还是商户赠送次数
		...
		//判断总次数超过限制
        if ($fans['totalnum'] >= $reply['number_times'] && $reply['number_times'] > 0) {
		   $this->Json_encode(array('errno'=>1,'error'=>'您已领取足够多的礼盒了！'));
        }
        //判断当日是否超过限制
        if ($fans['todaynum'] >= $reply['day_number_times'] && $reply['day_number_times'] > 0) {
			$this->Json_encode(array('errno'=>1,'error'=>'今天您已领取足够多的礼盒了,明天再来吧!'));
        }
		//判断是否中奖限制
		if($fans['awardnum']>=$reply['award_num']&&$reply['award_num']!=0){				
			$this->Json_encode(array('errno'=>1,'error'=>'您已中过大奖了，本活动仅限中奖'.$reply['award_num'].'次，谢谢!'));
		}
		...
		if(!empty($fans)){
			...
			if($exchange['ismobile'] && $reply['mobileverify']==1){
				$mobileverify = pdo_fetch("select * FROM ".tablename("stonefish_chailihe_mobileverify")." where rid = :rid and uniacid = :uniacid and mobile = :mobile", array(':rid' => $rid,':uniacid' => $uniacid,':mobile' => $_GPC['mobile']));
				if(empty($mobileverify)){
					$this->json_encode(array("errno"=>1, "error"=>'未成功验证手机号，请确认手机号码！'));
				}elseif($mobileverify['verifytime']){
					$this->json_encode(array("errno"=>1, "error"=>'此手机号码已使用过，请确认手机号码！'));
				}elseif($mobileverify['status']!=2){
					$this->json_encode(array("errno"=>1, "error"=>'此手机号码还未审核，请等待管理员审核后再来参加！'));
				}
			}
			//验证手机号
			$fansdata = array(
			...
			//验证手机号并记录使用时间
			if($reply['mobileverify']==1){
			    pdo_update('stonefish_chailihe_mobileverify', array('verifytime' => time()), array('id' => $mobileverify['id']));
			}
			//验证手机号并记录使用时间
			//自动读取会员信息存入FANS表中
			$ziduan = array('realname','mobile','qq','email','address','gender','telephone','idcard','company','occupation','position');
		...
		if($exchange['tmplmsg_participate']){
			$this->seed_tmplmsg($from_user,$exchange['tmplmsg_participate'],$rid,array('do' =>'mylihe', 'nickname' =>$_GPC['nickname']));
		}
		//发送消息模板之参与模板
		//生成Session_token令牌
		$this->Session_token($from_user);
	...
	public function doMobileregliheshow() {
		...
		$template = pdo_fetch("select * from " . tablename('stonefish_chailihe_template') . " where id = :id", array(':id' => $reply['templateid']));
		$share = pdo_fetch("select * from " . tablename('stonefish_chailihe_share') . " where rid = :rid and acid = :acid", array(':rid' => $rid,':acid' => $acid));
		$showlihe = pdo_fetch("select b.liheid,b.thumb1,b.shangjialogo,c.break,c.share_title,c.share_desc,c.share_img from " . tablename('stonefish_chailihe_fansaward') . " as a," . tablename('stonefish_chailihe_lihestyle') . " as b," . tablename('stonefish_chailihe_prize') . " as c where a.liheid = b.liheid and a.prizeid = c.id and a.id = :liheid", array(':liheid' => $liheid));
		//活动状态
		$this->check_reply($reply);
		//活动状态
		if($reply['issubscribe']>=1 && intval($_W['fans']['follow'])==0){
	...
	public function doMobilemylihe() {
		...
		//判断是否有礼盒
		if(empty($listlihe)){
		    $this->message_tips($rid,'您没有领取过礼盒,快点去领取礼盒吧!',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//判断是否有礼盒
		$fans = pdo_fetch("select * from ".tablename('stonefish_chailihe_fans')." where rid = :rid and uniacid = :uniacid and from_user= :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
		//查询是活动定义的次数还是商户赠送次数
	...
	public function doMobileopenlihe() {
		...
		//获取openid
		if(empty($prizeid)){
			$this->message_tips($rid,'打开礼盒出错!',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		$reply = pdo_fetch("select * from " . tablename('stonefish_chailihe_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
		$exchange = pdo_fetch("select * FROM ".tablename("stonefish_chailihe_exchange")." where rid = :rid", array(':rid' => $rid));
		$template = pdo_fetch("select * from " . tablename('stonefish_chailihe_template') . " where id = :id", array(':id' => $reply['templateid']));
		...
		if($award['sharenum']>=$prize['break'] && $award['openstatus']==0){
			...
			$this->message_tips($rid,'还没有足够亲友团帮你拆开此礼盒!',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//判断是否参与过
		$fans = pdo_fetch("select * from ".tablename('stonefish_chailihe_fans')." where rid = :rid and uniacid = :uniacid and from_user= :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
		if(empty($fans)){
			$this->message_tips($rid,'系统出错,没有参与此活动!',url('entry//index',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}
		//判断是否参与过
		//拆开查看是否中奖
		if($openstatus==0){
			...
			if(($zgiftnum<$reply['award_num'] || $reply['award_num']==0) && ($daygiftnum<$prize['prizeday'] || $prize['prizeday']==0) && ($mygiftnum<$prize['prizeren'] || $prize['prizeren']==0)){
				if($prize['prizetotal']>$prize['prizedraw'] && $prize['probalilty']!=0){
					...
					if($reply['mobileverify']==2){
						$mobileverify = pdo_fetch("select * FROM ".tablename("stonefish_chailihe_mobileverify")." where rid = :rid and uniacid = :uniacid and mobile = :mobile", array(':rid' => $rid,':uniacid' => $uniacid,':mobile' => $fans['mobile']));
						if(!empty($mobileverify)){
					...
					//是否有福利
					$probalilty = 1000*floatval($prize['probalilty'])*$fuli;
					...
					if($this->get_rand($arr)){
						...
						pdo_update('stonefish_chailihe_fansaward', array('zhongjiang' => 1,'codesn' => $codesn,'tickettype'=>$exchange['tickettype'],'zhongjiangtime' => time()), array('id' => $award['id']));
						pdo_update('stonefish_chailihe_fans', array('awardnum' => $fans['awardnum'] + 1,'zhongjiang' => 1), array('id' => $fans['id']));
	...
	public function doMobileShare_confirm() {
		...
		$uniacid = $_W['uniacid'];
		$iid = intval($_GPC['iid']);
		$who = empty($_GPC['who']) ? '自己' : $_GPC['who'];
		$fromuser = authcode(base64_decode($_GPC['fromuser']), 'DECODE');
		//获取openid
		$openid = $this->get_openid();
		$from_user = $openid['openid'];
		...
		$reply = pdo_fetch("select power from " . tablename('stonefish_chailihe_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
	    //获取粉丝信息
		if($reply['power']==2){
		...
		pdo_insert('stonefish_chailihe_sharedata', $fanssharedata);
		$fans = pdo_fetch("select * from " . tablename('stonefish_chailihe_fans') . " where rid = :rid and uniacid = :uniacid and from_user = :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
		if ($fans == true) {
			//保存分享次数
	...
	public function doMobileFirend() {
		...
		//获取openid
		//助力
		$firend = pdo_fetchall("select * from " . tablename('stonefish_chailihe_sharedata') . " where rid = :rid and uniacid = :uniacid and fromuser = :from_user and share_type=1 order by `id` desc", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
...
select * from ".tablename('stonefish_chailihe_fansaward')." order by zhongjiang asc) as tt  where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang>=1 GROUP BY prizeid order by `id` desc", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
	...
	public function doMobileExchange() {
		...
		if($exchange['tickettype']==5){
		    ...
		    //减少库存
			$prizetime = pdo_fetchcolumn("select createtime FROM " . tablename('stonefish_chailihe_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and prizeid =:prizeid and zhongjiang>=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user, ':prizeid' => $awardid));
			//发送消息模板之奖记录
			if($exchange['tmplmsg_exchange']){
			...
			if($exchange['awardingpas']!=$password){
				...
				if($awardid=='all'){
					...
					//减少库存
					$prizetime = pdo_fetchcolumn("select createtime FROM " . tablename('stonefish_chailihe_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and zhongjiang>=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
					//发送消息模板之奖记录
					if($exchange['tmplmsg_exchange']){
	...
	public function doWebTrend() {
		...
		//礼盒领取量以及更新分享量
		foreach ($award as $mid => $da) {
			$award[$mid]['lingqunum'] = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_chailihe_fansaward') . " where rid = :rid and uniacid=:uniacid and prizeid=:prizeid", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':prizeid' => $da['id']));
			$share_num = $sharenum = 0;
			$share = pdo_fetchall("select * from " . tablename('stonefish_chailihe_fansaward') . " where prizeid = :prizeid", array(':prizeid' => $da['id']));
			foreach ($share as $shares) {
				$sharenum += pdo_fetchcolumn("select COUNT(id) from " . tablename('stonefish_chailihe_sharedata') . " where share_type = 1 and fid = :fid", array(':fid' => $shares['id']));
				$share_num += pdo_fetchcolumn("select COUNT(id) from " . tablename('stonefish_chailihe_sharedata') . " where share_type = 0 and fid = :fid", array(':fid' => $shares['id']));
			}
			pdo_update('stonefish_chailihe_prize', array('sharenum'=>$sharenum,'share_num'=>$share_num), array('id' => $da['id']));
			$award[$mid]['sharenum'] = $sharenum;
			$award[$mid]['share_num'] = $share_num;
		}
		//礼盒领取量以及更新分享量
		//中奖数量
	...
	public function doWebPrize() {
		...
		$reply = pdo_fetch("select mobileverify from ".tablename('stonefish_chailihe_reply')." where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//查询do参数
		if(empty($_GPC['do'])){
			$_GPC['do'] = pdo_fetchcolumn("select do from " . tablename('modules_bindings') . "  where eid = :eid and module=:module", array(':eid' => $_GPC['eid'], ':module' => 'stonefish_chailihe'));
		}
		...
		$total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_chailihe_prize') . "  where rid = :rid and uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 30;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_chailihe_prize') . " where rid = :rid and uniacid=:uniacid " . $where . " order by id asc " . $limit, $params);
		//查询奖品是否可以删除以及更新分享量
		foreach ($list as $mid => $lists) {
			$list[$mid]['fans'] = pdo_fetchcolumn("select COUNT(id) from " . tablename('stonefish_chailihe_fansaward') . " where prizeid = :prizeid", array(':prizeid' => $lists['id']));
			$share_num = $sharenum = 0;
			$share = pdo_fetchall("select * from " . tablename('stonefish_chailihe_fansaward') . " where prizeid = :prizeid", array(':prizeid' => $lists['id']));
			foreach ($share as $shares) {
				$sharenum += pdo_fetchcolumn("select COUNT(id) from " . tablename('stonefish_chailihe_sharedata') . " where share_type = 1 and fid = :fid", array(':fid' => $shares['id']));
				$share_num += pdo_fetchcolumn("select COUNT(id) from " . tablename('stonefish_chailihe_sharedata') . " where share_type = 0 and fid = :fid", array(':fid' => $shares['id']));
			}
			pdo_update('stonefish_chailihe_prize', array('sharenum'=>$sharenum,'share_num'=>$share_num), array('id' => $lists['id']));
			$list[$mid]['sharenum'] = $sharenum;
			$list[$mid]['share_num'] = $share_num;
		}
		//查询奖品是否可以删除以及更新分享量
	...
	public function doWebPrizeedit() {
		...
		//积分类型
		//礼盒样式
		$lihestyle = pdo_fetchall("SELECT * FROM " . tablename('stonefish_chailihe_lihestyle') . " WHERE uniacid = :uniacid OR uniacid=0 ORDER BY `liheid` asc", array(':uniacid' => $_W['uniacid']));
		//礼盒样式
		...
		$item['share_title'] = empty($item['share_title']) ? $share['share_title'] : $item['share_title'];
		$item['share_desc'] = empty($item['share_desc']) ? $share['share_desc'] : $item['share_desc'];
		$item['share_img'] = empty($item['share_img']) ? $share['share_img'] : $item['share_img'];
		$item['xuninum'] = !isset($item['xuninum']) ? "500" : $item['xuninum'];
		$item['xuninumtime'] = !isset($item['xuninumtime']) ? "86400" : $item['xuninumtime'];
		$item['xuninuminitial'] = !isset($item['xuninuminitial']) ? "10" : $item['xuninuminitial'];
		$item['xuninumending'] = !isset($item['xuninumending']) ? "50" : $item['xuninumending'];
		if(checksubmit('submit')) {
			if(empty($_GPC['prizetype'])){
			...
			$data = array(
				...
				'xuninum_time' => $_GPC['xuninum_time'],
			);
			if(!empty($id)) {
				pdo_update('stonefish_chailihe_prize', $data, array('id' => $id));
				message('奖品配置修改成功！', url('site/entry/prize', array('m' => 'stonefish_chailihe','rid' => $rid)), 'success');
			}else{
				pdo_insert('stonefish_chailihe_prize', $data);
				message('奖品配置添加成功！', url('site/entry/prize', array('m' => 'stonefish_chailihe','rid' => $rid)), 'success');
			}
		}
	...
	public function doWebPrizedelete() {
        ...
        $id = intval($_GPC['id']);
		$business = pdo_fetch("select * from ".tablename('stonefish_chailihe_prize')." where id = :id", array(':id' => $id));
        if(empty($business)){
		...
		//删除粉丝分享记录
		if($_GPC['replyid']=='yes'){
			message('奖品配置数据删除成功', url('platform/reply/post',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}else{
			message('奖品配置数据删除成功', url('site/entry/prize',array('rid' => $rid, 'm' => 'stonefish_chailihe')));
		}
    }
	//删除奖品配置数据
	//幻灯配置数据
	public function doWebBanner() {
		...
		if (!empty($_GPC['displayorder'])) {
			foreach ($_GPC['displayorder'] as $id => $displayorder) {
				...
				pdo_update('stonefish_chailihe_banner', $update, array('id' => $id));					
			}
			message('排序更新成功！', 'refresh', 'success');
		}
		//更新排序
		$params = array(':rid' => $rid, ':uniacid' => $_W['uniacid']);
		$total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_chailihe_banner') . "  where rid = :rid and uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 30;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_chailihe_banner') . " where rid = :rid and uniacid=:uniacid " . $where . " order by displayorder desc,id asc " . $limit, $params);
	...
	public function doWebBanneredit() {
		...
		if(checksubmit('submit')) {
			if(empty($_GPC['bannerpic'])){
				message('幻灯图片必需上传', referer(), 'error');
			}			
			$data = array(
				'uniacid'          => $_W['uniacid'],
				'rid'              => $rid,
				'bannerpic'        => $_GPC['bannerpic'],
				'bannerurl'        => $_GPC['bannerurl'],
				'displayorder'     => $_GPC['displayorder'],
				'bannertitle'      => $_GPC['bannertitle'],
				'isshow'           => $_GPC['isshow'],
		    );
			if(!empty($id)) {
				pdo_update('stonefish_chailihe_banner', $data, array('id' => $id));
				message('幻灯配置修改成功！', url('site/entry/banner', array('m' => 'stonefish_chailihe','rid' => $rid)), 'success');
			}else{
				pdo_insert('stonefish_chailihe_banner', $data);
				message('幻灯配置添加成功！', url('site/entry/banner', array('m' => 'stonefish_chailihe','rid' => $rid)), 'success');
			}
		}
        include $this->template('banneredit');
    }
	//幻灯配置数据修改
	//删除幻灯配置数据
	public function doWebBannerdelete() {
        global $_GPC, $_W;
		$rid = intval($_GPC['rid']);
		$reply = pdo_fetch("select * from ".tablename('stonefish_chailihe_reply')." where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        if(empty($reply)){
        ...
        $id = intval($_GPC['id']);
		$business = pdo_fetch("select * from ".tablename('stonefish_chailihe_banner')." where id = :id", array(':id' => $id));
        if(empty($business)){
			message('抱歉，选中的数据不存在！', referer(), 'error');
        }
		//删除粉丝分享记录
		...
		//删除粉丝分享记录
		if($_GPC['replyid']=='yes'){
			message('分享配置数据删除成功', url('platform/reply/post',array('m'=>'stonefish_chailihe','rid'=>$rid)));
		}else{
			message('分享配置数据删除成功', url('site/entry/banner',array('rid' => $rid, 'm' => 'stonefish_chailihe')));
		}		
    }
	//删除幻灯配置数据
	//消息通知记录
	public function doWebPosttmplmsg() {
		...
		$rid = intval($_GPC['rid']);
		$rid = empty($rid) ? intval($_GPC['id']) : $rid;
		$reply = pdo_fetch("select poweravatar,mobileverify from ".tablename('stonefish_chailihe_reply')." where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//查询do参数
		if(empty($_GPC['do'])){
			$_GPC['do'] = pdo_fetchcolumn("select do from " . tablename('modules_bindings') . "  where eid = :eid and module=:module", array(':eid' => $_GPC['eid'], ':module' => 'stonefish_chailihe'));
		}
		...
		//查询是否有商户网点权限
		//消息模板
		$tmplmsg = pdo_fetchall("SELECT * FROM " . tablename('stonefish_chailihe_tmplmsg') . " WHERE uniacid = :uniacid ORDER BY `id` asc", array(':uniacid' => $_W['uniacid']));
		//消息模板
		$params = array(':rid' => $rid, ':uniacid' => $_W['uniacid']);
        ...
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select a.id,a.from_user,a.tmplmsg,a.createtime,a.seednum,b.avatar,b.realname,b.nickname,b.mobile,c.template_name from " . tablename('stonefish_chailihe_fanstmplmsg') . " as a," . tablename('stonefish_chailihe_fans') . " as b," . tablename('stonefish_chailihe_tmplmsg') . " as c where a.rid = :rid and a.uniacid=:uniacid and a.from_user=b.from_user and c.id=a.tmplmsgid" . $where . " order by a.id desc " . $limit, $params);
		//是否为关注会员并发送消息
	...
	public function doWebTmplmsginfo() {
		...
		if($_W['isajax']) {
			load()->func('tpl');
			$id = intval($_GPC['id']);
			$fanstmplmsg = pdo_fetch("select * from ".tablename('stonefish_chailihe_fanstmplmsg')." where id = :id", array(':id' => $id));
			if(!empty($fanstmplmsg)){
				$data = pdo_fetch("select avatar,nickname from ".tablename('stonefish_chailihe_fans')." where from_user = :from_user and rid = :rid", array(':from_user' => $fanstmplmsg['from_user'],':rid' => $fanstmplmsg['rid']));
	...
	public function doWebSeedtmplmsg() {
        ...
        $access_token = $accObj->fetch_token();
		if (empty($access_token)) {
            message('系统出错！', url('site/entry/posttmplmsg',array('rid' => $rid, 'm' => 'stonefish_chailihe','page'=>intval($_GPC['page']))), 'error');
        }
		if($_GPC['all']=='yes'){
			foreach ($_GPC['idArr'] as $k => $id) {
			    ...
			    pdo_update('stonefish_chailihe_fanstmplmsg', $tmplmsgdata, array('id' => $id));
            }
		    ...
		    if(!empty($fanstmplmsg)){
			    ...
			    pdo_update('stonefish_chailihe_fanstmplmsg', $tmplmsgdata, array('id' => $id));
			    message('模板消息再次发送成功！', url('site/entry/posttmplmsg',array('rid' => $rid, 'm' => 'stonefish_chailihe','page'=>intval($_GPC['page']))), 'success');
			    //添加消息发送记录
		    }else{
			    message('模板消息内容出错！', url('site/entry/posttmplmsg',array('rid' => $rid, 'm' => 'stonefish_chailihe','page'=>intval($_GPC['page']))), 'error');
		    }
		}
    }	
	//再次发送模板消息
	...
	public function doWebDeletefans() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
			...
			//删除粉丝中奖记录			
			//删除粉丝中奖详细记录
			pdo_delete('stonefish_chailihe_fansaward', array('from_user' => $fans['from_user'],'rid' => $rid,'uniacid' => $_W['uniacid']));
			//删除粉丝中奖详细记录
			//删除粉丝分享详细记录
			pdo_delete('stonefish_chailihe_sharedata', array('fromuser' => $fans['from_user'],'rid' => $rid,'uniacid' => $_W['uniacid']));
			//删除粉丝分享详细记录
			//删除粉丝消息通知记录
			pdo_delete('stonefish_chailihe_fanstmplmsg', array('from_user' => $fans['from_user'],'rid' => $rid,'uniacid' => $_W['uniacid']));
			//删除粉丝消息通知记录
			//删除粉丝手机验证记录
			pdo_update('stonefish_chailihe_mobileverify',array('verifytime'=>''), array('mobile' => $fans['mobile'],'rid' => $rid,'uniacid' => $_W['uniacid']));
			//删除粉丝手机验证记录
			//删除粉丝参与记录
	...
	public function doWebSharelist() {
		...
		if($_W['isajax']) {
			...
			$rid = intval($_GPC['rid']);
			//规则
			$reply = pdo_fetch("select poweravatar,mobileverify FROM " . tablename('stonefish_chailihe_reply') . " where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
			//粉丝数据
			$data = pdo_fetch("select id, from_user  FROM " . tablename('stonefish_chailihe_fans') . ' where id = :id', array(':id' => $uid));
	...
	public function doWebSavexunishare() {
		...
		if(!$liheid){
		    message('必需选择一个未打开的礼盒', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}
		if(!$rid){
		    message('系统出错', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}
		if($uid) {
		    //规则
			$reply = pdo_fetch("select * FROM " . tablename('stonefish_chailihe_reply') . " where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
			...
			//设置此粉丝为虚拟中奖者
			message('添加虚拟助力量成功', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))));
		}else{
			message('未找到指定用户', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}       
    }
	//虚拟助力
	//虚拟奖品
	...
	public function doWebSavexuniaward() {
		...
		if(!$awardid){
		    message('必需选择奖品才能生效', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}
		if(!$rid){
		    message('系统出错', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}
		if($uid) {
			...
			message('添加虚拟奖品成功', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))));
		} else {
			message('未找到指定用户', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_chailihe', 'page' => intval($_GPC['page']))), 'error');
		}      
    }
	...
	public function doWebEditbranchsave() {
		...
		if($usecount>$awardcount){
		    message('修改后的次数少于已使用的次数', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
		}
		if(!$rid){
		    message('系统出错', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
		}
		if($uid) {
		    //次数
            pdo_update('stonefish_branch_doings', array('awardcount' => $awardcount,'status' => $status), array('id' => $uid));
			message('修改增送次数成功', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))));
		} else {
			message('未找到指定用户', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
		}
    }
	//修改商家网店增送记录
	//增送使用记录
	...
	public function doWebmobileverify() {
		...
		$total = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_chailihe_mobileverify') . "  where rid = :rid and uniacid=:uniacid ".$where."", $params);
        $psize = 20;
		$pagemax = ceil($total/$psize);
		$_GPC['page'] = $_GPC['page']>$pagemax ? $pagemax : $_GPC['page'];
        $pindex = max(1, intval($_GPC['page']));
        ...
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * FROM " . tablename('stonefish_chailihe_mobileverify') . " where rid = :rid and uniacid=:uniacid ".$where." ORDER BY id DESC " . $limit, $params);
	...
	public function doWebMobileverifyimportingsave() {
		...
		$rid = intval($_GPC['rid']);
		if(!$rid){
		    message('系统出错', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe')), 'error');
			exit;
		}
		if(empty($_FILES["inputExcel"]["tmp_name"])){
			message('系统出错', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe')), 'error');
        ...
        for ($row = 2;$row <= $highestRow;$row++){
            ...
            //插入数据
			$chongfu = pdo_fetch("select id FROM ".tablename('stonefish_chailihe_mobileverify')." where mobile =:mobile and uniacid=:uniacid and rid=:rid", array(':mobile' => $strs[0],':uniacid' => $_W['uniacid'],':rid' => $rid));
			$data = array(
					'uniacid' => $_W['uniacid'],
					'rid' => $rid,
					'realname' => $strs[0],
			...
			if (!empty($chongfu)){
				pdo_update('stonefish_chailihe_mobileverify', $data, array('id' => $chongfu['id']));
			}else{
				pdo_insert('stonefish_chailihe_mobileverify', $data);
			}
        }
        unlink($inputFileName); //删除上传的excel文件
        message('导入手机验证成功', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe')));
		exit;    
    }
	//导入手机验证记录
	//修改手机验证记录
	public function doWebAddmobileverify() {
		...
		if($_W['isajax']) {
			$rid = intval($_GPC['rid']);
			$op = 'add';
			$reply = pdo_fetch("select mobileverify FROM ".tablename('stonefish_chailihe_reply')." where rid = :rid", array(':rid' => $rid));
			$data['status'] =2;
	...
	public function doWebEditmobileverify() {
		...
		if($_W['isajax']) {
			...
			$rid = intval($_GPC['rid']);
			$reply = pdo_fetch("select mobileverify FROM ".tablename('stonefish_chailihe_reply')." where rid = :rid", array(':rid' => $rid));
			$data = pdo_fetch("select * FROM " . tablename('stonefish_chailihe_mobileverify') . ' where id = :id AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
			include $this->template('mobileverifyedit');
			exit();
		}
    }
	public function doWebEditmobileverifysave() {
		...
		$op = $_GPC['op'];
		if(!$rid){
		    message('系统出错', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
		}
		$reply = pdo_fetch("select mobileverify FROM ".tablename('stonefish_chailihe_reply')." where rid = :rid", array(':rid' => $rid));
		if (empty($_GPC['mobile'])){
			message('必需输入手机号', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
		}
		if($uid && empty($op)) {
		    //次数
			$chongfu = pdo_fetch("select id FROM ".tablename('stonefish_chailihe_mobileverify')." where mobile =:mobile and uniacid=:uniacid and rid=:rid and id<>:id", array(':mobile' => $_GPC['mobile'],':uniacid' => $_W['uniacid'],':rid' => $rid,':id' => $uid));
			if (empty($chongfu)){
				pdo_update('stonefish_chailihe_mobileverify', array('realname' => $_GPC['realname'],'mobile' => $_GPC['mobile'],'status' => $status), array('id' => $uid));
				if($reply['mobileverify']==2){
					pdo_update('stonefish_chailihe_mobileverify', array('welfare' => $_GPC['welfare']), array('id' => $uid));
				}
			    message('修改手机验证成功', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))));
			}else{
				message('此手机号已存在', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
			}
		}else{
			if(!empty($op)){
				$chongfu = pdo_fetch("select id FROM ".tablename('stonefish_chailihe_mobileverify')." where mobile =:mobile and uniacid=:uniacid and rid=:rid", array(':mobile' => $_GPC['mobile'],':uniacid' => $_W['uniacid'],':rid' => $rid));
			    if (empty($chongfu)){
					$data = array(
					    'uniacid' => $_W['uniacid'],
					    'rid' => $rid,
					...
					pdo_insert('stonefish_chailihe_mobileverify', $data);
			        message('添加手机验证成功', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))));
			    }else{
				    message('此手机号已存在', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
			    }
			}else{
				message('未找到指定用户', url('site/entry/mobileverify',array('rid' => $rid, 'm' => 'stonefish_chailihe','page' => intval($_GPC['page']))), 'error');
			}
		}
    }
	//修改手机验证记录
	...
	public function doWebSetmobileverifycheck() {
        ...
        if (in_array($type, array('status'))) {
            $data = ($data==2?'1':'2');
            pdo_update("stonefish_chailihe_mobileverify", array("status" => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
            die(json_encode(array("result" => 1, "data" => $data)));
        }
        die(json_encode(array("result" => 0)));
    }
	...
	public function doWebDeletemobileverify() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if ($id == 0)
                continue;
            //删除使用记录
			$doings = pdo_fetch("select * FROM " . tablename('stonefish_chailihe_mobileverify') . " where id = :id", array(':id' => $id));
			if(empty($doings)){
				continue;
			}
			//删除赠送记录
			pdo_delete('stonefish_chailihe_mobileverify', array('id' => $id));
			//删除赠送记录
        }
