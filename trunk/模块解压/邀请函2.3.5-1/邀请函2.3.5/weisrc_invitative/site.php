...
*/
    ...
    public function doMobileCheckSignupStatus()
    {
        ...
        if (!empty($item)) {
            $this->showMessageAjax('已经报名！', 1);
        } else {
            $this->showMessageAjax('未报名！');
        }
    }
    ...
    public function doMobileGuestinfo()
    {
        ...
        if (empty($from_user)) {
            $this->showMessageAjax('会话已过期，请重新发送关键字!');
        }
        ...
        if (!empty($user)) {
            $this->showMessageAjax('您已经报过名了!');
        }

        if ($activityid == 0) {
            $this->showMessageAjax('该活动不存在!');
        } else {
            ...
            if (empty($activity)) {
                ...
                $this->showMessageAjax('对不起，报名时间已过!');
            }
        }

        if (empty($username)) {
            $this->showMessageAjax('请输入用户名!');
        }
        if (empty($tel)) {
            $this->showMessageAjax('请输入联系电话!');
        }
    ...
    public function showMessageAjax($msg, $status = 0)
    {
