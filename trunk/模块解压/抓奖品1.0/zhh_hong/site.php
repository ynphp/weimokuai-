...
define("ZHH_HONG_RES", "../addons/" . ZHH_HONG . "/");
require_once IA_ROOT . "/addons/" . ZHH_HONG . "/dbutil.class.php";
require IA_ROOT . "/addons/" . ZHH_HONG . "/oauth2.class.php";

/**
 * Class Mon_JggModuleSite
 */
class Zhh_HongModuleSite extends WeModuleSite
{
    public $weid;
    public $acid;
    public $oauth;
    ...
    public function  doMobileZjp()
    {
        ...
        $ads=$this->getArrayMsg($zjp,Zhh_HongModuleSite::$MSG_TYPE_DIALOG);
    ...
    public function  createPlayRecored($zjp, $uid, $openid)
    {
        ...
        if (!empty($prize)) {
            $pid = $prize['id'];
            $award_status = 1;
            $msgArray=$this->getArrayMsg($zjp,Zhh_HongModuleSite::$MSG_TYPE_SUCCESS);
        } else {
            $pid = 0;
            $award_status = 0;
            $msgArray=$this->getArrayMsg($zjp,Zhh_HongModuleSite::$MSG_TYPE_FAIL);
        }
    ...
    public function  defaultImg($type)
    {
        ...
        return ZHH_HONG_RES . "images/" . $imgName;
    ...
    public function  getArrayMsg($zjp, $msg_type)
    {
        ...
        switch ($msg_type) {
            case Zhh_HongModuleSite::$MSG_TYPE_DIALOG://对话框
                $content = $zjp['dialog_tips'] == '' ? CRUD::$DIALOG_MSG : $zjp['dialog_tips'];
                break;
            case Zhh_HongModuleSite::$MSG_TYPE_SUCCESS://抽奖成功
                $content = $zjp['success_award_tips'] == '' ? CRUD::$SUCC_MSG : $zjp['success_award_tips'];
                break;
            case Zhh_HongModuleSite::$MSG_TYPE_FAIL://抽奖失败
