<?php
	...
	if($op=='output'){
		...
		$outcrediting = pdo_fetchall("SELECT c.*,m.realname,m.mobile,m.bankcard,m.banktype,m.alipay,m.wxhao FROM `ims_hc_moreshop_commission` AS c LEFT JOIN `ims_hc_moreshop_member` AS m ON c.mid=m.id WHERE c.id in ".$ids." and c.isout = 0 AND c.flag = -1 AND c.weid=".$_W['uniacid']);
	} else {
		$outcrediting = pdo_fetchall("SELECT c.*,m.realname,m.mobile,m.bankcard,m.banktype,m.alipay,m.wxhao FROM `ims_hc_moreshop_commission` AS c LEFT JOIN `ims_hc_moreshop_member` AS m ON c.mid=m.id WHERE c.createtime>=".$starttime." AND c.createtime<=".$endtime." AND c.isout = 0 AND c.flag = -1 AND c.weid=".$_W['uniacid']);
	}
	...
	foreach($outcrediting as $k=>$v){
		pdo_update('hc_moreshop_commission', array('isout'=>1, 'flag'=>2), array('id'=>$v['id']));
		$list[$k]['realname'] = $v['realname'];
		$list[$k]['mobile'] = $v['mobile'];
		$list[$k]['commission'] = $v['commission'];
		$list[$k]['banktype'] = $v['banktype'];
	...
	foreach($list as $i=>$v){
		...
		$objPHPExcel->setActiveSheetIndex(0)			
					...
					->setCellValue('D'.$i, $v['banktype'])
					->setCellValue('E'.$i,' '.$v['bankcard'].' ')
					->setCellValue('F'.$i,' '.$v['alipay'].' ')
					->setCellValue('G'.$i,' '.$v['wxhao'].' ')
					->setCellValue('H'.$i, $v['createtime'])
					->setCellValue('I'.$i, $v['content']);
	}					
