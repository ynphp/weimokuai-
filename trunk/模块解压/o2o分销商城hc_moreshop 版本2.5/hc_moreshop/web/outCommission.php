<?php
	...
	//$sql = "SELECT c.id,c.weid,c.mid,c.ogid,c.commission,c.content,c.status,c.createtime,m.realname,m.mobile FROM `ims_hc_moreshop_commission` AS c LEFT JOIN `ims_hc_moreshop_member` AS m ON c.mid=m.id AND c.weid=".$_W['uniacid']." ";
	//$list = pdo_fetchall($sql);
	$outputdate = $_GPC['outputdate'];
	$starttime = strtotime($outputdate['start']);
	$endtime = strtotime($outputdate['end']);
	$info = pdo_fetch("select og.id, og.total, og.price, og.status, og.commission, og.applytime, og.content, g.title from ".tablename('hc_moreshop_order_goods')." as og left join ".tablename('hc_moreshop_goods')." as g on og.goodsid = g.id and og.weid = g.weid WHERE og.createtime>= ".$starttime." AND og.createtime<=".$endtime);
	//var_dump($info);
	$commissionList = pdo_fetchall("SELECT c.*,m.realname,m.mobile,m.bankcard,m.alipay,m.wxhao FROM `ims_hc_moreshop_commission` AS c LEFT JOIN `ims_hc_moreshop_member` AS m ON c.mid=m.id WHERE c.createtime>=".$starttime." AND c.createtime<=".$endtime." AND c.isout = 0 AND c.flag = 0 AND c.weid=".$_W['uniacid']);
	...
	foreach($commissionList as $k=>$v){
		$ogid = $v['ogid'];
		$info = pdo_fetch("select * from ".tablename('hc_moreshop_memberrelative')." where id = ".$ogid);
		pdo_update('hc_moreshop_commission', array('isout'=>1), array('id'=>$v['id']));
