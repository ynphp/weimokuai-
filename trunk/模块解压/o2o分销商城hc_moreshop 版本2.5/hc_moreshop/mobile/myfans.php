<?php
	...
	if($op=='display'){
		...
		for($i=1; $i<=$userdefault; $i++){
			...
			$fanslevel[$i] = pdo_fetchall("select * from ".tablename('hc_moreshop_member')." where shareid in ".$lowfansid." and status = 1");
			$lowfansid = '';
	...
	if($op=='more'){
		$level = $_GPC['level'];
		$fanslevel = pdo_fetchall("select * from ".tablename('hc_moreshop_member')." where shareid in ".$_GPC['lowfansids']." and status = 1");
	}
