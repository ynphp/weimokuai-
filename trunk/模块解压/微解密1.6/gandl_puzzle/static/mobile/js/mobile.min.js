...
;function FastClick(layer){'use strict';var oldOnClick;this.trackingClick=false;this.trackingClickStart=0;this.targetElement=null;this.touchStartX=0;this.touchStartY=0;this.lastTouchIdentifier=0;this.touchBoundary=10;this.layer=layer;if(FastClick.notNeeded(layer)){return;}
...
;;(function($,window,undefined){var win=$(window),doc=$(document),count=1,isLock=false;var Dialog=function(options){this.settings=$.extend({},Dialog.defaults,options);this.init();}
Dialog.prototype={init:function(){this.create();if(this.settings.lock){this.lock();}
if(!isNaN(this.settings.time)&&this.settings.time!=null){this.time();}},create:function(){var divHeader=(this.settings.title==null)?'<div class="rDialog-header-'+this.settings.icon+'"></div>':'<div class="rDialog-header">'+this.settings.title+'</div>';var templates='<div class="rDialog-wrap '+((this.settings.icon==null)?'':this.settings.icon)+'"><div class="rDialog-inner">'+
divHeader+'<div class="rDialog-content">'+this.settings.content+'</div>'+'<div class="rDialog-footer"></div>'+'</div></div>';this.dialog=$('<div>').addClass('rDialog').css({zIndex:this.settings.zIndex+(count++)}).html(templates).prependTo('body');if($.isFunction(this.settings.ok)){this.ok();}
if($.isFunction(this.settings.cancel)){this.cancel();}
this.size();this.position();},ok:function(){var _this=this,footer=this.dialog.find('.rDialog-footer');$('<a>',{href:'javascript:;',text:this.settings.okText}).on("click",function(){var okCallback=_this.settings.ok();if(okCallback==undefined||okCallback){_this.close();}}).addClass('rDialog-ok').prependTo(footer);},cancel:function(){var _this=this,footer=this.dialog.find('.rDialog-footer');$('<a>',{href:'javascript:;',text:this.settings.cancelText}).on("click",function(){var cancelCallback=_this.settings.cancel();if(cancelCallback==undefined||cancelCallback){_this.close();}}).addClass('rDialog-cancel').appendTo(footer);},cross:function(){var _this=this;$('<a>',{href:'javascript:;'}).on("click",function(){_this.close();}).addClass('rDialog-cross').appendTo(this.dialog.find('.rDialog-wrap'));},size:function(){var content=this.dialog.find('.rDialog-content'),wrap=this.dialog.find('.rDialog-wrap');content.css({width:this.settings.width,height:this.settings.height});},position:function(){var _this=this,winWidth=win.width(),winHeight=win.height(),scrollTop=0;this.dialog.css({left:(winWidth-_this.dialog.width())/2,top:(winHeight-_this.dialog.height())/2+scrollTop});},lock:function(){if(isLock)return;this.lock=$('<div>').css({zIndex:this.settings.zIndex}).addClass('rDialog-mask');this.lock.appendTo('body');isLock=true;},unLock:function(){if(this.settings.lock){if(isLock){this.lock.remove();isLock=false;}}},close:function(){this.dialog.remove();this.unLock();},time:function(){var _this=this;this.closeTimer=setTimeout(function(){_this.close();},this.settings.time);}}
Dialog.defaults={content:'',icon:null,title:null,width:'auto',height:'auto',ok:null,cancel:null,okText:'确定',cancelText:'取消',time:null,lock:true,zIndex:9999}
var rDialog=function(options){return new Dialog(options);}
window.rDialog=$.rDialog=$.dialog=rDialog;})(window.jQuery||window.Zepto,window);;;(function(window){var baidu=typeof module==='undefined'?(window.baidu=window.baidu||{}):module.exports;baidu.template=function(str,data){var fn=(function(){if(!window.document){return bt._compile(str);};var element=document.getElementById(str);if(element){if(bt.cache[str]){return bt.cache[str];};var html=/^(textarea|input)$/i.test(element.nodeName)?element.value:element.innerHTML;return bt._compile(html);}else{return bt._compile(str);};})();var result=bt._isObject(data)?fn(data):fn;fn=null;return result;};var bt=baidu.template;bt.versions=bt.versions||[];bt.versions.push('1.0.6');bt.cache={};bt.LEFT_DELIMITER=bt.LEFT_DELIMITER||'<%';bt.RIGHT_DELIMITER=bt.RIGHT_DELIMITER||'%>';bt.ESCAPE=true;bt._encodeHTML=function(source){return String(source).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\\/g,'&#92;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');};bt._encodeReg=function(source){return String(source).replace(/([.*+?^=!:${}()|[\]/\\])/g,'\\$1');};bt._encodeEventHTML=function(source){return String(source).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/\\\\/g,'\\').replace(/\\\//g,'\/').replace(/\\n/g,'\n').replace(/\\r/g,'\r');};bt._compile=function(str){var funBody="var _template_fun_array=[];\nvar fn=(function(__data__){\nvar _template_varName='';\nfor(name in __data__){\n_template_varName+=('var '+name+'=__data__[\"'+name+'\"];');\n};\neval(_template_varName);\n_template_fun_array.push('"+bt._analysisStr(str)+"');\n_template_varName=null;\n})(_template_object);\nfn = null;\nreturn _template_fun_array.join('');\n";return new Function("_template_object",funBody);};bt._isObject=function(source){return'function'===typeof source||!!(source&&'object'===typeof source);};bt._analysisStr=function(str){var _left_=bt.LEFT_DELIMITER;var _right_=bt.RIGHT_DELIMITER;var _left=bt._encodeReg(_left_);var _right=bt._encodeReg(_right_);str=String(str).replace(new RegExp("("+_left+"[^"+_right+"]*)//.*\n","g"),"$1").replace(new RegExp("<!--.*?-->","g"),"").replace(new RegExp(_left+"\\*.*?\\*"+_right,"g"),"").replace(new RegExp("[\\r\\t\\n]","g"),"").replace(new RegExp(_left+"(?:(?!"+_right+")[\\s\\S])*"+_right+"|((?:(?!"+_left+")[\\s\\S])+)","g"),function(item,$1){var str='';if($1){str=$1.replace(/\\/g,"&#92;").replace(/'/g,'&#39;');while(/<[^<]*?&#39;[^<]*?>/g.test(str)){str=str.replace(/(<[^<]*?)&#39;([^<]*?>)/g,'$1\r$2')};}else{str=item;}
return str;});str=str.replace(new RegExp("("+_left+"[\\s]*?var[\\s]*?.*?[\\s]*?[^;])[\\s]*?"+_right,"g"),"$1;"+_right_).replace(new RegExp("("+_left+":?[hvu]?[\\s]*?=[\\s]*?[^;|"+_right+"]*?);[\\s]*?"+_right,"g"),"$1"+_right_).split(_left_).join("\t");if(bt.ESCAPE){str=str.replace(new RegExp("\\t=(.*?)"+_right,"g"),"',typeof($1) === 'undefined'?'':baidu.template._encodeHTML($1),'");}else{str=str.replace(new RegExp("\\t=(.*?)"+_right,"g"),"',typeof($1) === 'undefined'?'':$1,'");};str=str.replace(new RegExp("\\t:h=(.*?)"+_right,"g"),"',typeof($1) === 'undefined'?'':baidu.template._encodeHTML($1),'").replace(new RegExp("\\t(?::=|-)(.*?)"+_right,"g"),"',typeof($1)==='undefined'?'':$1,'").replace(new RegExp("\\t:u=(.*?)"+_right,"g"),"',typeof($1)==='undefined'?'':encodeURIComponent($1),'").replace(new RegExp("\\t:v=(.*?)"+_right,"g"),"',typeof($1)==='undefined'?'':baidu.template._encodeEventHTML($1),'").split("\t").join("');").split(_right_).join("_template_fun_array.push('").split("\r").join("\\'");return str;};})(window);;$(function(){FastClick.attach(document.body);App.controller('index_page',function(page){App.controller('detail_page',function(page){this.transition='scale-in';});App.controller('clue_find_page',function(page){this.transition='scale-in';var list=$(page).find('#cf_list');var cf_list_load=$(page).find('#cf_list_load');var cf_list_tpl=baidu.template($(page).find('#cf_list_tpl').html());var loadList=function(){if(0==list.data('more')){return;}
cf_list_load.removeClass('loading more');cf_list_load.addClass('loading');cf_list_load.find('.text').html('正在加载...');cf_list_load.show();$.get(list.data('url'),{start:list.data('start')}).done(function(resp){cf_list_load.hide();if(!resp){alert('加载失败，请检查网络后重试');return;}
if(resp.status!=1){alert('加载失败，请检查网络后重试');return;}
var data=resp.data;list.data('start',data.start);list.data('more',data.more);var html=cf_list_tpl(data);list.html(list.html()+html);});};cf_list_load.on('tap',function(){loadList();});loadList();});$(page).find('#puzzle_banner').on('tap',function(){App.load('detail_page');});$(page).find('#to_exchange_btn').on('tap',function(){App.load('clue_find_page');});if($('#is_puzzle_end').val()==0){$(page).find('#clue_exchange_btn').on('tap',function(){var loading=$.dialog({icon:'load',content:'正在互换线索...'});$.post($(this).data('url'),{},function(data){loading.close();if(data.status==0){$.dialog({content:data.info,title:'失败',okText:'确定',ok:function(){return true;},lock:true});}else{$.dialog({content:'即将为您显示对方线索',title:'互换成功',okText:'确定',ok:function(){location.href=location.href;},lock:true});}});});$(page).find('#puzzle_answer_btn').on('tap',function(){if($(this).hasClass('disabled')){$.dialog({content:'至少获得'+$(this).data('keys_least')+'条线索才能解答',title:'线索不足',okText:'确定',ok:function(){return true;},lock:true});}else{var answer=$(page).find('#puzzle_answer_text').val();if(''==answer){return $.dialog({content:'您还没有填写解答内容',title:'提示',okText:'确定',ok:function(){return true;},lock:true});}
var loading=$.dialog({icon:'load',content:'正在提交解答...'});$.post($(this).data('url'),{answer:answer},function(data){loading.close();if(data.status==0){$.dialog({content:data.info,title:'提交失败',okText:'确定',ok:function(){return true;},lock:true});}else{$.dialog({content:'请耐心等待答案揭晓',title:'提交成功',okText:'确定',ok:function(){location.href=location.href;},lock:true});}});}});$(page).find('#puzzle_answered').on('tap',function(){$.dialog({content:'如果重新解答，原解答内容将被覆盖，且解答时间以最后一次提交为准',title:'确定要重新解答吗？',okText:'确定重答',ok:function(){$(page).find('#puzzle_answered').hide();$(page).find('#puzzle_hint').show();$(page).find('#puzzle_answer').show();},cancelText:'取消',cancel:function(){return true;},lock:true});});$(page).find('#puzzle_answer_select').on('tap',function(){if($(this).hasClass('disabled')){$.dialog({content:'至少获得'+$(this).data('keys_least')+'条线索才能解答',title:'线索不足',okText:'确定',ok:function(){return true;},lock:true});}else{if($(this).hasClass('droped')){$(this).removeClass('droped');$(page).find('#puzzle_answer_options').slideUp();}else{$(this).addClass('droped');$(page).find('#puzzle_answer_options').slideDown();}}});$(page).find('#puzzle_answer_options li').on('tap',function(){$(this).addClass('selected');var answer=$(this).data('value');var loading=$.dialog({icon:'load',content:'正在提交解答...'});$.post($(page).find('#puzzle_answer_select').data('url'),{answer:answer},function(data){loading.close();if(data.status==0){$.dialog({content:data.info,title:'提交失败',okText:'确定',ok:function(){return true;},lock:true});}else{$.dialog({content:'请耐心等待答案揭晓',title:'提交成功',okText:'确定',ok:function(){location.href=location.href;},lock:true});}});});var puzzle_timer=setInterval(function(){var limit=$(page).find('#puzzle_timer').data('end_time')-new Date().getTime()/1000;if(limit<=0){clearInterval(puzzle_timer);location.href=location.href;}else{var text='';if(limit>3600){text+=parseInt(limit/3600)+'小时';limit=limit%3600;}
if(limit>60){text+=parseInt(limit/60)+'分钟';limit=limit%60;}
if(limit>0){text+=parseInt(limit)+'秒';}
$(page).find('#puzzle_timer_disp').text(text+'后揭晓！');}},1000);}
if($('#is_puzzle_end').val()==1){App.controller('truth_page',function(page){this.transition='scale-in';});$(page).find('#puzzle_truth').on('tap',function(){App.load('truth_page');});App.controller('award_page',function(page){this.transition='scale-in';});$(page).find('#puzzle_to_award').on('tap',function(){App.load('award_page');});var list=$(page).find('#rank_list');var rank_list_load=$(page).find('#rank_list_load');var rank_list_tpl=baidu.template($(page).find('#rank_list_tpl').html());var loadRankList=function(){if(0==list.data('more')){return;}
rank_list_load.removeClass();rank_list_load.addClass('loading');rank_list_load.find('.text').html('正在加载...');rank_list_load.show();$.get(list.data('url'),{start:list.data('start')}).done(function(resp){rank_list_load.hide();if(!resp){Toast.error('加载失败，请检查网络后重试');return;}
if(resp.status!=1){Toast.error(resp.info);return;}
var data=resp.data;data.index=list.data('start');list.data('start',data.start);list.data('more',data.more);var html=rank_list_tpl(data);list.html(list.html()+html);});};$(page).find('#rank_list_load').on('tap',function(){loadRankList();});loadRankList();}});App.load('index_page');});