...
class hl_tugModuleSite extends WeModuleSite {
    ...
    public function doWebAjax() {
        ...
        if ($op == 'over_tug') {
            ...
            $a = pdo_fetch("select sum(`count`) FROM " . tablename('hl_tug_winner') . " WHERE rid = '$id' AND whoteam=1 ");
            $b = pdo_fetch("select sum(`count`) FROM " . tablename('hl_tug_winner') . " WHERE rid = '$id' AND whoteam=2 ");
