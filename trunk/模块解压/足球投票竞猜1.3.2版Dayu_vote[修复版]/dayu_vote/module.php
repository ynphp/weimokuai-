...
class dayu_voteModule extends WeModule {
    ...
    public function fieldsFormValidate($rid = 0) {
        ...
        if ($reid) {
            $sql = 'SELECT * FROM ' . tablename('dayu_vote') . " WHERE `reid`=:reid";
            $params = array();
            $params[':reid'] = $reid;
    ...
    public function fieldsFormSubmit($rid) {
        ...
        $reply = pdo_fetch("SELECT * FROM " . tablename('dayu_vote_reply') . " WHERE rid = :rid", array(':rid' => $rid));
        if ($reply) {
            pdo_update('dayu_vote_reply', $record, array('id' => $reply['id']));
        } else {
            pdo_insert('dayu_vote_reply', $record);
        }
    }

    public function ruleDeleted($rid) {
        pdo_delete('dayu_vote_reply', array('rid' => $rid));
    }
