...
define('RES', '../addons/businesscenter/template');
include "../addons/businesscenter/model.php";

class businesscenterModuleSite extends WeModuleSite
{
    //模块标识
    ...
    public function doMobileIndex()
    {
        ...
        $share_url = empty($setting['share_url']) ? $_W['siteroot'] . $this->createMobileUrl('index') : $setting['share_url'];
    ...
    public function doMobileSettled()
    {
        ...
        $title = "微商圈";
        $modulename = $this->modulename;
        $fromuser = !empty($_W['fans']['from_user']) ? $_W['fans']['from_user'] : authcode(base64_decode($_GPC['from_user']), 'DECODE');
    ...
    public function doMobileFeedback()
    {
        ...
        $rowcount = pdo_insert('businesscenter_feedback', $data);
        if ($rowcount > 0) {
    ...
    function code_compare($a, $b)
    {
        ...
        if ($a != $b) {
            message("对不起，您使用的系统是由非法渠道传播的，请支持正版。");
        }
    }
    ...
    public function insert_default_category($name, $logo, $parent_name = '', $isfirst = 0)
    {
        global $_GPC, $_W;
        checklogin();
        $path = '../addons/businesscenter/template/themes/images/';
    ...
    public function doWebStores()
    {
        ...
        if ($operation == 'post') {
            ...
            if (!empty($row['logo'])) {
               load()->func('file');file_delete($row['logo']);
            }
