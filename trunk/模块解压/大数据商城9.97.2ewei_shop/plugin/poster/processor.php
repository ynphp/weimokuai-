...
class PosterProcessor extends PluginProcessor
{
    ...
    private function responseText($zym_var_25)
    {
        ...
        $zym_var_19 = $_W["siteroot"] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&m=ewei_shop&do=plugin&p=poster&method=build&timestamp=" . time();
    ...
    private function responseScan($zym_var_25)
    {
        ...
        $zym_var_27 = pdo_fetch("select * from " . tablename("ewei_shop_poster") . " where type=4 and isdefault=1 and uniacid=:uniacid limit 1", array(
            ":uniacid" => $_W['uniacid']
        ));
        if (empty($zym_var_27)) {
            return $this->responseDefault($zym_var_25);
        }
        $this->model->scanTime($zym_var_26, $zym_var_30["openid"], $zym_var_27);
    ...
    private function responseSubscribe($zym_var_25)
    {
        ...
        if ($zym_var_17["isnew"]) {
            pdo_update("ewei_shop_poster", array(
                "follows" => $zym_var_27["follows"] + 1
            ), array(
                "id" => $zym_var_27["id"]
            ));
        }
        $zym_var_28 = m("member")->getMember($zym_var_30["openid"]);
        $zym_var_6  = pdo_fetch("select * from " . tablename("ewei_shop_poster_log") . " where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1", array(
            ":openid" => $zym_var_26,
        ...
        if (empty($zym_var_6) && $zym_var_26 != $zym_var_30["openid"]) {
            ...
            pdo_insert("ewei_shop_poster_log", $zym_var_6);
...
谢谢您对我们的支持！",
            ...
            if (!empty($zym_var_14)) {
                pdo_update("ewei_shop_poster_log", $zym_var_14, array(
                    "id" => $zym_var_6["id"]
                ));
            }
        }
