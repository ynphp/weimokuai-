...
//            }
        ...
        $fanslist = pdo_fetchall("SELECT * FROM " . tablename($this->table_fans) . " WHERE rid=:rid and credit <> 0 ORDER BY credit ASC LIMIT  10000", array(':rid' => $id));
        $rank = array();
        $myrank = 0;   // 我的排名
        $mycredit = 0; // 最佳得分
    ...
    public function doMobileMain()
    {
        ...
        $game_times = intval($reply['number_times']);//总游戏次数
        $day_times = intval($reply['most_num_times']);//每天游戏次数

        $fans = pdo_fetch("SELECT * FROM " . tablename($this->table_fans) . " WHERE from_user=:from_user AND rid=:rid LIMIT 1", array(':from_user' => $from_user, ':rid' => $id));
        $totalnum = intval($fans['totalnum']);//用户总游戏次数
        $todaynum = intval($fans['todaynum']);//用户今日游戏次数
        $sharelotterynum = intval($fans['sharelotterynum']);//分享可游戏次数
        ...
        if ($day_times != 0 && $todaynum <= 0) {
            message('您的游戏次数用完啦!', $this->createMobileUrl('index', array('id' => $id), true), 'error');
        }
    ...
    public function doMobileReward()
    {
        ...
        $awardlist = pdo_fetchall("SELECT * FROM " . tablename($this->table_award) . " WHERE rid=:rid ORDER BY id ASC LIMIT  10000", array(':rid' => $id));
        $award = array();
        foreach($awardlist as $key => $value) {
            if ($key < 10) {
                $award[$key] = array('img' => tomedia($value['prizepic']), 'type' => $value['prizetype'], 'name' => $value['prizename'], 'num' => $value['prizetotal']);
            }
        }
...
//        }
        ...
        if ($gamecredit != 0) { //原来有记录
            if ($num == $reply['gamelevel']) { //成功
                if ($score < $gamecredit) {
                    $gamecredit = $score;
                }
            }
    ...
    public function doMobileGetallrank()
    {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_fans) . " WHERE rid = :rid AND credit<>0 ORDER BY credit asc " . $limit, array(':rid' => $id));

        foreach($list as $key => $value) {
            $rank[$key] = array('img' => tomedia($value['headimgurl']), 'name' => $value['nickname'], 'score' => number_format($value['credit'],2));
        }
    ...
    public function doMobileSaveUserinfo()
    {
        ...
        pdo_update($this->table_fans, $datafans, array('id' => $fans['id']));
//        $url = $this->createMobileUrl('index', array('id' => $id), true);
//        header("location:$oauth2_code");
        message('注册成功！', $this->createMobileUrl('index', array('id' => $id), true), 'success');
    }
    ...
    public function doWebManage() {
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                $weisrc_magpiebridge = pdo_fetch("SELECT * FROM " . tablename($this->table_reply) . " WHERE rid = :rid ", array(':rid' => $item['id']));
    ...
    public function doWebfanslist() {
        ...
        if ($operation == 'display') {
            ...
            if ($_GPC['out_put'] == 'output') {
                $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_fans) . " WHERE rid = :rid AND credit<>0 ORDER BY {$condition} asc,id desc ", array(':rid' => $rid));
            ...
            $limit = "";
            $limit .= " LIMIT {$start},{$psize}";
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_fans) . " WHERE rid = :rid AND credit<>0 ORDER BY {$condition} asc,id DESC " . $limit, array(':rid' => $rid));

            $total = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_fans) . " WHERE rid = :rid  ", array(':rid' => $rid));
            $gametotal = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_fans) . " WHERE rid = :rid AND credit<>0  ", array(':rid' => $rid));
            $pager = pagination($total, $pindex, $psize);
    ...
    public function doWebsetshow() {
        ...
        $temp = pdo_update('weisrc_magpiebridge_reply', array('status' => $isstatus), array('rid' => $rid));
    ...
    public function doWebsetstatus() {
        ...
        $temp = pdo_update('weisrc_magpiebridge_sncode', $data, array('id' => $id, 'weid' => $_W['uniacid']));
        if ($temp == false) {
