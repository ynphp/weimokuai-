...
$reply = pdo_fetch("select isrealname,ismobile,isqq,isemail,isaddress,isgender,istelephone,isidcard,iscompany,isoccupation,isposition,isfansname from " . tablename('kang_egg_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
...
if($fans=='branch'){
	...
	$list = pdo_fetchall("select * from ".tablename('stonefish_branch_doings')."  where rid = :rid and uniacid=:uniacid and module=:module ".$where." order by id desc" , array(':rid' => $rid,':uniacid'=>$_W['uniacid'],':module'=>'kang_egg'));
    //查询区域以及商家 以及中奖情况
	foreach ($list as &$row) {
		...
		$fansID = pdo_fetchcolumn("select fansID from " . tablename('kang_egg_fans') . "  where mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("select name from " . tablename('kang_egg_award') . " where rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $fansID));
		if(!empty($awards)){
	...
	$list = pdo_fetchall("select * from ".tablename('kang_egg_fans')."  where rid = :rid and uniacid=:uniacid ".$where." order by id desc" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	//中奖情况
	foreach ($list as &$lists) {
		$lists['awardinfo']='';
		$awards = pdo_fetchall("select name from " . tablename('kang_egg_award') . " where rid = :rid and fansID=:fansID", array(':rid' => $rid,':fansID' => $lists['fansID']));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
    ...
    $list = pdo_fetchall("select a.*,b.realname,b.mobile,b.qq,b.email,b.address,b.gender,b.telephone,b.idcard,b.company,b.occupation,b.position from ".tablename('kang_egg_award')." as a  left join ".tablename('kang_egg_fans')."
