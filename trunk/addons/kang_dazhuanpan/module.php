...
class kang_dazhuanpanModule extends WeModule {
	...
	public function fieldsFormDisplay($rid = 0) {
		...
		//查询子公众号信息
		//活动模板
		$template = pdo_fetchall("SELECT * FROM " . tablename('kang_dazhuanpan_template') . " WHERE uniacid = :uniacid ORDER BY `id` asc", array(':uniacid' => $uniacid));
		if(empty($template)){			
		...
		//活动模板
		//消息模板
		$tmplmsg = pdo_fetchall("SELECT * FROM " . tablename('kang_dazhuanpan_tmplmsg') . " WHERE uniacid = :uniacid ORDER BY `id` asc", array(':uniacid' => $uniacid));
		//消息模板
		if (!empty($rid)) {
			$reply = pdo_fetch("SELECT * FROM ".tablename('kang_dazhuanpan_reply')." WHERE rid = :rid ORDER BY `id` desc", array(':rid' => $rid));
			$exchange = pdo_fetch("SELECT * FROM ".tablename('kang_dazhuanpan_exchange')." WHERE rid = :rid ORDER BY `id` desc", array(':rid' => $rid));
			$share = pdo_fetchall("select * from " . tablename('kang_dazhuanpan_share') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
			$prize = pdo_fetchall("select * from " . tablename('kang_dazhuanpan_prize') . " where rid = :rid order by `id` asc", array(':rid' => $rid));
			if(!empty($reply)){
		...
		if (empty($share)) {
			...
			foreach ($ids as $acid=>$idlists) {
                $share[$acid] = array(
					...
					"share_anniu" => "分享我的快乐",
					"share_firend" => "我的亲友团",
					"share_img" => "../addons/kang_dazhuanpan/template/images/img_share.png",
					"share_pic" => "../addons/kang_dazhuanpan/template/images/share.png",
		...
		if (!$reply) {
            $reply = array(
                "title" => "砸金蛋H5活动开始了!",
                "start_picurl" => "../addons/kang_dazhuanpan/template/images/activity-lottery-start.jpg",
                "description" => "欢迎参加砸金蛋H5活动",
                "repeat_lottery_reply" => "亲，继续努力哦~~",
                "ticket_information" => "兑奖请联系我们,电话: 13888888888",
                "end_title" => "砸金蛋H5活动已经结束了",
                "end_description" => "亲，活动已经结束，请继续关注我们的后续活动哦~",
                "end_picurl" => "../addons/kang_dazhuanpan/template/images/activity-lottery-end.jpg",
            );
        }
		...
		$reply['musicurl'] = empty($reply['musicurl']) ? "../addons/kang_dazhuanpan/template/audio/bg.mp3" : $reply['musicurl'];
		...
		$reply['bigwheelpic'] = empty($reply['bigwheelpic']) ? "../addons/kang_dazhuanpan/template/images/activity-lottery-6.png" : $reply['bigwheelpic'];
		$reply['bigwheelimg'] = empty($reply['bigwheelimg']) ? "../addons/kang_dazhuanpan/template/images/activity-inner.png" : $reply['bigwheelimg'];
		$reply['bigwheelimgan'] = empty($reply['bigwheelimgan']) ? "../addons/kang_dazhuanpan/template/images/activity.png" : $reply['bigwheelimgan'];
		$reply['bigwheelimgbg'] = empty($reply['bigwheelimgbg']) ? "../addons/kang_dazhuanpan/template/images/activity_bg.png" : $reply['bigwheelimgbg'];
	...
	public function fieldsFormSubmit($rid) {
		...
		$insertexchange = array(
		    ...
		    if(empty($id)){
			    pdo_insert("kang_dazhuanpan_reply", $insert);
				$id = pdo_insertid();
		    }else{
			    pdo_update("kang_dazhuanpan_reply", $insert, array('id' => $id));
		    }
			if(empty($exchangeid)){
			    pdo_insert("kang_dazhuanpan_exchange", $insertexchange);
		    }else{
			    pdo_update("kang_dazhuanpan_exchange", $insertexchange, array('id' => $exchangeid));
		    }
		...
		foreach ($ids as $acid=>$idlists) {
		    $insertshare = array(
				...
				if (empty($_GPC['acid_'.$acid])) {
                    pdo_insert('kang_dazhuanpan_share', $insertshare);
                } else {
                    pdo_update('kang_dazhuanpan_share', $insertshare, array('id' => $_GPC['acid_'.$acid]));
                }
		...
		for($i = 0; $i <= 11; $i++) {
			...
			if($_GPC['turntable']){
				...
				if (empty($_GPC['prize_id_'.$i])) {
                    pdo_insert('kang_dazhuanpan_prize', $insertprize);
                } else {
                    pdo_update('kang_dazhuanpan_prize', $insertprize, array('id' => $_GPC['prize_id_'.$i]));
                }
			
        }		
		if($updata['prize_num']){
			pdo_update('kang_dazhuanpan_reply', $updata, array('id' => $id));
		}
		//奖品配置
	...
	public function ruleDeleted($rid) {
		...
		global $_W;
		pdo_delete('kang_dazhuanpan_reply', array('rid' => $rid));
        pdo_delete('kang_dazhuanpan_exchange', array('rid' => $rid));
		pdo_delete('kang_dazhuanpan_share', array('rid' => $rid));
        pdo_delete('kang_dazhuanpan_prize', array('rid' => $rid));
		pdo_delete('kang_dazhuanpan_prizemika', array('rid' => $rid));
		pdo_delete('kang_dazhuanpan_fans', array('rid' => $rid));
		pdo_delete('kang_dazhuanpan_fansaward', array('rid' => $rid));
		pdo_delete('kang_dazhuanpan_sharedata', array('rid' => $rid));
		return true;
	}
	...
	public function settingsDisplay($settings) {
		...
		$settings['kang_dazhuanpan_num'] = !isset($settings['kang_dazhuanpan_num']) ? "1" : $settings['kang_dazhuanpan_num'];
		if(checksubmit()) {
			//字段验证, 并获得正确的数据$dat
			if($_GPC['kang_dazhuanpan_oauth']==2){
				if(empty($_GPC['appid'])||empty($_GPC['secret'])){
					message('请填写借用AppId或借用AppSecret', referer(), 'error');
				}
			}
			if($_GPC['kang_dazhuanpan_jssdk']==2){
			...
			$dat = array(
				...
				'kang_dazhuanpan_num'  => $_GPC['kang_dazhuanpan_num'],
				'kang_dazhuanpan_oauth'  => $_GPC['kang_dazhuanpan_oauth'],
				'kang_dazhuanpan_jssdk'  => $_GPC['kang_dazhuanpan_jssdk']
            );
