...
if($data=='branch'){
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_branch_doings')."  WHERE rid = :rid and uniacid=:uniacid and module=:module ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid'],':module'=>'kang_dazhuanpan'));
    //查询区域以及商家 以及中奖情况
	foreach ($list as &$row) {
		...
		$fid = pdo_fetchcolumn("SELECT from_user FROM " . tablename('kang_dazhuanpan_fans') . "  WHERE mobile = :mobile and rid=:rid and uniacid=:uniacid", array(':mobile' => $row['mobile'],':rid' => $rid,':uniacid' => $_W['uniacid']));
		$row['awardinfo']='';
		$awards = pdo_fetchall("SELECT prizeid FROM " . tablename('kang_dazhuanpan_fansaward') . " WHERE rid = :rid and from_user=:from_user", array(':rid' => $rid,':from_user' => $fid));
		if(!empty($awards)){
			foreach ($awards as &$awardid) {
				$row['prizename'] = pdo_fetchcolumn("SELECT prizename FROM " . tablename('kang_dazhuanpan_prize') . "  WHERE id = :id", array(':id' => $awardid['prizeid']));
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('kang_dazhuanpan_fans')."  WHERE rid = :rid and uniacid=:uniacid ".$where." ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	//变换变量
	...
	$statustitle.='排名';
	$list = pdo_fetchall("SELECT * FROM ".tablename('kang_dazhuanpan_fans')."  WHERE rid = :rid and uniacid=:uniacid ORDER BY ".$ORDER." DESC,id asc" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
	//变换变量
	foreach ($list as &$lists) {
		$lists['status']='';
		if($lists['zhongjiang']==0){
