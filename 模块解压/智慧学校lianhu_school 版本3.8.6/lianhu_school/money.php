...
class money{
	...
	public function __construct($do_action,$table_pe){
		...
		if($student_id)	$this->student_id=$student_id;
	}
	public function uid_to_studentId(){
		$uid=$this->uid;
		$fanid=pdo_fetchcolumn("select fanid from ".tablename('mc_mapping_fans')." where uid={$this->uid} ");
		$this->fanid=$fanid;
		$student_id=pdo_fetchcolumn("select student_id from ".$this->table_pe."lianhu_student where uniacid=:uniacid and school_id=:sid and (fanid=:fanid or fanid1=:fanid1 or fanid2=:fanid2) ",array(':uniacid'=>$this->uniacid,':sid'=>$_SESSION['school_id'],':fanid'=>$fanid,':fanid1'=>$fanid1,':fanid2'=>$fanid2));
		return $student_id;
	}
	#获取相应缴费规则
	public function get_money_limit(){
		$result=pdo_fetch("select * from ".$this->table_pe."lianhu_money_limit where uniacid=:uniacid and school_id=:sid and limit_module=:limit_module and  status=1 order by addtime desc  ",array(':uniacid'=>$this->uniacid,':sid'=>$_SESSION['school_id'],':limit_module'=>$this->do_action));
		$this->money_limit=$result;
	}
	#获取最后一次缴费历史
	public function last_money_record(){
		$limit_id=$this->money_limit['limit_id'];
		$result=pdo_fetch("select * from ".$this->table_pe."lianhu_money_record where limit_id=:lid and status=1 and school_id=:sid and uid=:uid",array(':lid'=>$limit_id,':sid'=>$_SESSION['school_id'],':uid'=>$this->uid));
	...
	public function money_judge(){
		...
		if(empty($this->money_limit))	return true;
		$this->last_money_record();
		if($this->money_limit['limit_type']==1 && $this->money_record_last) return true;
		if($this->money_limit['limit_type']==2){
			$next_need_time=$this->money_record_last['addtime']+3600*24*365;
			if($next_need_time >= TIMESTAMP) return true;
		}
