...
defined('IN_IA') or exit('Access Denied');
    ...
    if($_GPC['hid']){
		...
		$result=pdo_fetch("select ta.*,tea.teacher_realname,tea.teacher_id from {$table_pe}lianhu_work  ta left join {$table_pe}lianhu_teacher tea on tea.fanid=ta.teacher_id where work_id=:wid  order by addtime desc",array(':wid'=>$_GPC['wid']));   
        $record=1;
    }else{
