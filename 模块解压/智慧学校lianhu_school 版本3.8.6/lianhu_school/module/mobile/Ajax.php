...
if ($op == 'line_like') {
    ...
    if (!$this->zanLine($send_id)) {
        ...
        $send_re  = pdo_fetch("select * from {$table_pe}lianhu_send where send_id=:id", array(
            ":id" => $send_id
        ));
        $like_num = $send_re['send_like'] + 1;
        pdo_update('lianhu_send', array(
            'send_like' => $like_num
        ), array(
            'send_id' => $send_id
        ));
...
if ($op == 'line_change') {
    ...
    if ($ac == 'line_pass') {
        ...
        $up['send_status'] = 1;
        pdo_update("lianhu_send", $up, array(
            'send_id' => $send_id
        ));
        $out = array(
            'errcode' => 0,
    ...
    if ($ac == 'delete') {
        ...
        $result           = pdo_update("lianhu_send", array(
            'send_status' => 3
        ), $where);
        if (!$result)
            echo json_encode(array(
            ...
            <LI class="box" >
                    ...
                    <div class="click_hf">
                        ...
                        <span id="like_num_' . $value['send_id'] . '"  class="like_name">' . $this->getLineZanName($value['send_id']) . '</span>
                         <div  class="comment huifu" data-id="' . $value['send_id'] . '"><i class="fa fa-comment-o we1101" ></i>
                        </div>
            ...
            <li class="box" >
            		<div class="author">
                            ...
                            <div class="topic" style="margin-top:-10px;">
                                    <a href="' . $this->createMobileUrl('line_article', array(
                'hid' => $row['homework_id']
            )) . '">
...
if ($op == 'line' && $_GPC['home_work'] != 'home_work') {
    $student_info = $this->mobile_from_find_student();
    $typeid       = $_GPC['type_id'];
    $news_list    = pdo_fetchall("select line.*,tea.teacher_realname from {$table_pe}lianhu_line line left join {$table_pe}lianhu_teacher tea on tea.teacher_id=line.teacher_id  where line.class_id=:cid and line.status=1 and  line_type={$typeid} order by line.addtime desc {$sql_limit}", array(
    ...
    if ($count > 0) {
        $arr['list'] = 'yes';
        foreach ($news_list as $key => $value) {
            $arr['list_con'][$key]['url']              = $this->createMobileUrl('line_article', array(
                'op' => $value['line_id']
            ));
...
if ($op == 'appointment') {
    ...
    $app_result   = pdo_fetch("select * from {$table_pe}lianhu_appointment where ($where) and  appointment_id=:id", array(
        ':id' => $_GPC['appointment_id']
    ));
    if (!$app_result) {
        echo json_encode(array(
            'msg' => '无此预约'
    ...
    if (empty($app_result['appointment_mutex'])) {
        ...
        if ($join_num >= $app_result['appointment_max_num']) {
            echo json_encode(array(
                'msg' => '已经满员啦'
            ));
            exit();
        ...
        $join_num = pdo_fetchcolumn("select count(*) num  from {$table_pe}lianhu_applist where appointment_id={$app_result['appointment_id']} and status !=2 and content like :con", array(
            ':con' => "%{$_GPC['active']}%"
        ));
        foreach ($active_arr as $key => $value) {
            ...
            if ($active_list[$key]['name'] == $_GPC['active']) {
                if ($join_num >= $active_list[$key]['max']) {
                    echo json_encode(array(
                        'msg' => '此项目已经满员啦！'
                    ));
                    exit();
    ...
    pdo_update('lianhu_appointment', array(
        'appointment_join_num' => $app_result['appointment_join_num'] + 1
    ), array(
        'appointment_id' => $app_result['appointment_id']
    ));
    pdo_insert('lianhu_applist', array(
        'appointment_id' => $app_result['appointment_id'],
...
if ($ac == 'student_score_list') {
    ...
    $list         = pdo_fetchall("select * from {$table_pe}lianhu_scorelist where course_id=:course_id and ji_lv_id=:ji_lv_id and class_id=:class_id", array(
        ':course_id' => $course_id,
        ':ji_lv_id' => $scorejilv_id,
        ':class_id' => $class_id
    ));
    echo json_encode(array(
        'status' => "yes",
