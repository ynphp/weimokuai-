...
class Mon_QmShakeModuleSite extends WeModuleSite
{
	...
	/**
	 * author: codeMonkey QQ:2463619823兑换
	 *
	 */
	...
	public function  doWebRecord_list()
	{
		...
		if ($operation == 'display') {
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT r.*,r.id as rid,u.uname,u.tel,u.nickname,u.headimgurl  FROM " . tablename(DBUtil::$TABLE_QMSHAKE_RECORD) . " r left join " . tablename(DBUtil::$TABLE_QMSHAKE_USER) . " u  on r.uid=u.id  WHERE r.sid =:sid " . $where . " ORDER BY r.createtime DESC, r.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
...
//
	...
	public function doMobileBindUser()
	{
		...
		DBUtil::updateById(DBUtil::$TABLE_QMSHAKE_USER,array('tel'=>$tel,'uname' => $uname),$uid);
		$res['code'] = 200;
		die(json_encode($res));
	}
	/***************************函数********************************/
