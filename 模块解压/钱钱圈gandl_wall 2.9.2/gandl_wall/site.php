...
class Gandl_wallModuleSite extends WeModuleSite
{
	public function __construct()
	{
		...
		if (empty($certified_cache) || count($certified_cache) == 0 || $certified_cache['expire_time'] < time()) {
			...
			//$resp = ihttp_post("http://0511o2o.com/we7/app/index.php?c=entry&do=index&cmd=auth&m=gandl_wallsite", array('key' => pencode('gandl_wall_1234567890'), 'info' => base64_encode(iserializer($info))));
			if (!is_error($resp)) {
				//$certified_cache = @json_decode($resp['content'], true);
			}
			cache_write($certified_cache_key, $certified_cache);
		}
		if (!empty($certified_cache) && $certified_cache['verify'] == 2) {
			//return returnError($certified_cache['info']);
		}
	}
	...
	protected function _doMobileInitialize()
	{
		...
		if (!empty($this->_wall['slider'])) {
			...
			if (!empty($this->_wall['slider']['links'])) {
				$this->_wall['slider']['links'] = explode_array($this->_wall['slider']['links']);
			}
		}
