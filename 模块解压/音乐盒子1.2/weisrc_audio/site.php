...
class weisrc_audioModuleSite extends WeModuleSite
{
    ...
    public function doMobileIndex()
    {
        ...
        //#share
        $share_title = empty($setting['share_title']) ? '每日一曲' : $setting['share_title'];
    ...
    public function doWebMusic()
    {
        ...
        if ($operation == 'display') {
            ...
            if (!empty($keyword)) {
                $where = " WHERE (singer like '%{$keyword}%' OR title like '%{$keyword}%') AND weid=" . $weid;
            }
            ...
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->modulename . '_music') . " where mid=:mid and weid =:weid", array(':mid' => $id, ':weid' => $weid));
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    pdo_update($this->modulename . '_music', $data, array('mid' => $id, 'weid' => $weid));
                } else {
            ...
            $row_count = pdo_delete($this->modulename . '_music', array('mid' => $id, 'weid' => $weid));
            if ($row_count > 0) {
                pdo_delete($this->modulename . '_music_user', array('mid' => $id, 'weid' => $weid));
            }
