...
class Lonaking_super_questionModuleSite extends WeModuleSite {
	...
	public function doWebSetCurrentActivity(){
		...
		$id = $_GPC['id'];
		try{
			$this->activityService->checkCurrentAcitivity($id);
	...
	public function doWebQuestionManage(){
		...
		$page_index = max(1, intval($_GPC['page']));
		$page_size = (is_null($_GPC['size']) || $_GPC['size'] <= 0 )? 10 : $_GPC['size'];
		$page = $this->questionService->selectPage("ORDER BY create_time ASC");
	...
	public function doWebActivityQuestionManage(){
		...
		//所有的question
		$question_page = $this->questionService->selectPage("ORDER BY create_time ASC");
		//取出所有已经选择的记录
	...
	public function doWebRecordManage(){
		...
		$tmpData = pdo_fetchall("select r.id,r.uniacid,r.record_number,r.type,r.player_id,r.openid,r.uid,r.is_captain,r.right,r.wrong,r.right_ids,r.wrong_ids,r.score,r.activity_id,r.answer_seconds,r.shared,r.is_help,r.help_record_id,r.create_time,r.update_time,a.name activity_name,p.headimgurl,p.nickname FROM ".tablename($this->recordService->table_name) ." r LEFT JOIN ".tablename($this->activityService->table_name)." a ON r.activity_id=a.id LEFT JOIN ".tablename($this->playerService->table_name)." p ON r.player_id=p.id WHERE r.uniacid={$_W['uniacid']} ORDER BY {$orderBy}r.id DESC LIMIT {$where} ");
	...
	public function doMobileIndex() {
		...
		$question_id = $_GPC['question_id'];
		$activity = $this->activityService->selectOne("AND current=1");
		//fetch all questions
		if(empty($activity)){
			return message("当前没有活动",'','error');
		}
		$activity_questions = $this->activityQuestionService->selectAll("AND activity_id={$activity['id']}");
		$question_ids = array();
		foreach($activity_questions as $aq){
			if(!empty($aq['question_id'])){
		...
		$share = array(
			'share_title' => "",
			'share_logo' => $_W['attachurl'].$activity['share_logo'],
	...
	private function prepareShareContentWithRecord($record,$player,$activity){
		...
		$result = array(
			...
			'score' => $record['score'],
			'tip' => " 答错".$record['wrong']."道题,转发朋友圈求助圈里好友,让他/她帮你答题,答对全部可获".$record['score']."积分哦",
		);
		return $result;
	}
	...
	public function doMobileRecordNote(){
		...
		try{
			...
			//when this record is bigger than old one ,update it !!!fuck fuck fuck
			if($maxRecord['score'] <= $record['score']){
				$record['max'] = 1;
				$this->recordService->updateColumn("max",0,$maxRecord['id']);
			}
			//取出之前成绩最高的record 比较 so ~ write like these code is not strong!!! should edit them
