<?php
	...
	if($op==0){
			...
			$sql = 'SELECT * FROM '.tablename('tg_order').' WHERE openid = :openid  and uniacid = :uniacid'; //从订单信息表里面取得数据的sql语句
		...
		$sql = 'SELECT * FROM '.tablename('tg_order').' WHERE openid = :openid  and uniacid = :uniacid  and status = :status '; //从订单信息表里面取得数据的sql语句
