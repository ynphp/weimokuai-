...
if($data=='mobileverify'){
    $statustitle='手机验证名单';	
	$list = pdo_fetchall("SELECT * FROM ".tablename('stonefish_chailihe_mobileverify')."  WHERE rid = :rid and uniacid=:uniacid ORDER BY id DESC" , array(':rid' => $rid,':uniacid'=>$_W['uniacid']));
    if($reply['yanzheng']==3){
