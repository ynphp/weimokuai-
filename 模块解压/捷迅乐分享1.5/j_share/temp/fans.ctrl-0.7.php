...
if($do == 'initsync') {
	...
	if(!empty($ds)) {
		foreach($ds as $row) {
			if(!empty($row['tag']) && $row['unionid']) {
				continue;
			}
			$fan = $acc->fansQueryInfo($row['openid'], true);
			if(!empty($row['tag']) && !$fan['unionid']){
				continue;
			}elseif(!empty($row['tag']) && $fan['unionid'] && !$row['unionid']){
				pdo_update('mc_mapping_fans', array('unionid'=>$fan['unionid']), array('fanid' => $row['fanid']));
				continue;
			}
			if(!is_error($fan) && $fan['subscribe'] == 1) {
				$group = $acc->fetchFansGroupid($row['openid']);
				...
				if(!is_error($fan) && !$row['uniacid'] && $fan['unionid']){
					pdo_update('mc_mapping_fans', array('unionid'=>$fan['unionid']), array('fanid' => $row['fanid']));
				}
			}
		}
