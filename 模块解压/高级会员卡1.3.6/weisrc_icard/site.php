...
define('RES', '../addons/weisrc_icard/template/');
include "model.php";
include "templateMessage.php";

class weisrc_icardModuleSite extends WeModuleSite
{
    public $tablename = 'weisrc_icard_reply';
    public $modulename = 'weisrc_icard';
    ...
    public $table_business = "weisrc_icard_business";
    public $table_template = "weisrc_icard_template";
    public $table_gift = "weisrc_icard_gift";
    public $table_recharge = "weisrc_icard_recharge";
    public $table_card = "weisrc_icard_card";
    public $table_score = "weisrc_icard_score";
    public $table_announce = "weisrc_icard_announce";
    public $table_announce_read = "weisrc_icard_announce_read";
    public $table_menu = "weisrc_icard_menu";
    public $table_score_log = "weisrc_icard_score_log";
    public $table_sncode = "weisrc_icard_sncode";
    public $table_bill_log = "weisrc_icard_bill_log";
    public $table_card_log = "weisrc_icard_card_log";
    public $table_coupon = "weisrc_icard_coupon";
    public $table_level = "weisrc_icard_level";
    public $table_store = "weisrc_icard_store";
    public $table_password = "weisrc_icard_password";
    public $table_order = "weisrc_icard_order";
    public $table_style = "weisrc_icard_style";
    public $table_privilege = "weisrc_icard_privilege";
    public $table_sign = "weisrc_icard_sign";
    public $table_recharge_log = "weisrc_icard_recharge_log";
    public $table_olduser = "weisrc_icard_olduser";
    ...
    function __construct()
    {
        ...
        if ($_SERVER['HTTP_HOST'] == '127.0.0.1:8888') {
    ...
    public function doMobileGetMember()
    {
        ...
        $isfollow = false;
        if ($_W['fans']['follow'] == 1) { //关注用户直接获取信息
            $isfollow = true;
        }
        ...
        //商家信息
        $business = pdo_fetch("SELECT * FROM " . tablename($this->table_business) . " WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
        //开卡即送
        $gift = pdo_fetch("SELECT * FROM " . tablename($this->table_gift) . " WHERE weid = :weid AND status=1 AND :nowtime<endtime AND :nowtime>starttime ORDER BY `id` DESC LIMIT 1", array(':weid' => $weid, ':nowtime' => TIMESTAMP));
    ...
    public function doMobileGetOldCard()
    {
        ...
        $cardno = trim($_GPC['entry_cardno']);
        $mobile = trim($_GPC['entry_mobile']);
        if (empty($cardno)) {
            $this->showMessageAjax('请输入会员卡号!', 1);
        }
        if (empty($mobile)) {
        ...
        $olduser = pdo_fetch("SELECT * FROM " . tablename($this->table_olduser) . " WHERE cardno=:cardno AND mobile=:mobile LIMIT 1", array(':cardno' => $cardno, ':mobile' => $mobile));

        if (empty($olduser)) {
            $this->showMessageAjax('绑定会员不存在!', 1);
        }
        ...
        $data_old = array(
            ...
            'credit1' => $olduser['credit'],
            'credit2' => $olduser['coin']
        );
        mc_update($from_user, $data_old);
        pdo_update($this->table_olduser, array('status' => 1, 'from_user' => $from_user), array('id' => $olduser['id']));
        $this->setFansCredit($olduser['credit'], '预存积分');
        $this->setFansMoney($olduser['coin'], '预存金额');
        ...
        $data_card = array(
            ...
            'sign_score' => 0,
            'money' => 0,
            'carnumber' => trim($_GPC['carnumber']),//车牌
            'status' => 0,
            'updatetime' => TIMESTAMP,
            'dateline' => TIMESTAMP
        );
        $data_card['cardnumber'] = $olduser['cardno'];//卡号

        $flag = pdo_insert($this->table_card, $data_card);
        if ($flag > 0) {
            //开卡即送
            $gift = pdo_fetch("SELECT * FROM " . tablename($this->table_gift) . " WHERE weid = :weid AND status=1 AND :nowtime<endtime AND :nowtime>starttime ORDER BY id DESC LIMIT 1", array(':weid' => $weid, ':nowtime' => TIMESTAMP));
            if (!empty($gift) && $gift['score'] > 0) {
                ...
                $announce_content = "尊敬的{$data_old['realname']}会员，您于{$datetime}参与获得{$gift['score']}积分，请及时查收。";
...
//        }
    ...
    public function addCardScoreLog($title, $type, $score, $operationtype = 0, $count = 0)
    {
        ...
        pdo_insert($this->table_score_log, $data_score_log);
    }
    ...
    public function doMobileIndex()
    {
        ...
        //优惠券
        $coupons = pdo_fetchall("SELECT * FROM (SELECT a.id as id, a.title as title,b.id as sncodeid FROM " . tablename($this->table_coupon) . " a INNER JOIN " . tablename($this->table_sncode) . " b ON a.id = b.pid WHERE b.from_user=:from_user AND isshow=1 AND status=0 AND a.type in(1,2) AND a.weid=:weid ORDER BY sncodeid DESC) c", array(':weid' => $weid, ':from_user' => $from_user));
        //业务关联
        $menus = pdo_fetchall("SELECT * FROM " . tablename($this->table_menu) . " WHERE status=1 AND weid=:weid ORDER BY displayorder DESC,id DESC limit 5", array(':weid' => $weid));
    ...
    public function doMobileAnnounce()
    {
        ...
        //会员卡通知
        $announces = pdo_fetchall("SELECT * FROM " . tablename($this->table_announce) . " $strwhere ORDER BY id DESC limit 50", array(':weid' => $weid));
        //阅读记录
        $announcesRead = pdo_fetchall("SELECT * FROM " . tablename($this->table_announce_read) . " WHERE weid=:weid AND from_user=:from_user", array(':weid' => $weid, ':from_user' => $from_user), 'announceid');
    ...
    public function doMobilePayResult()
    {
        ...
        $bill = pdo_fetch("SELECT * FROM " . tablename($this->table_bill_log) . " WHERE id=:id", array(':id' => $billid));
        if (empty($bill)) {
            message('记录不存在');
        }
        load()->model('mc');
    ...
    public function getCardLevel($total_score)
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        return pdo_fetch("SELECT * FROM " . tablename($this->table_level) . " WHERE weid = :weid and :totalscore>=min and :totalscore<=max ORDER BY `min` limit 1", array(':weid' => $weid, ':totalscore' => $total_score));
    }
    ...
    public function getCardStyle()
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        return pdo_fetch("SELECT * FROM " . tablename($this->table_style) . " WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
    }
    ...
    public function getBusiness()
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        return pdo_fetch("SELECT * FROM " . tablename($this->table_business) . " WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
    }
    ...
    public function getPrivileges($type = 'level', $levelid = 0)
    {
        ...
        if ($type == 'getmember') {
            $privileges = pdo_fetchall("SELECT * FROM " . tablename($this->table_privilege) . " WHERE weid = :weid and endtime>:time and FIND_IN_SET(0,levelids) ORDER BY displayorder DESC,id DESC", array(':weid' => $weid, ':time' => TIMESTAMP));
        } else {
            $privileges = pdo_fetchall("SELECT * FROM " . tablename($this->table_privilege) . " WHERE weid = :weid and endtime>:time and (FIND_IN_SET(:levelid,levelids) Or FIND_IN_SET(0,levelids)) ORDER BY displayorder DESC,id DESC", array(':weid' => $weid, ':time' => TIMESTAMP, ':levelid' => $levelid));
        }
        return $privileges;
    }
...
//        } else {
//            $level = pdo_fetch("SELECT id FROM " . tablename($this->table_level) . " WHERE weid = :weid and :totalscore>=min and :totalscore<=max ORDER BY `min` limit 1", array(':weid' => $weid, ':totalscore' => $totalscore));
//            $levelid = intval($level['id']);
//            $strWhere = " WHERE weid = :weid AND (((levelid = 0  OR levelid = $levelid) AND type = 0) OR (type=1 AND from_user='$from_user')) ";
//            $userAnnounceCount = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_announce) . " $strWhere", array(':weid' => $weid));
//            $readcount = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_announce_read) . " WHERE weid=:weid AND from_user=:from_user", array(':weid' => $weid, ':from_user' => $from_user));
...
//        }
    ...
    public function doMobileRead()
    {
        ...
        //判断通知是否存在
        $announce = pdo_fetch("SELECT * FROM " . tablename($this->table_announce) . " WHERE id=:id AND weid=:weid  ORDER BY id DESC limit 1", array(':weid' => $weid, ':id' => $id));
        if (empty($announce)) {
            return false;
        }

        //判断是否已经阅读
        $flag = pdo_fetch("SELECT id FROM " . tablename($this->table_announce_read) . " WHERE weid = :weid and from_user=:from_user AND announceid=:announceid limit 1", array(':weid' => $weid, ':from_user' => $from_user, ':announceid' => $id));
        ...
        if (empty($flag)) {
            ...
            pdo_insert($this->table_announce_read, $data);
...
//            }
            ...
            $flag = pdo_insert($this->table_sign, $data_sign);
            if ($flag > 0) { //增加会员卡积分
                $card = $this->getUserCard();
                if (!empty($card)) {
                    //更新会员卡总积分、签到积分
    ...
    public function setCardCredit($score, $type = 'sign', $add = true)
    {
        ...
        return pdo_update($this->table_card, $fields, array('from_user' => $from_user));
    }
    ...
    public function doMobileScorelist()
    {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_score_log) . " {$condition} ORDER BY id DESC limit 50", array(':weid' => $weid, ':from_user' => $from_user));
    ...
    public function doMobileUpdateUserinfo()
    {
        ...
        if (!empty($_GPC['carnumber'])) {
            pdo_query("UPDATE " . tablename($this->table_card) . " SET carnumber = :carnumber WHERE from_user=:from_user", array(':carnumber' => trim($_GPC['carnumber']), ':from_user' => $from_user));
        }
        load()->model('mc');
    ...
    public function doMobileExchangeList()
    {
        ...
        $stores = pdo_fetchall("select * from " . tablename($this->table_store) . " WHERE weid=:weid", array(':weid' => $weid), 'id');
        ...
        $exchanges = pdo_fetchall("SELECT a.id as id, a.title as title,a.needscore as needscore,a.endtime as endtime,b.thumb as thumb,b.content as content FROM " . tablename($this->table_coupon) . " a INNER JOIN " . tablename($this->table_coupon) . " b ON a.ticket_id = b.id WHERE a.weid=:weid AND :time<a.endtime AND a.type=4 AND b.type<>4 AND (a.levelid=0 OR a.levelid=:levelid) order by a.displayorder DESC,a.id DESC", array(':weid' => $weid, ':time' => TIMESTAMP, ':levelid' => $level['id']));
    ...
    public function doMobileCoupon()
    {
        ...
        $strwhere = $this->getCouponStrWhere($level['id']);
        $condition .= " AND type={$type} ";
        $coupons = pdo_fetchall("SELECT * FROM " . tablename($this->table_coupon) . " WHERE weid = :weid AND :time<endtime  AND attr_type=1 And (" . $strwhere . ") $condition ORDER BY displayorder DESC,id DESC limit 50", array(':weid' => $weid, ':time' => TIMESTAMP));
    ...
    public function getUserCouponUseTimes($type)
    {
        ...
        //取得兑换礼品兑换次数
        $data = array();
        $sncodes = pdo_fetchall("SELECT COUNT(1) as count,pid FROM " . tablename($this->table_sncode) . " GROUP BY from_user,type,pid,weid having weid = :weid AND from_user=:from_user AND type=:type ", array(':weid' => $weid, ':from_user' => $from_user, ':type' => $type));
    ...
    public function getCouponUseTimes($type)
    {
        ...
        //取得兑换礼品兑换次数
        $data = array();
        $sncodes = pdo_fetchall("SELECT COUNT(1) as count,pid FROM " . tablename($this->table_sncode) . " GROUP BY from_user,type,pid,weid having weid = :weid AND type=:type ", array(':weid' => $weid, ':type' => $type));
        foreach ($sncodes as $key => $value) {
            $data[$value['pid']] = $value['count'];
        }
        return $data;
    ...
    public function doMobileUseCoupon()
    {
        ...
        if (empty($coupon)) {
            ...
            //未使用、未隐藏最新的数据
            $sncode = pdo_fetch("SELECT * FROM ".tablename("weisrc_icard_sncode")." WHERE pid=:pid AND from_user=:from_user AND status=0 AND isshow=1 ORDER BY id DESC LIMIT 1", array(':pid' => $id, ':from_user' => $from_user));
        }

        if ($coupon['storeid'] == 0) {
            //$stores = pdo_fetchall("SELECT * FROM ".tablename($this->table_store)." WHERE weid=:weid", array(':weid' => $weid));
        } else {
            $store = pdo_fetch("SELECT * FROM ".tablename($this->table_store)." WHERE id=:id LIMIT 1", array(':id' => $coupon['storeid']));
        }
    ...
    public function doMobileUseSncode()
    {
        ...
        $sncode = pdo_fetch("SELECT * FROM ".tablename("weisrc_icard_sncode")." WHERE id=:id AND isshow=1 ORDER BY id DESC LIMIT 1", array(':id' => $sncodeid));
        if (empty($sncode)) {
        ...
        //检查优惠券
        $couponid = $sncode['pid'];
        $coupon = pdo_fetch("SELECT * FROM ".tablename($this->table_coupon)." WHERE id=:id ORDER BY id DESC LIMIT 1", array(':id' => $couponid));
        ...
        if ($coupon['type'] == 3) {
            //更新sn码使用状态
            pdo_update($this->table_sncode, array('status' => 1, 'usetime' => TIMESTAMP), array('id' => $sncodeid));
...
//            }
        ...
        $obj_score = pdo_fetch("SELECT * FROM " . tablename($this->table_score) . " WHERE weid = :weid ", array(':weid' => $weid));
        ...
        pdo_insert($this->table_bill_log, $data);
        $billid = pdo_insertid();
        ...
        //更新sn码使用状态
        pdo_update($this->table_sncode, array('status' => 1, 'storeid' => $storeid, 'passwordid' => $passwordid, 'usetime' => TIMESTAMP), array('id' => $sncodeid));
    ...
    public function doMobileTest()
    {
        ...
        $template = array(
            ...
            'data' => array(
                'first' => array(
                    'value' => urlencode("迷微赞科技通知你，你已成功报名了活动"),
                    'color' => '#f00'
                ),
                'keyword1' => array(
    ...
    public function doMobileDelUserCoupon()
    {
        ...
        if (empty($coupon)) {
            ...
            if (TIMESTAMP > $coupon['endtime']) { //过期的全隐藏
                pdo_update("weisrc_icard_sncode", array('isshow' => 0), array('from_user' => $from_user, 'weid' => $weid, 'pid' => $id));
            } else { //隐藏已使用的券
                pdo_update("weisrc_icard_sncode", array('isshow' => 0), array('from_user' => $from_user, 'weid' => $weid, 'status' => 1, 'pid' => $id));
            }
        }
    ...
    public function doMobileGetCoupon()
    {
        ...
        if (empty($coupon)) {
            ...
            if (TIMESTAMP < $coupon['starttime']) {
                $this->showMessageAjax('活动时间还未开始,不能领取...', 1);
            }

            if (TIMESTAMP > $coupon['endtime']) {
                $this->showMessageAjax('活动时间已经结束啦,不能领取...', 1);
            }
        }
        ...
        //用户已领取数量
        $user_coupon_count = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_sncode) . " WHERE weid = :weid and from_user=:from_user AND pid=:pid ORDER BY id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':pid' => $id));
        //已领取总数量
        $user_coupon_total_count = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_sncode) . " WHERE weid = :weid AND pid=:pid ORDER BY id DESC", array(':weid' => $weid, ':pid' => $id));
    ...
    public function doMobileUseExchange()
    {
        ...
        $id = intval($_GPC['id']);
        //积分兑换券
        $exchange = pdo_fetch("SELECT * FROM ".tablename($this->table_coupon)." WHERE id=:id LIMIT 1", array(':id' => $id));
        if (empty($exchange)) {
            ...
            //用户已领取数量
            $user_exchange_count = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_sncode) . " WHERE weid = :weid and from_user=:from_user AND pid=:pid ORDER BY id DESC", array(':weid' => $weid, ':from_user' => $from_user, ':pid' => $id));
            //已领取总数量
            $user_exchange_total_count = pdo_fetchcolumn("SELECT count(1) FROM " . tablename($this->table_sncode) . " WHERE weid = :weid AND pid=:pid ORDER BY id DESC", array(':weid' => $weid, ':pid' => $id));
            ...
            $this->addSncode($data);
            $coupon = pdo_fetch("SELECT * FROM ".tablename($this->table_coupon)." WHERE id=:id LIMIT 1", array(':id' => $exchange['ticket_id']));
    ...
    public function doMobileBill()
    {
        ...
        //查询某月份的数据
        $sql_child = "select * from " . tablename($this->table_bill_log) . " where weid =:weid AND from_user=:from_user AND date_format(FROM_UNIXTIME(dateline), '%Y-%m')='{$y}-{$m}'";
        //分组查询每日消费总金额
        ...
        $datalist = pdo_fetchall("SELECT * FROM " . tablename($this->table_bill_log) . " WHERE weid = :weid AND from_user=:from_user {$condition_date} ORDER BY id DESC", array(':weid' => $weid, ':from_user' => $from_user));
        $totalMoney = pdo_fetchcolumn("SELECT sum(money) FROM " . tablename($this->table_bill_log) . " WHERE weid = :weid AND from_user=:from_user AND operationtype=1 $condition_date", array(':weid' => $weid, ':from_user' => $from_user));
        $totalMoney2 = pdo_fetchcolumn("SELECT sum(money) FROM " . tablename($this->table_bill_log) . " WHERE weid = :weid AND from_user=:from_user AND operationtype=0 $condition_date", array(':weid' => $weid, ':from_user' => $from_user));
    ...
    public function doMobileSetPayPassIng()
    {
        ...
        pdo_update($this->table_card, $data, array('id' => $card['id'], 'weid' => $weid));
        $this->showMessageAjax('操作成功!');
    }
    ...
    {
        global $_W, $_GPC;
        $sncode = pdo_fetch("SELECT * FROM " . tablename($this->table_sncode) . " WHERE pid = :pid AND type = :type AND status !=1 AND from_user=:from_user ORDER BY `id` DESC limit 1", array(':pid' => $id, ':type' => $type, ':from_user' => $from_user));
        return $sncode;
    }
    ...
    public function doMobileStore()
    {
        ...
        //门店列表
        $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " WHERE weid = :weid and is_show=1 ORDER BY displayorder DESC,id DESC limit 50", array(':weid' => $weid));
        //会员卡
        $card = pdo_fetch("SELECT * FROM " . tablename($this->table_card) . " WHERE weid = :weid and from_user=:from_user ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':from_user' => $from_user));
    ...
    public function getSncodeCount($type = 1)
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        return pdo_fetchcolumn("SELECT sum(1) AS nums FROM (SELECT b.id as sncodeid FROM " . tablename($this->table_coupon) . " a INNER JOIN " . tablename($this->table_sncode) . " b ON a.id = b.pid WHERE from_user=:from_user AND isshow=1 AND status=0 AND a.type=:type AND a.weid=:weid ORDER BY sncodeid DESC) c", array(':weid' => $weid, ':from_user' => $from_user, ':type' => $type));
    }
    ...
    public function doMobileRechargeing()
    {
        ...
        $paytype = intval($_GPC['paytype']); //支付类型1:现金2:在线
        ...
        if ($paytype == 1) {
            ...
            $obj_score = pdo_fetch("SELECT * FROM " . tablename($this->table_score) . " WHERE weid = :weid ", array(':weid' => $weid));
            $recharge_score = intval($obj_score['recharge_score']);
            $recharge_totalscore = $money * $recharge_score;
            $remark = '充值赠送积分';
            if (!empty($fans) && !empty($recharge_score)) {
                $uid = intval($fans['uid']);
                $log = array();
                $log[0] = $uid;
                $log[1] = $remark;
                mc_credit_update($uid, 'credit1', $recharge_totalscore, $log);

                $data_score_log = array(
                    'weid' => $weid,
                    'from_user' => $from_user,
                    'type' => 3,//积分类型 签到:1，消费:2,充值:3
                    'title' => '充值赠送积分',
                    'score' => $recharge_totalscore,
                    'operationtype' => 1,//积分操作类型 增加:1  扣除:0
                    'count' => 0,
                    'dateline' => TIMESTAMP
                );
            ...
            //充值赠送
            $recharge_setting = pdo_fetch("SELECT * FROM " . tablename($this->table_recharge) . " WHERE weid = :weid AND status=1 AND :nowtime<endtime AND :nowtime>starttime AND :recharge_value>=recharge_value ORDER BY `recharge_value` DESC,`id` DESC LIMIT 1", array(':weid' => $weid, ':nowtime' => TIMESTAMP, ':recharge_value' => $money));
            if (!empty($recharge_setting) && $recharge_setting['give_value'] > 0) {
                $remark = '充值赠送';
                if (!empty($fans)) {
                    $uid = intval($fans['uid']);
                    $log = array();
                    $log[0] = $uid;
                    $log[1] = $remark;
                    mc_credit_update($uid, 'credit2', $recharge_setting['give_value'], $log);
                }

                //通知
                $datetime = date('Y年m月d日H点i分', TIMESTAMP);
                $announce_content = "您的尾号{$card['cardnumber']}会员卡于{$datetime}参与充值赠送获得{$recharge_setting['give_value']}元，请及时查收。";
                $this->AddAnnounce('充值赠送', $announce_content, $from_user, $this->_announce_card, -1);
            }
    ...
    public function doMobileRecharge()
    {
        ...
        if ($order['status'] != '0') {
            message('抱歉，您的订单已经付款或是被关闭，请重新充值！', $this->createMobileUrl('index', array()), 'error');
        }
    ...
    public function payResult($params)
    {
        ...
        if ($card['status'] == 1) {
            message('您的会员卡已被冻结,请联系商户！');
        }

        if ($params['type'] == 'wechat') {
            $data['transid'] = $params['tag']['transaction_id'];
        } else {
            message('只能选择在线支付充值！' , $this->createMobileUrl('index', array(), true), 'error');
        }
        ...
        if ($params['from'] == 'return') {
            $setting = uni_setting($weid, array('creditbehaviors'));
            ...
            if ($data['status'] == 1) {
                ...
                $this->AddAnnounce($announce_title, $announce_content, $from_user, $announce_type, -1);
                $this->addCardRechargeLog(1, $money, $card['id'], 0, 0);
            ...
            if ($params['type'] == $credit) {
                message('充值成功！' , $this->createMobileUrl('index', array(), true), 'success');
            } else {
                message('充值成功！' , '../../app/' . $this->createMobileUrl('index', array(), true), 'success');
            }
        }
    }
...
//        );
    ...
    public function getTodaySignStatus()
    {
        ...
        $date = date('Y-m-d');
        $sign = pdo_fetch("SELECT * FROM " . tablename($this->table_sign) . " WHERE weid = :weid and from_user = :from_user and  date_format(FROM_UNIXTIME(dateline), '%Y-%m-%d') = :date ", array(':weid' => $weid, ':from_user' => $from_user, ':date' => $date));
        if (!empty($sign)) {
            return 1;
        } else {
            return 0;
    ...
    public function getSignInMonth($y, $m)
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        $data = pdo_fetchall("SELECT * FROM " . tablename($this->table_sign) . " WHERE from_user = :from_user and weid = :weid and date_format(FROM_UNIXTIME(dateline),'%Y-%m') = '{$y}-{$m}'", array(':from_user' => $from_user, ':weid' => $weid));
        return $data;
    }
    ...
    public function getSignList()
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        $data = pdo_fetchall("SELECT * FROM " . tablename($this->table_sign) . " WHERE from_user = :from_user and weid = :weid ORDER BY dateline DESC", array(':from_user' => $from_user, ':weid' => $weid));
        return $data;
    }
    ...
    public function getAnnounceCount($level)
    {
        ...
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        $sql = "SELECT COUNT(*) FROM " . tablename($this->table_announce) . " WHERE weid = :weid and (levelid=0 or levelid = :level or (levelid=-1 and from_user=:from_user)) ORDER BY id DESC limit 50";
    ...
    {
        global $_W, $_GPC;
        $sign = pdo_fetch("SELECT * FROM " . tablename($this->table_sign) . " WHERE weid = :weid and from_user = :from_user order by dateline desc limit 1 ", array(':weid' => $weid, ':from_user' => $from_user));
        return $sign;
    }
    ...
    public function getCardNumber($weid)
    {
        ...
        //当前会员卡
        $card = pdo_fetch("select cardstart from " . tablename($this->table_style) . " where weid =" . $weid . " order by id desc limit 1");
        if (!empty($card)) {
            $cardstart = intval($card['cardstart']);
        }
        //查询公众号会员卡目前最大卡号
        $user_card = pdo_fetch("select cardno from " . tablename($this->table_card) . " where weid =" . $weid . " order by id desc limit 1");
        if (!empty($user_card)) {
    ...
    public function updateBalanceScore($need_score)
    {
        ...
        pdo_query("UPDATE " . tablename($this->table_card) . " SET balance_score = balance_score-:needscore WHERE weid = :weid AND from_user = :from_user ", array(':needscore' => $need_score, ':weid' => $weid, ':from_user' => $from_user));
    }
    ...
    public function get_gift($id, $weid)
    {
        ...
        return pdo_fetch("SELECT * FROM " . tablename($this->table_gift) . " WHERE weid = :weid and id = :id ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':id' => $id));
    }
    ...
    public function get_privilege($id, $weid)
    {
        ...
        return pdo_fetch("SELECT * FROM " . tablename($this->table_privilege) . " WHERE weid = :weid and id = :id ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':id' => $id));
    }
    ...
    public function get_coupon($id, $weid)
    {
        ...
        return pdo_fetch("SELECT * FROM " . tablename($this->table_coupon) . " WHERE weid = :weid and id = :id ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':id' => $id));
    }

    public function getCardByAdmin($weid, $from_user)
    {
        return pdo_fetch("SELECT * FROM " . tablename($this->table_card) . " WHERE weid = :weid and from_user = :from_user ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':from_user' => $from_user));
    }
    ...
    {
        global $_W, $_GPC;
        return pdo_fetch("SELECT id FROM " . tablename($this->table_style) . " WHERE weid = :weid and pwd=:pwd ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':pwd' => $pwd));
    }
    ...
    public function checkUserPassword($username, $password)
    {
        global $_W, $_GPC;
        load()->model('user');
        return pdo_fetch("SELECT * FROM " . tablename($this->table_password) . " WHERE weid = :weid AND password=:password AND username=:username limit 1", array(':weid' => $_W['uniacid'], ':password' => user_hash($password, ''), ':username' => $username));
    }
    ...
    public function check_store_password($weid, $pwd, $id)
    {
        ...
        return pdo_fetch("SELECT id FROM " . tablename($this->table_store) . " WHERE weid = :weid and password=:pwd and id=:id ORDER BY `id` DESC limit 1", array(':weid' => $weid, ':pwd' => $pwd, ':id' => $id));
    }
    ...
    public function addCardRechargeLog($type, $score, $cardid = 0, $storeid = 0, $passwordid = 0)
    {
        ...
        pdo_insert($this->table_recharge_log, $data);
    }
...
//        }
//
//        $order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " WHERE id=:id", array(':id' => $params['tid']));
//
//        if (empty($order)) {
...
//
//        //更新订单状态
//        pdo_update($this->table_order, $data, array('id' => $params['tid']));
//
//        $weid = $order['weid'];
//        $from_user = $order['from_user'];
//        $price = $order['price'];
...
//
//        if ($order['status'] == 0) {
//            $rowcount = pdo_query("UPDATE " . tablename($this->table_card) . " SET coin = coin+:price WHERE id=:id", array(':price' => $price, ':id' => $card["id"]));
//            if ($rowcount > 0) {
//                $result['status'] = 1;
//                //日志
...
//    }
    ...
    public function getCardScore()
    {
        $weid = $this->_weid;
        $from_user = $this->_fromuser;
        return pdo_fetch("SELECT * FROM " . tablename($this->table_score) . " WHERE weid = :weid ORDER BY `id` DESC", array(':weid' => $weid));
    }

    //tool function
    public function showMessageAjax($msg, $status = 0, $success = 0, $billid = 0)
    {
        $result = array('message' => $msg, 'status' => $status, 'issuccess' => $success, 'billid' => $billid, 'orderid' => 0);
        echo json_encode($result);
        exit;
    }
    ...
    public function doWebTemplate()
    {
        ...
        $tpl = dir(IA_ROOT . '/addons/weisrc_icard/template/mobile/');
        $tpl->handle;
        $templates = array();
    ...
    {
        $sql = "SELECT id,levelname FROM " . tablename($this->table_level) . " WHERE weid = :weid and :totalscore>=min and :totalscore<=max ORDER BY `min` limit 1";
    ...
    public function doWebStyle()
    {
        ...
        if (checksubmit('submit')) {
            ...
            $this->weisrc_icard_style_update($data);
            message('操作成功！', $url);
        } else {
            $reply = pdo_fetch("select * from " . tablename($this->table_style) . " where weid =" . $_W['uniacid']);
            ...
            if (!empty($reply)) {
                ...
                if (!empty($reply['bg'])) {
                    ...
                    $bg = "../attachment/images/global/card/1.png";
                }
...
//                }
                $bg_num = $i;
                $imgpath = "../attachment/images/global/card/" . $bg_num . ".png"; ".png";
                if ($reply['bg'] == $imgpath) {
    ...
    public function doWebBusiness()
    {
        ...
        $item = pdo_fetch("select * from " . tablename($this->table_business) . " where weid = :weid", array(':weid' => $weid));
        ...
        if (checksubmit('submit')) {
            ...
            if (!empty($item)) {
                unset($data['dateline']);
                pdo_update($this->table_business, $data, array('weid' => $weid));
            } else {
                pdo_insert($this->table_business, $data);
            }
            message('操作成功！', $url);
        }
    ...
    public function doWebScore()
    {
        ...
        $reply = pdo_fetch("select * from " . tablename($this->table_score) . " where weid =" . $_W['uniacid']);
        if (checksubmit('submit')) {
            $data = array(
                ...
                'recharge_score' => intval($_GPC['recharge_score']),
            );
            ...
            if (!empty($reply)) {
                $this->weisrc_icard_score_update($data);
            } else {
                $this->weisrc_icard_score_insert($data);
            }
            message('操作成功！', $url);
        } else {
    ...
    public function doWebLevel()
    {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                if (is_array($_GPC['levelname'])) {
                    foreach ($_GPC['levelname'] as $id => $val) {
                        ...
                        pdo_update($this->table_level, $data, array('id' => $id));
                    }
                }
                //增加
                if (is_array($_GPC['newlevelname'])) {
                    foreach ($_GPC['newlevelname'] as $nid => $val) {
                        ...
                        pdo_insert($this->table_level, $data);
                    }
                }
            ...
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_level, array('id' => $id, 'weid' => $_W['uniacid']));
            }
            message('操作成功!', $url);
        }
    ...
    public function doWebAnnounce()
    {
        ...
        if ($operation == 'display') {
            ...
            $announces = pdo_fetchall("SELECT * FROM " . tablename($this->table_announce) . " {$where} order by displayorder desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            if (!empty($announces)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_announce) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_announce) . " where id =" . $id);
            $levels = pdo_fetchall("SELECT * FROM " . tablename($this->table_level) . " WHERE weid = '{$_W['uniacid']}' order by max");
            if (!empty($reply)) {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    pdo_update($this->table_announce, $data, array('id' => $id, 'weid' => $_W['uniacid']));
                } else {
            ...
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_announce, array('id' => $id, 'weid' => $weid));
            }
            message('操作成功!', $url);
        }
    ...
    {
        global $_W;
        $levels = pdo_fetchall("SELECT * FROM " . tablename($this->table_level) . " WHERE weid = '{$_W['uniacid']}' ");
        $arr = array();
        $arr[0] = "所有会员";
    ...
    public function doWebCoupon()
    {
        ...
        if ($operation == 'display') {
            ...
            $sncount = pdo_fetchall("SELECT count(1) as count,pid FROM " . tablename($this->table_sncode) . " WHERE weid={$weid} GROUP BY pid", array(), 'pid');
            //普通券
            $type_count1 = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_coupon) . " WHERE attr_type = 1 AND weid=:weid AND type<>4", array(':weid' => $weid));
            //营销券
            $type_count2 = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_coupon) . " WHERE attr_type = 2 AND weid=:weid AND type<>4", array(':weid' => $weid));

            //优惠券券
            $coupon_count1 = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_coupon) . " WHERE type = 1 AND type<>4 AND weid=:weid", array(':weid' => $weid));
            //代金券
            $coupon_count2 = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_coupon) . " WHERE type = 2 AND type<>4 AND weid=:weid", array(':weid' => $weid));
            //优惠券券
            $coupon_count3 = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_coupon) . " WHERE type = 3 AND type<>4 AND weid=:weid", array(':weid' => $weid));
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_coupon) . " where id =" . $id);
            if (!empty($reply)) {
        ...
        if ($operation == 'display') {
            if (checksubmit('submit')) { //排序
                if (is_array($_GPC['displayorder'])) {
                    foreach ($_GPC['displayorder'] as $id => $val) {
                        $data = array('displayorder' => intval($_GPC['displayorder'][$id]));
                        pdo_update($this->table_gift, $data, array('id' => $id));
                    }
                }
                message('操作成功!', $url);
            }
            ...
            $gifts = pdo_fetchall("SELECT * FROM " . tablename($this->table_gift) . " {$where} order by displayorder desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            if (!empty($gifts)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_gift) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_gift) . " where id = :id", array(':id' => $id));
            ...
            if (!empty($reply)) {
                ...
                $starttime = TIMESTAMP;
                $endtime = strtotime(date("Y-m-d H:i", TIMESTAMP + 7 * 24 * 3600));
            }
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    unset($data['status']);
                    pdo_update($this->table_gift, $data, array('id' => $id, 'weid' => $_W['uniacid']));
                } else {
                    pdo_insert($this->table_gift, $data);
                }
                message('操作成功!', $url);
            }
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_gift, array('id' => $id, 'weid' => $_W['uniacid']));
            }
            message('操作成功!', $url);
            ...
            if ($status == 0) {
                pdo_query("UPDATE " . tablename($this->table_gift) . " SET status = 0 WHERE weid=:weid", array(':weid' => $_W['uniacid']));
            }
            pdo_query("UPDATE " . tablename($this->table_gift) . " SET status = abs(status - 1) WHERE id=:id AND weid=:weid", array(':id' => $id, ':weid' => $_W['uniacid']));
            message('操作成功!', $url);
        }
    ...
    public function doWebRecharge()
    {
        ...
        if ($operation == 'display') {
            ...
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_recharge) . " where id = :id", array(':id' => $id));
            ...
            if (checksubmit('submit')) {
                $data = array(
                    ...
                    'storeid' => intval($_GPC['store']),
                    'type' => 1,
                    'recharge_value' => intval($_GPC['recharge_value']),
                    'give_value' => intval($_GPC['give_value']),
                    'displayorder' => intval($_GPC['displayorder']),
                    'starttime' => strtotime($_GPC['datelimit']['start']),
                    'endtime' => strtotime($_GPC['datelimit']['end']),
                ...
                if ($data['recharge_value'] <= 0) {
                    message('充值金额必须大于0。', '', 'error');
                }
                if ($data['give_value'] <= 0) {
                    message('赠送金额必须大于0。', '', 'error');
                }
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    unset($data['status']);
                    pdo_update($this->table_recharge, $data, array('id' => $id, 'weid' => $weid));
                } else {
            ...
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_recharge, array('id' => $id, 'weid' => $weid));
            }
            message('操作成功!', $url);
        } else if ($operation == 'check') {
            $id = intval($_GPC['id']);
            $status = intval($_GPC['status']);
            if ($status == 0) {
                pdo_query("UPDATE " . tablename($this->table_recharge) . " SET status = 0 WHERE weid=:weid", array(':weid' => $weid));
            }
            pdo_query("UPDATE " . tablename($this->table_recharge) . " SET status = abs(status - 1) WHERE id=:id AND weid=:weid", array(':id' => $id, ':weid' => $weid));
            message('操作成功!', $url);
        }
    ...
    public function doWebExchange()
    {
        ...
        $category = pdo_fetchall("SELECT * FROM " . tablename($this->table_coupon) . " WHERE weid = :weid AND attr_type=2 AND :time<endtime ORDER BY displayorder DESC,id DESC", array(':weid' => $weid, ':time' => TIMESTAMP), 'id');
        if (!empty($category)) {
            $children = array();
        ...
        if ($operation == 'display') {
            ...
            $psize = 15;
            $where = "WHERE weid = '{$weid}' AND type=4 ";
            $exchanges = pdo_fetchall("SELECT * FROM " . tablename($this->table_coupon) . " {$where} order by displayorder desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            if (!empty($exchanges)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_coupon) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }
            ...
            //等级
            $levels = pdo_fetchall("SELECT * FROM " . tablename($this->table_level) . " WHERE weid = :weid order by max", array(':weid' => $weid));
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    pdo_update($this->table_coupon, $data, array('id' => $id, 'weid' => $weid));
                } else {
                    pdo_insert($this->table_coupon, $data);
                }
                message('操作成功!', $url);
            ...
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_coupon, array('id' => $id, 'weid' => $weid));
            }
            message('操作成功!', $url);
        }
        ...
        if ($operation == 'display') {
            if (checksubmit('submit')) { //排序
                if (is_array($_GPC['displayorder'])) {
                    foreach ($_GPC['displayorder'] as $id => $val) {
                        $data = array('displayorder' => intval($_GPC['displayorder'][$id]));
                        pdo_update($this->table_privilege, $data, array('id' => $id));
                    }
                }
                message('操作成功!', $url);
            }
            ...
            $gifts = pdo_fetchall("SELECT * FROM " . tablename($this->table_privilege) . " {$where} order by displayorder desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            if (!empty($gifts)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_privilege) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_privilege) . " where id = :id AND weid=:weid", array(':id' => $id, ':weid' => $_W['weid']));
            if (!empty($reply)) {
                $levelarr = explode(',', $reply['levelids']);
            }
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($reply)) {
                    unset($data['dateline']);
                    pdo_update($this->table_privilege, $data, array('id' => $id, 'weid' => $_W['uniacid']));
                } else {
                    $total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_privilege) . " where weid = {$_W['uniacid']}");
                    if ($total >= 8) {
                        message('您最多可以创建8条特权!', $url);
                    }
                    pdo_insert($this->table_privilege, $data);
                }
                message('操作成功!', $url);
            }
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_privilege, array('id' => $id, 'weid' => $_W['uniacid']));
            }
            message('操作成功!', $url);
        }
    ...
    public function doWebStore()
    {
        ...
        if ($operation == 'display') {
            ...
            $psize = 15;
            $where = "WHERE weid = '{$_W['uniacid']}'";
            $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " {$where} order by displayorder desc,id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
            if (!empty($stores)) {
            ...
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_store) . " where id =" . $id);
            if (checksubmit('submit')) {
                $data = array(
                    'weid' => intval($_W['uniacid']),
                    'title' => trim($_GPC['title']),
    ...
    public function doWebSncodeList()
    {
        ...
        $coupon_list = pdo_fetchall("SELECT * FROM " . tablename($this->table_coupon) . " WHERE weid = '{$_W['uniacid']}' ", array(), 'id');

        //门店
        $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " WHERE weid = :weid ORDER BY displayorder DESC,id DESC", array(':weid' => $weid), 'id');

        $password = pdo_fetchall("SELECT * FROM " . tablename($this->table_password) . " WHERE weid = '{$_W['uniacid']}' ORDER BY  id DESC", array(), 'id');
        ...
        $list = pdo_fetchall("SELECT a.*,b.id as cardid,b.cardnumber as cardnumber FROM " . tablename($this->table_sncode) . " a INNER JOIN " . tablename($this->table_card) . " b ON a.from_user=b.from_user {$where} order by a.id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");

        if (!empty($list)) {
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_sncode) . " $wheretotal");
            $pager = pagination($total, $pindex, $psize);
        }
    ...
    public function doWebUseSncodeAdmin()
    {
        ...
        $sncode = pdo_fetch("SELECT * FROM " . tablename($this->table_sncode) . " WHERE weid = :weid AND id=:id", array(':weid' => $weid, ':id' => $sncodeid));
        ...
        //更新sn码使用状态
        pdo_update($this->table_sncode, array('status' => 1, 'money' => $money, 'storeid' => $storeid, 'passwordid' => $passwordid, 'usetime' => TIMESTAMP), array('id' => $sncodeid));
    ...
    public function doWebStoreLog()
    {
        ...
        $sql = "SELECT a.*,b.cardpre as cardpre,b.cardno as cardno,b.from_user as from_user,b.cardnumber as cardnumber,b.id as cardid FROM " . tablename($this->table_bill_log) . " a INNER JOIN " . tablename($this->table_card) . " b ON a.from_user=b.from_user {$where} order by a.id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}";

        $list = pdo_fetchall($sql);
        if (!empty($list)) {
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_bill_log) . " $wheretotal");
            $pager = pagination($total, $pindex, $psize);
        }
        ...
        //门店
        $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " WHERE weid = :weid ORDER BY displayorder DESC,id DESC", array(':weid' => $_W['uniacid']));
    ...
    public function doWebShopingLog()
    {
        ...
        $card = pdo_fetch("select * from " . tablename($this->table_card) . " where id =" . $cardid);
        if (empty($card)) {
            message('会员卡不存在');
        }
        ...
        $pindex = max(1, intval($_GPC['page']));
        $psize = 15;
        $where = "WHERE weid = '{$weid}' and from_user = '{$card['from_user']}'";
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_bill_log) . " {$where} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
        if (!empty($list)) {
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_bill_log) . " $where");
            $pager = pagination($total, $pindex, $psize);
        }
        ...
        $password = pdo_fetchall("SELECT * FROM " . tablename($this->table_password) . " WHERE weid = :weid ORDER BY  id DESC", array(':weid' => $weid), 'id');
    ...
    public function doWebRechargeLog()
    {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_recharge_log) . " {$where} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
        if (!empty($list)) {
            $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_recharge_log) . " $where");
            $pager = pagination($total, $pindex, $psize);
        }
    ...
    public function doWebAllRechargeLog()
    {
        ...
        $wheretotal = "WHERE weid = {$weid} ";
        $sql = "SELECT a.*,b.cardpre as cardpre,b.cardno as cardno,b.from_user as from_user,b.cardnumber as cardnumber FROM " . tablename($this->table_recharge_log) . " a INNER JOIN " . tablename($this->table_card) . " b ON a.cardid=b.id {$where} order by a.id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}";
        ...
        if (!empty($list)) {
            $total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_recharge_log) . " $wheretotal");
            $pager = pagination($total, $pindex, $psize);
        }

        $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " WHERE weid = :weid ORDER BY displayorder DESC,id DESC", array(':weid' => $_W['uniacid']), "id");
    ...
    public function doWebRechargeLogExcel()
    {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_card_log) . " {$where} order by id desc");
        $stores = pdo_fetchall("SELECT * FROM " . tablename($this->table_store) . " WHERE weid = :weid ORDER BY displayorder DESC,id DESC", array(':weid' => $_W['uniacid']));
        $store = array();
        foreach ($stores as $key => $value) {
            $store[$value['id']] = $value['title'];
    ...
    public function doWebCardExcel()
    {
        ...
        $cardlist = pdo_fetchall("select * from " . tablename($this->table_card) . " where weid =:weid", array(':weid' => $_W['uniacid']));
    ...
    public function get_gifts_arr($type)
    {
        ...
        switch ($type) {
            case 4:
                $tablename = tablename($this->table_gift);
                break;
            case 3:
                $tablename = tablename($this->table_privilege);
                break;
            case 2:
                $tablename = tablename($this->table_coupon);
                break;
        }
    ...
    public function doWebPassword()
    {
        ...
        $stores = pdo_fetchall("select * from " . tablename($this->table_store) . " WHERE weid=:weid", array(':weid' => $weid), 'id');
        if ($operation == 'display') {
            $password = pdo_fetchall("SELECT * FROM " . tablename($this->table_password) . " WHERE weid = :weid ORDER BY id DESC", array(':weid' => $_W['uniacid']));
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $password = pdo_fetch("select * from " . tablename($this->table_password) . " WHERE id=:id", array(':id' => $id));
            }
            if (checksubmit()) {
                $data = array(
                    'weid' => $_W['uniacid'],
                ...
                if (empty($password)) {
                    pdo_insert($this->table_password, $data);
                } else {
                    unset($data['type']);
                    pdo_update($this->table_password, $data, array('id' => $id, 'weid' => $_W['uniacid']));
                }
                message('数据更新成功！', $this->createWebUrl('password', array('op' => 'display')), 'success');
            }
        } else if ($operation == 'check') {
            $id = intval($_GPC['id']);
            pdo_query("UPDATE " . tablename($this->table_password) . " SET status=abs(1-status) WHERE id=:id AND weid = :weid", array(':id' => $id, ':weid' => $_W['uniacid']));
            message('数据更新成功！', $this->createWebUrl('password', array('op' => 'display')), 'success');
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $password = pdo_fetch("select * from " . tablename($this->table_password) . " WHERE id=:id", array(':id' => $id));
...
//                }
                pdo_delete($this->table_password, array('id' => $id, 'weid' => $_W['uniacid']));
            }
            message('数据更新成功！', $this->createWebUrl('password', array('op' => 'display')), 'success');
        }
        ...
        if ($operation == 'display') {
            if (!empty($_GPC['displayorder'])) {
                foreach ($_GPC['displayorder'] as $id => $displayorder) {
                    pdo_update($this->table_menu, array('displayorder' => $displayorder), array('id' => $id));
                }
                foreach ($_GPC['title'] as $id => $title) {
                    pdo_update($this->table_menu, array('title' => $title), array('id' => $id));
                }
                foreach ($_GPC['url'] as $id => $url) {
                    pdo_update($this->table_menu, array('url' => $url), array('id' => $id));
                }
                message('数据更新成功！', $this->createWebUrl('menu', array('op' => 'display')), 'success');
            }
            $menu = pdo_fetchall("SELECT * FROM " . tablename($this->table_menu) . " WHERE weid = :weid ORDER BY displayorder DESC,id DESC", array(':weid' => $_W['uniacid']));
        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $menu = pdo_fetch("select * from " . tablename($this->table_menu) . " WHERE id=:id", array(':id' => $id));
            }
            if (checksubmit()) {
                $data = array(
                    'weid' => $_W['uniacid'],
                ...
                if (empty($menu)) {
                    pdo_insert($this->table_menu, $data);
                } else {
                    unset($data['type']);
                    pdo_update($this->table_menu, $data, array('id' => $id, 'weid' => $_W['uniacid']));
                }
                message('数据更新成功！', $this->createWebUrl('menu', array('op' => 'display')), 'success');
            }
        } else if ($operation == 'check') {
            $id = intval($_GPC['id']);
            pdo_query("UPDATE " . tablename($this->table_menu) . " SET status=abs(1-status) WHERE id=:id", array(':id' => $id));
            message('数据更新成功！', $this->createWebUrl('menu', array('op' => 'display')), 'success');
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $menu = pdo_fetch("select * from " . tablename($this->table_menu) . " WHERE id=:id", array(':id' => $id));
            if (!empty($menu)) {
                ...
                pdo_delete($this->table_menu, array('id' => $id, 'weid' => $_W['uniacid']));
            }
            message('数据更新成功！', $this->createWebUrl('menu', array('op' => 'display')), 'success');
        }
    ...
    {
        global $_W;
        $flag = pdo_fetch('SELECT * FROM ' . tablename($this->table_menu) . ' Where method=:method And weid=:weid', array(':method' => $method, ':weid' => $_W['uniacid']));
        if (empty($flag)) {
            $data = array(
                'weid' => $_W['uniacid'],
                'title' => $menuname,
    ...
    public function doWebCard()
    {
        ...
        if ($operation == 'display') {
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_card) . " {$where} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}", array(), 'uid');
            if (!empty($list)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_card) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }
            ...
            $id = intval($_GPC['id']);
            $reply = pdo_fetch("select * from " . tablename($this->table_card) . " where id = :id", array(':id' => $id));
            if (empty($reply)) {
            ...
            if (checksubmit('submit')) {
                ...
                pdo_query("UPDATE " . tablename($this->table_card) . " SET carnumber = :carnumber WHERE id=:id", array(':carnumber' => trim($_GPC['carnumber']), ':id' => $id));
                message('操作成功!', $url);
            }
        }
    ...
    public function doWebOldUser()
    {
        ...
        if ($operation == 'display') {
            ...
            if ($_GPC['out_put'] == 'output') {
                $sql = "select * from " . tablename($this->table_olduser)
                ...
                $i = 0;
                foreach ($list as $key => $value) {
                    $arr[$i]['cardno'] = $value['cardno'];
                    $arr[$i]['username'] = $value['username'];
                    $arr[$i]['mobile'] = $value['mobile'];
                    $arr[$i]['sex'] = $arrsex[$value['sex']];
                    $arr[$i]['idcard'] = $value['idcard'];
                    $arr[$i]['credit'] = $value['credit'];
                    $arr[$i]['coin'] = $value['coin'];
                    $arr[$i]['status'] = $arrstatus[$value['status']];
                    $i++;
                }

                $this->exportexcel($arr, array('会员卡号', '姓名', '电话', '性别', '身份证', '预存积分', '预存余额', '状态'), time());
                exit();
            }

            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_olduser) . " {$where} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}", array(), 'uid');
            if (!empty($list)) {
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_olduser) . " $where");
                $pager = pagination($total, $pindex, $psize);
            }

        } else if ($operation == 'post') {
            $id = intval($_GPC['id']);
            $item = pdo_fetch("select * from " . tablename($this->table_olduser) . " where id =:id", array(':id' => $id));
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($item)) {
                    pdo_update($this->table_olduser, $data, array('id' => $id));
                } else {
                    pdo_insert($this->table_olduser, $data);
                }
                message('操作成功!', $url);
            }
        } else if ($operation == 'delete') {
            $id = intval($_GPC['id']);
            if ($id > 0) {
                pdo_delete($this->table_olduser, array('id' => $id, 'weid' => $weid));
            }
            message('操作成功!', $url);
        }
        include $this->template('olduser');
    }

    function upload_olduser($strs, $time, $array)
    {
        global $_W;
        $weid = $_W['uniacid'];
        //会员姓名 性别 手机号码	注册门店	身份证	区域	预存余额	预存积分
        $sex = 0;
        if ($strs[4] == '男') {
            $sex = 1;
        }
        ...
        $user = pdo_fetch("SELECT * FROM ".tablename($this->table_olduser). " WHERE username=:username AND mobile=:mobile AND weid=:weid ", array(':username' => $strs[1], ':mobile' => $strs[2], ':weid' => $weid));
        if (empty($user)) {
            return pdo_insert($this->table_olduser, $insert);
        } else {
            return 0;
        }
    }
    ...
    {
        //自己设置的上传文件存放路径
        $filePath = '../addons/weisrc_icard/upload/';
        ...
        if (copy($filetempname, $uploadfile)) {
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) { //$=2 第二行开始
                ...
                if ($array['ac'] == "olduser") {
                    $count = $count + $this->upload_olduser($row, TIMESTAMP, $array);
                } else if ($array['ac'] == "sncodelist") {
//                    $excount = $this->upload_sncodelist($row, TIMESTAMP, $array);
//                    if ($excount == -1) {
//                        $count = $excount;
//                    } else {
//                        $count = $count + $excount;
//                    }
    ...
    public function doWebUserlog()
    {
        ...
        //总用户数量
        $user_total_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_card) . " WHERE weid = '{$weid}'");
        //今天新增数量
        $user_today_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_card) . " WHERE weid = '{$weid}' AND  date_format(from_UNIXTIME(`dateline`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d')");
        //昨天新增数量
        $user_yesterday_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_card) . " WHERE weid = '{$weid}' AND to_days(now())-to_days(from_UNIXTIME(`dateline`,'%Y-%m-%d %H:%i:%S'))=1");

        //一个月内的会员数据
        $data_user = pdo_fetchall("Select date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d') as date,count(date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d')) as usercount FROM (SELECT * FROM " . tablename($this->table_card) . " where DATE_SUB(CURDATE(), INTERVAL 1 month) <= date(FROM_UNIXTIME(dateline)) AND weid='{$weid}' ) a Group by date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d')");

        //今天消费次数
        $consume_today_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_bill_log) . " WHERE weid = '{$weid}' AND  date_format(from_UNIXTIME(`dateline`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d')");
        //昨天消费次数
        $consume_yesterday_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_bill_log) . " WHERE weid = '{$weid}' AND to_days(now())-to_days(from_UNIXTIME(`dateline`,'%Y-%m-%d %H:%i:%S'))<=1");
        //总消费次数
        $consume_total_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_bill_log) . " WHERE weid = '{$weid}'");
        //一个月内的消费数据
        $data_money = pdo_fetchall("Select date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d') as date,count(date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d')) as moneycount FROM (SELECT * FROM " . tablename($this->table_bill_log) . " where DATE_SUB(CURDATE(), INTERVAL 1 month) <= date(FROM_UNIXTIME(dateline)) AND weid='{$weid}' ) a Group by date_format(FROM_UNIXTIME(dateline),'%Y-%m-%d')");
    ...
    public function doWebCheckedCardstatus()
    {
        ...
        pdo_query("UPDATE " . tablename($this->table_card) . " SET status = abs(:status - 1) WHERE id=:id", array(':status' => $status, ':id' => $cardid));
        message('操作成功!', $url);
    }
    ...
    public function doWebAddCardPrice()
    {
        ...
        $card = pdo_fetch("SELECT * FROM " . tablename("weisrc_icard_card") . " WHERE id=:id LIMIT 1", array(':id' => $cardid));
        if (empty($card)) {
    ...
    public function AddAnnounce($title, $content, $from_user, $type = 0, $levelid = -1)
    {
        ...
        pdo_insert("weisrc_icard_announce", $data_announce);
    }
    ...
    {
        global $_W;
        $rule = pdo_fetch("SELECT id FROM " . tablename('rule') . " WHERE module = 'weisrc_icard' AND weid = '{$_W['uniacid']}' order by id desc");
        if (empty($rule)) {
            header('Location: ' . $_W['siteroot'] . create_url('rule/post', array('module' => 'weisrc_icard')));
            exit;
        } else {
            header('Location: ' . $_W['siteroot'] . create_url('rule/post', array('module' => 'weisrc_icard', 'id' => $rule['id'])));
            exit;
        }
    }

    //会员卡
    function weisrc_icard_style_update($data)
    {
        global $_W;
        $data['updatetime'] = TIMESTAMP;
        return pdo_update($this->table_style, $data, array('weid' => $_W['uniacid']));
    }

    function weisrc_icard_style_check()
    {
        global $_W;
        $card_had = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_style) . " WHERE weid = '{$_W['uniacid']}'");
        if (!$card_had) {
            ...
            pdo_insert($this->table_style, $data);
        }
    }

    //积分策略
    function weisrc_icard_score_insert($data)
    {
        global $_W;
        $data['weid'] = $_W['uniacid'];
        $data['dateline'] = TIMESTAMP;
        $data['updatetime'] = TIMESTAMP;
        return pdo_insert($this->table_score, $data);
    }

    function weisrc_icard_score_update($data)
    {
        global $_W;
        $data['updatetime'] = TIMESTAMP;
        return pdo_update($this->table_score, $data, array('weid' => $_W['uniacid']));
    }

    /*
    ** 设置切换导航
    */
    private $version = '2|7|9|464401';
