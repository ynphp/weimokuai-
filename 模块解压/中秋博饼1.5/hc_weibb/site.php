...
class hc_weibbModuleSite extends WeModuleSite {
	...
	public function doMobileLottery() {
		...
		if(empty($member['nickname'])){
			message("请先完善资料后开始游戏",$this->createMobileUrl('regist',array('id'=>$rid)),'success');
		}
	...
	public function doMobilecreditRank(){
		...
		$rid = intval($_GPC['id']);
		$this->checkBowser();
		$mycredit = pdo_fetchall(" SELECT SUM(d.credit) as credit, m.nickname FROM ".tablename('hc_weibb_dayscredit')." AS d LEFT JOIN ".tablename('hc_weibb_member')." AS m ON d.mid=m.id WHERE d.rid='".$rid."' GROUP BY d.openid  DESC LIMIT 50  ");
	...
	public function addLotteryNum($shareid,$weibb,$rid){
		...
		if(!empty($mid)){
			...
			$cookies = $_W['uniacid']."hc_weibb11".$mid;
			if(!isset($_COOKIE[$cookies])){
