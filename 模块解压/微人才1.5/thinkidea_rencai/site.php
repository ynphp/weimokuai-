...
class Thinkidea_rencaiModuleSite extends WeModuleSite {
	...
	public function __construct(){
		...
		//调自定义分享数据
		$this->SHARE = pdo_fetch("SELECT * FROM ".tablename($this->share_table));
	...
	public function doMobileIndex(){
		...
		//幻灯AD
		$ad_lists = pdo_fetchall("SELECT * FROM ".tablename($this->ads_table)." WHERE weid = :weid AND isshow = 1 ORDER BY display ASC ",array(":weid" => $this->weid));
		...
		//=========================置顶======================
		$time = time();
		$top_lists = pdo_fetchall("SELECT * FROM ".tablename($this->job_table)." WHERE weid = :weid AND istop = 1 AND expiration > {$time}",array(":weid" => $this->weid));
		...
		//=========================最新招聘===========================
        $last_tmp_jobs = pdo_fetchall("SELECT id,mid,title,payroll,welfare FROM ".tablename($this->job_table)." WHERE weid = :weid ORDER BY dateline DESC limit 10 ",array(":weid" => $this->weid));
        //最新招聘背后的企业
        $tmp = $last_jobs = array();
        ...
        unset($tmp);
		//=========================热门职位=========================
		$hot_jobs = pdo_fetchall("SELECT * FROM ".tablename($this->job_table)." WHERE weid = :weid AND ishot = 1 ORDER BY dateline DESC",array(":weid" => $this->weid));
        //热门信息背后的企业
        $tmp = array();
		...
		if(!empty($tmp)){
			$sub_cate = pdo_fetchall("SELECT * FROM ".tablename($this->category_table)." WHERE weid = :weid AND parent_id IN (". implode(",", $tmp) .") AND isshow = 1",array(":weid" => $this->weid));
	...
	public function doMobileJobList(){
		 ...
		 * 最终取栏目id查职位
		 */
		$ret = pdo_fetch("SELECT * FROM ".tablename($this->category_table)." WHERE weid = :weid AND id = :cid LIMIT 1", array(":weid" => $this->weid, ":cid" => $cid));
		$cids_tmp = array();
		if($ret['parent_id'] == 0){		//是父目录，则调取所有子栏目
			$parent_id = $ret['id'];	//父栏目id
			$subs = pdo_fetchall("SELECT * FROM ".tablename($this->category_table)." WHERE weid = :weid AND parent_id = :parent_id", array(":weid" => $this->weid, ":parent_id" => $ret['id']));
			foreach ($subs AS $sub){
				array_push($cids_tmp, $sub['id']);
			}
		}else {
			$parent_id = $ret['parent_id'];		//子栏目的父栏目id
			$cids_tmp = array($ret['id']);
		}
		$cids = implode(",", $cids_tmp);
		//查询条件
		$where = '';
		//*****************************搜索框****************************
		//关键词搜索
		...
		//******************************首页*****************************
		//包吃住
		if(isset($_GPC['welfare'])){
			$query_welfare = intval($_GPC['welfare']);
			$where .= ' AND j.welfare = '.$query_welfare;
		}

		//应届生
		if(isset($_GPC['workexperience'])){
			$query_workexperience = intval($_GPC['workexperience']);
			$where .= ' AND j.workexperience = '.$query_workexperience;
		}
		//****************************列表页****************************
		//薪资
		if(isset($_GPC['payroll'])){
			$query_payroll = intval($_GPC['payroll']);
			$where .= ' AND j.payroll = '.$query_payroll;
		}
		
		//类型
		if(isset($_GPC['positiontype'])){
			$query_positiontype =  intval($_GPC['positiontype']);
			$where .= ' AND j.positiontype = '.$query_positiontype;
		}
		//**************************************************************
		//取所有子栏目
		$sub_cates = pdo_fetchall("SELECT * FROM ".tablename($this->category_table)." WHERE weid = :weid AND parent_id = :cid", array(":weid" => $this->weid,":cid" => $parent_id));
	...
	public function doMobileJobShow(){
        ...
        //评论
        $comments = pdo_fetchall("SELECT * FROM ".tablename($this->jobs_comments_table)." WHERE weid = :weid AND jobid = :jobid AND status = 1 ORDER BY dateline DESC LIMIT 20", array(":weid" => $this->weid, ":jobid" => $job_id));
        $tmp = array();
	...
	*/
	public function doMobilePublicIndex(){
		...
		//检查是个人:2,还是企业:1
		$row = pdo_fetch("SELECT * FROM ".tablename($this->member_table)." WHERE weid = :weid AND from_user = :from_user LIMIT 1", array(":weid" => $this->weid, ":from_user" => $this->from_user));
		if ($row !== false){
			$this->doMobileMy();
		}else{
	...
	*/
    ...
    public function doMobileMyCompanyInfo(){
    	...
    	if(checksubmit('savejobsubmit')){
    		if(false == $id = intval($_GPC['id'])){
    			...
    			if (!empty($_FILES['uploadfile']['name'])){
	    			...
	    			if(intval($error) > 0){
	    				...
	    				if(!is_array($ret)){
	    					//路径存入数据库
	    					$new_filename = date("YmdHis").'_thumb_'.$name;
	    					$data['license'] = $new_filename;
	    				}
    				}
    				//=====================上传营业执照图片处理end==============
    			}
	    	...
	    	//===============================取行业分类=========================
	    	//取父类
	    	$parent_industry = pdo_fetchall("SELECT * FROM ".tablename($this->industry_table)." WHERE parent_id = 0");
	    	$tmp = array();
	    	foreach ($parent_industry AS $parent){
	    		array_push($tmp, $parent['id']);
	    	}
	    	...
	    	if(!empty($pids)){
		    	//取子类
		    	$sub_industry = pdo_fetchall("SELECT * FROM ".tablename($this->industry_table)." WHERE weid = :weid AND parent_id IN ({$pids})", array(":weid" => $this->weid));
		    	foreach ($parent_industry AS $key => $parent){
		    		foreach ($sub_industry AS  $k => $sub){
		    			if($sub['parent_id'] == $parent['id']){
		    				$parent_industry[$key]['sub'][$k] = $sub;
    	...
    	if(checksubmit('save_basic_submit')){
			...
			if(empty($person_id)){
    			...
    			if (pdo_insert($this->person_table, $person_data)){
    				...
    				message('操作失败或表单无变化', 'referer', 'error');
    			}
...
//    		}
    ...
    public function doMobileMy(){
    	...
    	if ($row !== false){
    		...
    		include $this->template('my_index');
    	}
    }
    ...
    public function doMobileAboutUs(){
    	include $this->template('about_us');
    }
	...
	public function doWebDeleteCompanyAjax(){
		global $_W, $_GPC;
		$company_id = intval( $_GPC['company_id'] );
		if(pdo_delete($this->company_table, array('id' => $company_id))){
			exit('1');
		}else{
			exit('0');
		}
    ...
    public function get_companys_info($ids){
        global $_GPC,$_W;
        $tmp_companys = pdo_fetchall("SELECT id,name FROM ".tablename($this->company_table)." WHERE id IN (".implode(',', $ids).")");
        $companys = array();
