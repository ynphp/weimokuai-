...
//		if (SCREEN_SHOW_ALL) k / b > W / H ? k = W * b / H : b = H * k / W, c.style.marginTop = 0;
		if (SCREEN_SHOW_ALL) k / b > W / H ? k = W * b / H : c.style.marginTop = 0;
		else {
			var d = W * b / H;
	...
	a.init = function(c) {
		...
		if('' != param.effectAudioScores && '' != param.effectAudioUrls && null != param.effectAudioScores && null != param.effectAudioUrls){
            ...
            if((effectAudioUrls.length - effectAudioScores.length) == 1){
            	var aduio = new Audio();
            	for(var i = 0; i < effectAudioUrls.length; i++){
            		aduio.src = effectAudioUrls[i];
            		aduio.load();
            	}
            }
        }
	...
	a.showMoreGames = function(b) {
		if (null == e) {
			e = new createjs.Text("\u3010\u66f4\u591a\u5947\u8469\u6e38\u620f\u3011", "28px Arial", "#fff");
			e.textAlign = "right";
...
function MyProgressBar(a, d, b) {
	...
	// b.textAlign = "center";
	// this.addChild(b);
	// b = new createjs.Text("微信公众号: 优友游戏", "38px Arial", "black");
	// b.x = a / 2;
	// b.y = 260;
	// b.textAlign = "center";
	// this.addChild(b);
	// b = new createjs.Text("©2014 优友游戏\nwww.playwx.com", "32px Arial", "black");
	// b.x = a / 2;
	// b.y = d - 100;
	// b.textAlign = "center";
	// this.addChild(b)
