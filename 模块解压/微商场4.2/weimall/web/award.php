<?php
	...
	if ($op == 'post') {
		...
		if (!empty($id)) {
			$sql             = "select * from".tablename('weimall_prize')."where id=".$id;
			$item            = pdo_fetch($sql);
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				pdo_insert('weimall_prize',$data);
				message('添加成功',$this->createWebUrl('award',array('op' => 'display')),'success');
			}else{

				pdo_update('weimall_prize',$data,array('id' => $id));
				message('更新成功',$this->createWebUrl('award',array('op' => 'display')),'success');
			}
		}
	}elseif ($op == 'display') {
		$sql = "select * from".tablename('weimall_prize')."where weid = ".$weid." order by inkind asc, createtime desc";
		$list = pdo_fetchall($sql);
	}elseif ($op == 'delete') {
		pdo_delete('weimall_prize',array('id' => $id));
