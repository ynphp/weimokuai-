<?php
	...
	$ismobile = 'weimallmobile'.$weid;
	$ispwd = 'weimallpwd'.$weid;
	...
	if($op=='display'){
		...
		if(empty($_COOKIE[$ismobile]) && empty($_COOKIE[$ispwd])){
			...
			$host = pdo_fetchall("select id, mobile, pwd from ".tablename('weimall_shophost')." where weid = ".$weid);
			foreach($host as $h){
				if($h['mobile']==$_COOKIE[$ismobile] && $h['pwd']==$_COOKIE[$ispwd]){
					$isreg = 1;
					$shops = pdo_fetchall("select * from ".tablename('weimall_stores')." where weid = ".$weid." and hostid = ".$h['id']);
					$activities = pdo_fetchall("select storeid, count(num) as num, flag from ".tablename('weimall_activity')." where weid = ".$weid." group by storeid, flag");
					$num = array();
	...
	if($op=='reg'){
		$isreg = 0;
		$mobile = pdo_fetchall("select from_user, mobile from ".tablename('weimall_shophost')." where weid = ".$weid);
		foreach($mobile as $m){
		...
		if($isreg == 1){
			...
			pdo_insert('weimall_shophost', $host);
	...
	if($op=='couponlist'){
		...
		$activities = pdo_fetchall("select * from ".tablename('weimall_activity')." where weid = ".$weid." and flag = 0 and storeid = ".$storeid);
	...
	if($op=='couponpost'){
		...
		if($_GPC['opp']=='detail'){
			$id = $_GPC['id'];
			$active = pdo_fetch("select * from ".tablename('weimall_activity')." where weid = ".$weid." and flag = 0 and id = ".$_GPC['id']);
		}
		if(checksubmit('submit')){
			...
			if(intval($_GPC[id])){
				...
				$temp = pdo_update('weimall_activity', $activity, array('id'=>$_GPC['id']));
			} else {
				$temp = pdo_insert('weimall_activity', $activity);
			}
			if($temp){
	...
	if($op=='activitylist'){
		...
		$storeid = $_GPC['storeid'];
		$activities = pdo_fetchall("select * from ".tablename('weimall_activity')." where weid = ".$_W['weid']." and flag = 1 and storeid = ".$storeid);
	...
	if($op=='memberlist'){
		...
		$storeid = $_GPC['storeid'];
		$members = pdo_fetchall("select distinct from_user, realname, mobile from ".tablename('weimall_credit')." where weid = ".$weid." and storeid = ".$_GPC['storeid']." LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
		$m = pdo_fetchall("select distinct from_user, realname, mobile from ".tablename('weimall_credit')." where weid = ".$weid." and storeid = ".$_GPC['storeid']);
		$total = sizeof($m);
	...
	if($op=='build'){
		...
		$categorys = pdo_fetchall("select id, name from ".tablename('weimall_category')." where enabled = 1 and parentid = 0 and weid = ".$weid);
        $category = pdo_fetchall("SELECT id, parentid, name FROM ".tablename('weimall_category')." WHERE weid = ".$weid." ORDER BY parentid ASC, displayorder DESC");
		if (!empty($category)) {
            $child = array();
	...
	if(checksubmit('submit')){
		...
		pdo_insert('weimall_stores', $data);
		$url = $this->createMobileUrl('myshop');
		header("Location: $url");
