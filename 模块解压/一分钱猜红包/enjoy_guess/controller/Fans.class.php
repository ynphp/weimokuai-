...
class Fans {
     ...
     * @param array $entity     用户数据www.tmd9.com
     * @return int|error        成功返回用户编号, 失败返回错误信息
     */
    public function save($entity) {
        global $_W;
        $rec = array_elements(array('openid', 'nickname', 'gender', 'state', 'city', 'country', 'avatar','subscribe','createtime','puid'), $entity);
        $rec['uniacid'] = $_W['uniacid'];
        $sql = 'SELECT * FROM ' . tablename('enjoy_back_fans') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid';
        $pars = array();
        ...
        if(!empty($exists)) {
            ...
            $ret = pdo_update('enjoy_back_fans', $rec, $filter);
            if($ret !== false) {
                return $exists['uid'];
            }else{
        ...
        $ret = pdo_insert('enjoy_back_fans', $rec);
        if(!empty($ret)) {
            return pdo_insertid();
        } else {
    ...
    public function remove($uid, $isOpenid = false) {
        ...
        $del = pdo_delete('enjoy_back_fans', $pars);
        if($del !== false) {
            return true;
        }else{
    ...
    public function getOne($uid, $isOpenid = false) {
        ...
        if($isOpenid){
            ...
            $pars[':uid'] = intval($uid);
            $sql = 'SELECT * FROM ' . tablename('enjoy_back_fans') . ' WHERE `uniacid`=:uniacid AND `uid` =:uid';
        }
    ...
    public function getAll($filters = array(), $pindex = 0, $psize = 15, &$total = 0) {
        ...
        $sql = 'FROM ' . tablename('enjoy_back_fans') . ' AS `f` LEFT JOIN ' . tablename('mbrp_records') . ' AS `r` ON (`f`.`uid`=`r`.`uid`) WHERE ';
        $sql .= $condition;
        if($pindex > 0){
