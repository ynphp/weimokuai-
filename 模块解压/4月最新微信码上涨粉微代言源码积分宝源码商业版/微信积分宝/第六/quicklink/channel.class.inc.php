...
class Channel{
    ...
    public function update($weid, $channel_id, $gpc){
        $bgparam = self :: encode_channel_param($gpc);
        $ret = pdo_update(self :: $t_channel, array('createtime' => time(), 'title' => $gpc['title'], 'thumb' => $gpc['thumb'],'xzdiqu' => $gpc['xzdiqu'], 'bg' => $gpc['bg'], 'bgparam' => $bgparam, 'desc' => $gpc['desc'], 'url' => $gpc['url'], 'click_credit' => $gpc['click_credit'], 'sub_click_credit' => $gpc['sub_click_credit'], 'newbie_credit' => $gpc['newbie_credit']), array('channel' => $channel_id, 'weid' => $weid));
        return $ret;
    }
