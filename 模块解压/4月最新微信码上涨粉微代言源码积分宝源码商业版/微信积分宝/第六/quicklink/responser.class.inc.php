<?php
 class Responser{
    ...
    public function respondText($weid, $from_user, $channel_id, $rule){
       ...
       //限制地区
       $zxdq=array();
       $xzdq[]=$fans['nationality'];  
       $xzdq[]=$fans['resideprovince']; 
       $xzdq[]=$fans['residecity']; 
       $zxdqnews= implode('',$xzdq); 
       if($ch['xzdiqu']){
       $pos = strpos($zxdqnews, $ch['xzdiqu']);
       $xztishi="只限【".$ch['xzdiqu']."】地区的用户能参加活动，请先核对位置!";
       
         if ($pos===false){
            $ret = $weapi -> sendText($from_user,$xztishi);
            exit(0);  
         }
       }
        //结束
