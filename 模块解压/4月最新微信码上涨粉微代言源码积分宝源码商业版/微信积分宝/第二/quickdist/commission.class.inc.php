<?php
 class Commission{
    ...
    public function giveCommission($weid, $orderid, $goodsid, $order_leader, $order_openid, $order_createtime, $price, $rate, $total, $com_level, $com_val, $credit_type){
        $data = array('weid' => $weid, 'orderid' => $orderid, 'goodsid' => $goodsid, 'order_leader' => $order_leader, 'order_openid' => $order_openid, 'order_createtime' => $order_createtime, 'price' => $price, 'rate' => $rate, 'total' => $total, 'level' => $com_level, 'commission_value' => $com_val, 'createtime' => TIMESTAMP);
