...
class MemberOrder{
    ...
    private function getCommissionInfoByLevel($weid, $openid, $level){
        ...
        if (!empty($order_goods)){
            foreach ($order_goods as $item){
                $line = array('nickname' => $item['nickname'], 'from_user' => $item['from_user'], 'price' => $item['price'], 'total' => $item['total'], 'level' => $level, 'status' => $item['status'], 'commission_rate' => $commission_rate[$item['goodsid']]['rate' . $level], 'commission' => $commission_rate[$item['goodsid']]['rate' . $level] * $item['price'] * $item['total']);
                $result[] = $line;
            }
        }
        return $result;
    ...
    public function calcCommission($weid, $orderId){
        ...
        while ($cur_level <= Member :: MAX_LEVEL){
            ...
            foreach ($order_goods as $item){
                $commission[$cur_level]['com_val'] += $commission_rate[$item['goodsid']]['rate' . $cur_level] * $item['price'] * $item['total'];
            }
            $cur_level++;
        }
        return $commission;
    ...
    public function giveCommission($weid, $orderId){
        ...
        while ($cur_level <= Member :: MAX_LEVEL){
            ...
            foreach ($order_goods as $item){
                $com_val = $commission_rate[$item['goodsid']]['rate' . $cur_level] * $item['price'] * $item['total'];
                $_commission -> giveCommission($weid, $orderId, $item['goodsid'], $members[$cur_level]['leader'], $order['from_user'], $order['createtime'], $item['price'], $commission_rate[$item['goodsid']]['rate' . $cur_level], $item['total'], $cur_level, $com_val, $goods[$item['goodsid']]['credittype']);
            }
            $cur_level++;
        }
        if (!empty($order)){
    ...
    public function notifyDealer($order){
        ...
        if (!empty($order_goods)){
            ...
            foreach ($goods as $item){
                $template = '[buyer]购买了新商品，请注意发货. 订单编号:' . $order['ordersn'] . '，商品名:' . $item['title'];
