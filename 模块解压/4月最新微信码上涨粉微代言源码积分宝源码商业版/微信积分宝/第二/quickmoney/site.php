...
class QuickMoneyModuleSite extends WeModuleSite{
    ...
    public function doWebGoods(){
        ...
        if ($operation == 'post'){
            ...
            if (checksubmit('submit')){
                ...
                $data = array('weid' => $_W['weid'], 'title' => $_GPC['title'], 'logo' => $_GPC['logo'], 'deadline' => $_GPC['deadline'], 'exchangetype' => $_GPC['exchangetype'], 'userchangecost' => intval($_GPC['userchangecost']), 'amount' => $amount, 'per_user_limit' => $per_user_limit, 'vip_require' => $vip_require, 'cost' => $cost, 'content' => $_GPC['content'], 'createtime' => TIMESTAMP,);
                if (!empty($goods_id)){
            ...
            $condition = '';
            $list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_goods) . " WHERE weid = '{$_W['weid']}' $condition ORDER BY goods_id DESC");
        }
    ...
    public function doWebRequest(){
        ...
        if ($operation == 'delete'){
            ...
            $sql = "SELECT t1.title, t1.logo, t2.exchangetype, t2.* FROM " . tablename($this -> table_goods) . " as t1," . tablename($this -> table_request) . "as t2 WHERE t1.goods_id=t2.goods_id AND t1.weid = {$_W['weid']} and t2.status = 'done' " . $condition . " ORDER BY t2.createtime DESC";
            $list = pdo_fetchall($sql);
            $ar = pdo_fetchall($sql, array(), 'from_user');
            $fans = fans_search(array_keys($ar), array('realname', 'mobile', 'alipay'));
        }else{
    ...
    public function doMobileGoods(){
        global $_W, $_GPC;
        checkauth();
        $goods_list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_goods) . " WHERE weid = '{$_W['weid']}' and NOW() < deadline and amount > 0 order by createtime");
