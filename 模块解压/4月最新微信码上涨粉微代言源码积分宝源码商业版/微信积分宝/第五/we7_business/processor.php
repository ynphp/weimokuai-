...
//residecity 粉丝表 uid  openid
//国nationality	 省resideprovince	市residecity   县district 
$user=mc_fansinfo($openid);
$useid=$user['uid']; //用户ID


/////获取用户位置名称
$content = file_get_contents("http://api.map.baidu.com/geocoder?location=$lat,$lng&output=xml&key=28bcdd84fae25699606ffad27f8da77b");
$ob= simplexml_load_string($content);
$json  = json_encode($ob);
$configData = json_decode($json, true);

$xzpro=$configData['result']['addressComponent']['province']; //省
$xzcity=$configData['result']['addressComponent']['city'];//用户到所在城市
$xzdist=$configData['result']['addressComponent']['district'];//用户到所在城市

// 用户表ims_mc_members  更新用户所在城市
$fhz=pdo_update('mc_members', array('resideprovince'=>$xzpro,'residecity'=>$xzcity,'residedist'=>$xzdist), array('uid'=>$useid));
$hddqa='你的地址是【'.$xzpro.$xzcity.$xzdist.'】核对成功!';
if($fhz){
  return $this->respText($hddqa);
}else{
  return $this->respText('地址已核对,请不要重复提交!');
}
