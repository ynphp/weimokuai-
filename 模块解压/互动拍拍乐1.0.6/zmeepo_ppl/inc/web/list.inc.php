	  ...
	  $psize = 20;
        ...
        $where = " weid = :weid AND rid = :rid AND actid = :actid";
				...
				foreach($lists as &$row){
						$it = pdo_fetch("SELECT `picurl`,`voteres` FROM".tablename($photo_table)." WHERE openid = :openid AND rid = :rid AND actid = :actid",array(':openid'=>$row['openid'],':rid'=>$id,':actid'=>$actid));
						$row['picurl'] = $it['picurl'];
						$row['voteres'] = $it['voteres'];
				}
				unset($row);
			  ...
			  $pager = pagination($total, $pindex, $psize);
					...
					if(empty($fanid)){
		           message('删除项目不存在',$this->createWebUrl("list",array('id'=>$id,'page'=>$pindex)),"error");
					}
					$fans = pdo_fetch("SELECT * FROM ".tablename($user_table)." WHERE id = :id AND weid = :weid AND rid = :rid",array(':id'=>$fanid,':weid'=>$_W['uniacid'],':rid'=>$id));
					pdo_delete($cj_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					pdo_delete($vote_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					pdo_delete($photo_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					pdo_delete($user_table,array('id'=>$fanid,'rid'=>$id,'weid'=>$_W['uniacid']));
					
					message('删除成功',$this->createWebUrl("list",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"success");
			 }else{
			   message('访问错误');
			 }
				...
				if(checksubmit('delete')){
					...
					$select = $_GPC['select'];
					if(empty($select)){
						message('请选择删除项',$this->createWebUrl("list",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"error");
					}
					foreach ($select as $se) {
						$fans = pdo_fetch("SELECT * FROM ".tablename($user_table)." WHERE id = :id AND weid = :weid AND rid = :rid",array(':id'=>$se,':weid'=>$_W['uniacid'],':rid'=>$id));
					  pdo_delete($cj_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					  pdo_delete($vote_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
						pdo_delete($photo_table,array('openid'=>$fans['openid'],'actid'=>$actid,'rid'=>$id,'weid'=>$_W['uniacid']));
					  pdo_delete($user_table,array('id'=>$se,'rid'=>$id,'weid'=>$_W['uniacid']));
					}
					message('批量删除成功',$this->createWebUrl("list",array('id'=>$id,'actid'=>$actid,'page'=>$pindex)),"success");
				}
				if(checksubmit('upload')){
						...
						foreach($select as $row){
								$sql = "SELECT * FROM ".tablename($user_table)." WHERE rid = :rid AND weid = :weid AND id = :id";
				...
				if(checksubmit('uploadall')){
					$sql = "SELECT * FROM ".tablename($user_table)." WHERE rid = :rid AND weid = :weid";
