				...
				if (empty($command) || $rets) {
					...
					if ($_SESSION['ok'] <> 1) {
						if (!$zjrets && !$rets && !is_numeric($this->message['content'])) {
							...
							return $this->respNews(array('Title' => $row['title'], 'Description' => htmlspecialchars_decode($row['description']), 'PicUrl' => $picture, 'Url' => $this->createMobileUrl('photosvote', array('rid' => $rid)),));
						}
						...
						if ($this->message['content'] != $_SESSION['code']) {
							...
							if ($zjrets) {
								...
								if (!empty($row['limitsd']) && !empty($row['limitsdps'])) {
									...
									$limitsdvote = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename($this->table_log) . ' WHERE uniacid= :uniacid AND from_user = :from_user AND rid = :rid ' . $tptimes . ' ORDER BY createtime DESC', array(':uniacid' => $uniacid, ':from_user' => $from_user, ':rid' => $rid));
								...
								if (!empty($t['limitsd'])) {
									...
									$limitsdvotegr = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename($this->table_log) . ' WHERE uniacid= :uniacid AND tfrom_user = :tfrom_user AND rid = :rid ' . $tptimesgr . ' ORDER BY createtime DESC', array(':uniacid' => $uniacid, ':tfrom_user' => $tfrom_user, ':rid' => $rid));
									if ($t['limitsd'] > 0) {
							...
							if ($rets) {
								...
								$times .= ' AND createtime <=' . $endtime;
								$daytpxz = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_log) . ' WHERE uniacid= :uniacid AND from_user = :from_user AND rid = :rid ' . $times . ' ORDER BY createtime DESC', array(':uniacid' => $uniacid, ':from_user' => $from_user, ':rid' => $rid));
								if (is_numeric($word)) $where .= " AND uid = '" . $word . "'"; else $where .= " AND realname = '" . $word . "'";
								$t = pdo_fetch("SELECT * FROM " . tablename($this->table_users) . " WHERE uniacid = :uniacid and rid = :rid " . $where . " LIMIT 1", array(':uniacid' => $uniacid, ':rid' => $rid));
								if (empty($t)) {
					...
					switch ($this->message['type']) {
						case 'text':
							...
							$data = array('rid' => $rid, 'uid' => $uuid['uid'] + 1, 'uniacid' => $uniacid, 'from_user' => $from_user, 'avatar' => $avatar, 'nickname' => $nickname, 'music' => $_SESSION["voiceurl"], 'voice' => $_SESSION["voiceurl"], 'vedio' => $_SESSION["videourl"], 'description' => $_SESSION["description"], 'photoname' => $_SESSION["photoname"], 'realname' => $_SESSION["realname"], 'mobile' => $_SESSION["mobile"], 'photosnum' => '0', 'xnphotosnum' => '0', 'hits' => '1', 'xnhits' => '1', 'yaoqingnum' => '0', 'status' => $row['tpsh'] == 1 ? '0' : '1', 'createip' => getip(), 'lastip' => getip(), 'lasttime' => time(), 'sharetime' => time(), 'createtime' => time());
							$data['iparr'] = $this->getiparr($data['lastip']);
							pdo_insert($this->table_users, $data);
							for ($i = 1; $i <= $row['tpxz']; $i++) {
								if (!empty($_SESSION['imagesurl' . $i])) {
									$insertdata = array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'photoname' => $_SESSION['nfilename' . $i], 'photos' => $_SESSION['imagesurl' . $i], 'status' => 1, 'isfm' => 0, 'createtime' => $now,);
