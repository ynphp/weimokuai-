...
include 'model.php';
require_once IA_ROOT . '/addons/hc_moreshop/responser.php';

class hc_moreshopModuleSite extends WeModuleSite {
	//用于异步执行二维码图片合成程序
	public function doMobileRunTask() {
		global $_W, $_GPC;
		WeUtility::logging(" jin ru runtask", "sdfsdfsdf");
		ignore_user_abort(true);
		$uniacid = $_W['uniacid'];
		$openid = $_GPC['from_user'];
		$qr = new QRResponser($this->app, $this->sec);
		//$qr->respondText($_GPC['from_user']);
		$member = pdo_fetch("select * from ".tablename('hc_moreshop_member')." where weid = ".$uniacid." and from_user = '".$openid."'");
		$id = $member['id'];
	...
	public function doMobileSetGoodsProperty() {
		global $_GPC, $_W;
		$id = intval($_GPC['id']);
		$type = $_GPC['type'];
		...
		if (in_array($type, array('new', 'hot', 'recommand', 'discount'))) {
			$data = ($data==1?'0':'1');
			pdo_update("hc_moreshop_goods", array("is" . $type => $data), array("id" => $id, "weid" => $_W['uniacid']));
			die(json_encode(array("result" => 1, "data" => $data)));
		}
		if (in_array($type, array('status'))) {
			$data = ($data==1?'0':'1');
			pdo_update("hc_moreshop_goods", array($type => $data), array("id" => $id, "weid" => $_W['uniacid']));
			die(json_encode(array("result" => 1, "data" => $data)));
		}
    ...
    public function doWebGoods() {
        global $_GPC, $_W;
        load()->func('tpl');
        $category = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_category') . " WHERE weid = '{$_W['uniacid']}' ORDER BY parentid ASC, displayorder DESC", array(), 'id');
		if (!empty($category)) {
            $children = '';
        ...
        if ($operation == 'post') {
            ...
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE id = :id", array(':id' => $id));
                if (empty($item)) {
                    message('抱歉，商品不存在或是已经删除！', '', 'error');
                }
                $allspecs = pdo_fetchall("select * from " . tablename('hc_moreshop_spec')." where goodsid=:id order by displayorder asc",array(":id"=>$id));
                foreach ($allspecs as &$s) {
                    $s['items'] = pdo_fetchall("select * from " . tablename('hc_moreshop_spec_item') . " where specid=:specid order by displayorder asc", array(":specid" => $s['id']));
                }
                unset($s);

                $params = pdo_fetchall("select * from " . tablename('hc_moreshop_goods_param') . " where goodsid=:id order by displayorder asc", array(':id' => $id));
                $piclist = unserialize($item['thumb_url']);
                //处理规格项
                $html = "";
                $options = pdo_fetchall("select * from " . tablename('hc_moreshop_goods_option') . " where goodsid=:id order by id asc", array(':id' => $id));
            ...
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
                    pdo_insert('hc_moreshop_goods', $data);
                    $id = pdo_insertid();
                } else {
                    unset($data['createtime']);
                    pdo_update('hc_moreshop_goods', $data, array('id' => $id));
                }
                ...
                for ($k = 0; $k < $len; $k++) {
					...
					if (!is_numeric($get_param_id)) {
						pdo_insert("hc_moreshop_goods_param", $a);
						$param_id = pdo_insertid();
					} else {
						pdo_update("hc_moreshop_goods_param", $a, array('id' => $get_param_id));
						$param_id = $get_param_id;
					}
					$paramids[] = $param_id;
				}
                if (count($paramids) > 0) {
                    pdo_query("delete from " . tablename('hc_moreshop_goods_param') . " where goodsid=$id and id not in ( " . implode(',', $paramids) . ")");
                }
                else{
                    pdo_query("delete from " . tablename('hc_moreshop_goods_param') . " where goodsid=$id");
                }
//                if ($totalstocks > 0) {
//                    pdo_update("hc_moreshop_goods", array("total" => $totalstocks), array("id" => $id));
//                }
                //处理商品规格
                $files = $_FILES;
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    if (is_numeric($get_spec_id)) {
                        ...
                        pdo_update("hc_moreshop_spec", $a, array("id" => $get_spec_id));
                        $spec_id = $get_spec_id;
                    } else {
                        pdo_insert("hc_moreshop_spec", $a);
                    ...
                    for ($n = 0; $n < $itemlen; $n++) {
                        ...
                        if (is_numeric($get_item_id)) {
                            pdo_update("hc_moreshop_spec_item", $d, array("id" => $get_item_id));
                            $item_id = $get_item_id;
                        } else {
                            pdo_insert("hc_moreshop_spec_item", $d);
                    ...
                    //删除其他的
                    if(count($itemids)>0){
                         pdo_query("delete from " . tablename('hc_moreshop_spec_item') . " where weid={$_W['uniacid']} and specid=$spec_id and id not in (" . implode(",", $itemids) . ")");    
                    }
                    else{
                         pdo_query("delete from " . tablename('hc_moreshop_spec_item') . " where weid={$_W['uniacid']} and specid=$spec_id");    
                    }
                    
                    //更新规格项id
                    pdo_update("hc_moreshop_spec", array("content" => serialize($itemids)), array("id" => $spec_id));
                ...
                //删除其他的
                if( count($specids)>0){
                	pdo_query("delete from " . tablename('hc_moreshop_spec') . " where weid={$_W['uniacid']} and goodsid=$id and id not in (" . implode(",", $specids) . ")");
                }
                else{
                    pdo_query("delete from " . tablename('hc_moreshop_spec') . " where weid={$_W['uniacid']} and goodsid=$id");
                }
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    if (empty($get_option_id)) {
                        pdo_insert("hc_moreshop_goods_option", $a);
                        $option_id = pdo_insertid();
                    } else {
                        pdo_update("hc_moreshop_goods_option", $a, array('id' => $get_option_id));
                        $option_id = $get_option_id;
                    }
                    $optionids[] = $option_id;
                }
                if (count($optionids) > 0) {
                    pdo_query("delete from " . tablename('hc_moreshop_goods_option') . " where goodsid=$id and id not in ( " . implode(',', $optionids) . ")");
                }
                else{
                    pdo_query("delete from " . tablename('hc_moreshop_goods_option') . " where goodsid=$id");
                }
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE weid = '{$_W['uniacid']}' and deleted=0 $condition ORDER BY status DESC, displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('hc_moreshop_goods') . " WHERE weid = '{$_W['uniacid']}'  and deleted=0 $condition");
            $pager = pagination($total, $pindex, $psize);
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id, thumb FROM " . tablename('hc_moreshop_goods') . " WHERE id = :id", array(':id' => $id));
            if (empty($row)) {
...
//            }
//            pdo_delete('hc_moreshop_goods', array('id' => $id));
            //修改成不直接删除，而设置deleted=1
            pdo_update("hc_moreshop_goods", array("deleted" => 1), array('id' => $id));

            message('删除成功！', referer(), 'success');
        } elseif ($operation == 'productdelete') {
            $id = intval($_GPC['id']);
            pdo_delete('hc_moreshop_product', array('id' => $id));
    ...
    public function doWebOrder() {
        ...
        if ($operation == 'display') {
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE weid = '{$_W['uniacid']}' $condition ORDER BY status ASC, createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('hc_moreshop_order') . " WHERE weid = '{$_W['uniacid']}' $condition");
            $pager = pagination($total, $pindex, $psize);
			if(!empty($shareid)){
				if (!empty($list)) {
			...
			$id = intval($_GPC['id']);
			$memberrelative = pdo_fetchall("select weid, commission, shareid from ".tablename('hc_moreshop_memberrelative')." where weid = ".$_W['uniacid']." and orderid = ".$id);
            ...
            if (checksubmit('confirmsend')) {
               ...
               // $item = pdo_fetch("SELECT transid FROM " . tablename('hc_moreshop_order') . " WHERE id = :id", array(':id' => $id));
				...
				$issendGoodsSend = pdo_fetch('SELECT id, issendGoodsSend FROM '.tablename('hc_moreshop_membernews')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $order['weid'],':from_user' => $order['from_user']));
				if(empty($issendGoodsSend) || $issendGoodsSend['issendGoodsSend']==1){
            ...
            if (checksubmit('finish')) {
				...
				if(!empty($m['commission'])){
					$issendCommWarm = pdo_fetch('SELECT id, issendCommWarm FROM '.tablename('hc_moreshop_membernews')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $m['weid'],':from_user' => $from_user[$m['shareid']]));
					if(empty($issendCommWarm) || $issendCommWarm['issendCommWarm']==1){
...
//            }
    ...
    private function setOrderStock($id = '', $minus = true) {
        ...
        $goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,g.total as goodstotal,o.total,o.optionid,g.sales FROM " . tablename('hc_moreshop_order_goods') . " o left join " . tablename('hc_moreshop_goods') . " g on o.goodsid=g.id "
                . " WHERE o.orderid='{$id}'");
        foreach ($goods as $item) {
            if ($minus) {
                //属性
                if (!empty($item['optionid'])) {
                    pdo_query("update " . tablename('hc_moreshop_goods_option') . " set stock=stock-:stock where id=:id", array(":stock" => $item['total'], ":id" => $item['optionid']));
                }
                $data = array();
                ...
                pdo_update('hc_moreshop_goods', $data, array('id' => $item['id']));
            } else {
                //属性
                if (!empty($item['optionid'])) {
                    pdo_query("update " . tablename('hc_moreshop_goods_option') . " set stock=stock+:stock where id=:id", array(":stock" => $item['total'], ":id" => $item['optionid']));
    ...
    public function doWebNotice() {
        ...
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('hc_moreshop_feedback') . $where, $paras);
        $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_feedback') . $where . " ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $paras);
        $pager = pagination($total, $pindex, $psize);
        ...
        if (!empty($transids)) {
            $sql = "SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE weid='{$_W['uniacid']}' AND transid IN ( '" . implode("','", $transids) . "' )";
        ...
        $addresses = array();
        if (!empty($addressids)) {
            $sql = "SELECT * FROM " . tablename('hc_moreshop_address') . " WHERE weid='{$_W['uniacid']}' AND id IN ( '" . implode("','", $addressids) . "' )";
    ...
    public function getCartTotal() {
        global $_W;
        $cartotal = pdo_fetchcolumn("select sum(total) from " . tablename('hc_moreshop_cart') . " where weid = '{$_W['uniacid']}' and from_user='{$_W['openid']}'");
	...
	// 开店
	public function doMobileMoreShop(){
		$this->__mobile(__FUNCTION__);
	}
	
	// 我的分红
	public function doMobileMyGrowMoney(){
		$this->__mobile(__FUNCTION__);
	}
	
	// 我推荐的订单
	public function doMobileMyRecOrder(){
		$this->__mobile(__FUNCTION__);
	}
	
	// 我的下属
	public function doMobileMyLows(){
		$this->__mobile(__FUNCTION__);
	}
	
	// 开店编辑
	public function doMobileOpenEdit(){
		$this->__mobile(__FUNCTION__);
	}
	...
	// 消息设置
	public function doMobileSetnews(){
		$this->__mobile(__FUNCTION__);
	}
	...
	// 余额提现
	public function doMobileMoneyApply(){
		$this->__mobile(__FUNCTION__);
	}
	
	// 翻翻看
	public function doMobileFangfangkan(){
		$this->__mobile(__FUNCTION__);
	}
...
//-----------------------------------web端
	...
	// 店家管理
	public function doWebShopHolder(){
		$this->__web(__FUNCTION__);
	}
	...
	// 余额提现
	public function doWebMoneyApply(){
		$this->__web(__FUNCTION__);
	}
    ...
    public function doMobileindex() {
        ...
        if($op=='display'){
			...
			$rule = pdo_fetch("select * from ".tablename('hc_moreshop_rules')." where weid = ".$_W['uniacid']);
			if(empty($_COOKIE[$shareid]) || (($_GPC['mid']!=$_COOKIE[$shareid]) && !empty($_GPC['mid']))){
				setcookie("$shareid", $_GPC['mid'], time()+3600*24*$day_cookies);
			}
			$mid = intval($_GPC['mid']) ? intval($_GPC['mid']) : $_COOKIE[$shareid];
			$member = pdo_fetch("SELECT id, realname, shareid, headimg FROM ".tablename('hc_moreshop_member')." WHERE from_user = '".$_W['openid']."' AND weid = ".$_W['uniacid']);
			if(!empty($member['shareid'])){
				$highmember = pdo_fetch("SELECT realname, headimg, realname FROM ".tablename('hc_moreshop_member')." WHERE id = ".$member['shareid']);
			} else {
				if(intval($mid)){
					$highmember = pdo_fetch("SELECT realname, headimg FROM ".tablename('hc_moreshop_member')." WHERE id = ".$mid);
				}
			}
			$shopflag = 'hc_moreshop_shopfalg';
			$shopflag = $_COOKIE[$shopflag];
			$shophost = pdo_fetch("SELECT * FROM ".tablename('hc_moreshop_shophost')." WHERE ischeck = 1 and weid = ".$_W['uniacid']." and from_user = '".$_W['openid']."'");
			if(!empty($shophost)){
				if($shopflag==0){
					$shopname = '商城首页';
					$isopen = " hid = 0 and ";
					...
					$shopname = '我的小店';
					$isopen = " (hid = 0 or hid = ".$shophost['id'].") and ";
				}
			} else {
				if(intval($mid)){
					$shophost = pdo_fetch("SELECT realname FROM ".tablename('hc_moreshop_shophost')." WHERE weid = ".$_W['uniacid']." and mid = ".$mid);
					if(!empty($shophost)){
						$shopname = $shophost['realname'].'的小店';
						$shopflag = 1;
						$isopen = " (hid = 0 or hid = ".$mid.") and ";
					} else {
						$shopname = '商城首页';
						$isopen = " hid = 0 and ";
					}
			...
			//幻灯片
			$advs = pdo_fetchall("select * from " . tablename('hc_moreshop_adv') . " where enabled=1 and weid= '{$_W['uniacid']}'  order by displayorder asc");
			foreach ($advs as &$adv) {
			...
			unset($adv);
			$rlist = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition ORDER BY displayorder DESC, sales DESC limit ".$psize);
			$total = pdo_fetchcolumn("SELECT count(id) FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition");
		}
		
		if($op=='loadmore'){
			$shopflag = intval($_GPC['shopflag']);
			...
			if($shopflag==0){
				...
				$shophost = pdo_fetch("SELECT * FROM ".tablename('hc_moreshop_shophost')." WHERE ischeck = 1 and weid = ".$_W['uniacid']." and from_user = '".$_W['openid']."'");
				$mid = !empty($shophost) ? $shophost['id'] : intval($_COOKIE[$shareid]);
				$isopen = " (hid = 0 or hid = ".$mid.") and ";
			}
			$rlist = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}' and deleted=0 AND status = '1' $condition ORDER BY displayorder DESC, sales DESC limit ".$psize1.",".$psize);
			if(!empty($rlist)){
				$json['psize'] = $psize1;
    ...
    public function doMobilelistmore_rec() {
        ...
        $psize = 6;
        $condition = ' and isrecommand=1 ';
        $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition ORDER BY displayorder DESC, sales DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
    ...
    public function doMobilelistmore() {
        ...
        if (!empty($_GPC['ccate'])) {
            ...
            $condition .= " AND ccate = '{$cid}'";
            $_GPC['pcate'] = pdo_fetchcolumn("SELECT parentid FROM " . tablename('hc_moreshop_category') . " WHERE id = :id", array(':id' => intval($_GPC['ccate'])));
        } elseif (!empty($_GPC['pcate'])) {
            $cid = intval($_GPC['pcate']);
            $condition .= " AND pcate = '{$cid}'";
        }
        $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE weid = '{$_W['uniacid']}' AND status = '1' $condition ORDER BY displayorder DESC, sales DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
        include $this->template('list_more');
    }
   ...
   public function doMobilelist2() {
        ...
        if($op=='display'){
			...
			$rule = pdo_fetch("select * from ".tablename('hc_moreshop_rules')." where weid = ".$_W['uniacid']);
			$member = pdo_fetch("SELECT id FROM ".tablename('hc_moreshop_member')." WHERE from_user = '".$_W['openid']."' AND weid = ".$_W['uniacid']);
			$urlcookie = "hc_moreshop_url".$_W['uniacid'];
			if (empty($member)) {
			...
			$list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition ORDER BY displayorder desc limit ".$psize);
			$total = pdo_fetchcolumn("SELECT count(id) FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition");
        }
		if($op=='loadmore'){
			$shopflag = intval($_GPC['shopflag']);
			...
			$list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE ".$isopen." weid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition ORDER BY displayorder desc limit ".$psize1.",".$psize);
			if(!empty($list)){
				$json['psize'] = $psize1;
				foreach($list as $key=>$l){
	...
	public function doMobilelistCategory() {
		...
		$category = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_category') . " WHERE weid = '{$_W['uniacid']}' and enabled=1 ORDER BY parentid ASC, displayorder DESC", array(), 'id');
		//$children = array();
    ...
    public function doMobileMyCart() {
        ...
        if ($op == 'add') {
			...
			$profile = pdo_fetch('SELECT * FROM '.tablename('hc_moreshop_member')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $_W['uniacid'],':from_user' => $_W['openid']));
            if(empty($profile)){
				$res['follow']= -2;
				die(json_encode($res));
				exit;
            ...
            $goods = pdo_fetch("SELECT id, type, total,marketprice,maxbuy FROM " . tablename('hc_moreshop_goods') . " WHERE id = :id", array(':id' => $goodsid));
            if (empty($goods)) {
            ...
            if (!empty($optionid)) {
                $option = pdo_fetch("select marketprice from " . tablename('hc_moreshop_goods_option') . " where id=:id limit 1", array(":id" => $optionid));
                if (!empty($option)) {
                    $marketprice = $option['marketprice'];
                }
            }

            $row = pdo_fetch("SELECT id, total FROM " . tablename('hc_moreshop_cart') . " WHERE from_user = :from_user AND weid = '{$_W['uniacid']}' AND goodsid = :goodsid  and optionid=:optionid", array(':from_user' => $_W['openid'], ':goodsid' => $goodsid,':optionid'=>$optionid));
            if ($row == false) {
                //不存在
                $data = array(
                    'weid' => $_W['uniacid'],
                ...
                pdo_insert('hc_moreshop_cart', $data);
            } else {
                //累加最多限制购买数量
                $t = $total + $row['total'];
                ...
                pdo_update('hc_moreshop_cart', $data, array('id' => $row['id']));
            }
		...
		$this->checkAuth();
		if ($op == 'clear') {
            pdo_delete('hc_moreshop_cart', array('from_user' => $_W['openid'], 'weid' => $_W['uniacid']));
            die(json_encode(array("result" => 1)));
        } else if ($op == 'remove') {
            $id = intval($_GPC['id']);
            pdo_delete('hc_moreshop_cart', array('from_user' => $_W['openid'], 'weid' => $_W['uniacid'], 'id' => $id));
            die(json_encode(array("result" => 1, "cartid" => $id)));
        } else if ($op == 'update') {
            $id = intval($_GPC['id']);
            $num = intval($_GPC['num']);
            $sql = "update " . tablename('hc_moreshop_cart') . " set total=$num where id=:id";
            pdo_query($sql, array(":id" => $id));
            die(json_encode(array("result" => 1)));
        } else {
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_cart') . " WHERE  weid = '{$_W['uniacid']}' AND from_user = '{$_W['openid']}'");
            $totalprice = 0;
            if (!empty($list)) {
                foreach ($list as &$item) {
                    $goods = pdo_fetch("SELECT  title, thumb, marketprice, unit, total,maxbuy FROM " . tablename('hc_moreshop_goods') . " WHERE id=:id limit 1", array(":id" => $item['goodsid']));
                    //属性
                    $option = pdo_fetch("select title,marketprice,stock from " . tablename("hc_moreshop_goods_option") . " where id=:id limit 1", array(":id" => $item['optionid']));
                    if ($option) {
    ...
    public function setOrderCredit($orderid, $add = true) {
        global $_W;
		$order = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE id = :id limit 1", array(':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $ordergoods = pdo_fetchall("SELECT goodsid, total FROM " . tablename('hc_moreshop_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');
        $ordergood = array();
		foreach($ordergoods as $o){
			$ordergood[$o['goodsid']] = $o['total'];
		}
		if (!empty($ordergoods)) {
            $goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total,credit FROM " . tablename('hc_moreshop_goods') . " WHERE id IN ('" . implode("','", array_keys($ordergoods)) . "')");
        }
        //增加积分
        if (!empty($goods)) {
	...
	//首页查询功能
		...
		$keyword = $_GPC['keyword'];
		$url = $_W['siteroot'].$this->createMobileUrl('list2', array('name' =>'hc_moreshop','weid'=>$_W['uniacid'], 'keyword'=>$keyword, 'sort'=>1));
		header("location:$url");
        include $this->template('list2');
    }
    ...
    public function doMobilePay() {
        ...
        $order = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE id = :id", array(':id' => $orderid));
        if ($order['status'] != '0') {
            message('抱歉，您的订单已经付款或是被关闭，请重新进入付款！', $this->createMobileUrl('myorder'), 'error');
        }
        if (checksubmit('codsubmit')) {

            $ordergoods = pdo_fetchall("SELECT goodsid, total,optionid FROM " . tablename('hc_moreshop_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');
            if (!empty($ordergoods)) {
                $goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total,credit FROM " . tablename('hc_moreshop_goods') . " WHERE id IN ('" . implode("','", array_keys($ordergoods)) . "')");
            }
			//邮件提醒
			if (!empty($this->module['config']['noticeemail'])) {
				$address = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_address') . " WHERE id = :id", array(':id' => $order['addressid']));
				$body = "<h3>购买商品清单</h3> <br />";
				if (!empty($goods)) {
					foreach ($goods as $row) {
						//属性
						$option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("hc_moreshop_goods_option") . " where id=:id limit 1", array(":id" => $ordergoods[$row['id']]['optionid']));
						if ($option) {
            ...
            pdo_update('hc_moreshop_order', array('status' => '1', 'paytype' => '3'), array('id' => $orderid));
            //增加积分
            $this->setOrderCredit($orderid);
    ...
    public function doMobileMyOrder() {
        ...
        if ($op == 'confirm') {
            $orderid = intval($_GPC['orderid']);
            $order = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE id = :id AND from_user = :from_user", array(':id' => $orderid, ':from_user' => $_W['openid']));
            if (empty($order)) {
                message('抱歉，您的订单不存在或是已经被取消！', $this->createMobileUrl('myorder'), 'error');
            }
            pdo_update('hc_moreshop_order', array('status' => 3), array('id' => $orderid, 'from_user' => $_W['openid']));
			pdo_update('hc_moreshop_memberrelative', array('flag'=>3), array('orderid'=>$orderid, 'weid'=>$_W['uniacid']));
			pdo_update('hc_moreshop_credit', array('status'=>0), array('orderid'=>$id, 'weid'=>$_W['uniacid']));
			$memberrelative = pdo_fetchall("select weid, commission, shareid from ".tablename('hc_moreshop_memberrelative')." where weid = ".$_W['uniacid']." and orderid = ".$orderid);
			$members = pdo_fetchall("select id, realname, from_user from ".tablename('hc_moreshop_member')." where weid = ".$_W['uniacid']." and status = 1");
			$member = array();
			foreach($members as $m){
				$from_user[$m['id']] = $m['from_user'];
			...
			$orderid = intval($_GPC['orderid']);
			pdo_update('hc_moreshop_order', array('status' => -1), array('id' => $orderid, 'from_user' => $_W['openid']));
			message('订单取消成功！', $this->createMobileUrl('myorder'), 'success');
		} else if ($op=="recover"){
			$orderid = intval($_GPC['orderid']);
			pdo_update('hc_moreshop_order', array('status' => 0), array('id' => $orderid, 'from_user' => $_W['openid']));
			message('订单恢复成功！', $this->createMobileUrl('myorder'), 'success');
		} else if ($op=="refund"){
			$orderid = intval($_GPC['orderid']);
			if($_GPC['opp']=='refund'){
				pdo_update('hc_moreshop_order', array('status' => -2, 'refundreason'=>trim($_GPC['refundreason']), 'refundtime'=>time()), array('id' => $orderid, 'from_user' => $_W['openid']));
				$order = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_order')." WHERE id = ".$orderid);
				$ordergoods = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order_goods')." WHERE weid = ".$_W['uniacid']." and orderid = ".$orderid);
				$goods = pdo_fetchall("SELECT id, title FROM " . tablename('hc_moreshop_goods')." WHERE weid = ".$_W['uniacid']);
				$good = array();
				foreach($goods as $g){
					$good[$g['id']] = $g['title'];
				}
				...
				$title = trim($title, ',');
				$issendApplyMoneyBack = pdo_fetch('SELECT id, issendApplyMoneyBack FROM '.tablename('hc_moreshop_membernews')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $order['weid'],':from_user' => $order['from_user']));
				if(empty($issendApplyMoneyBack) || $issendApplyMoneyBack['issendApplyMoneyBack']==1){
            ...
            $orderid = intval($_GPC['orderid']);
            $item = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE weid = '{$_W['uniacid']}' AND from_user = '{$_W['openid']}' and id='{$orderid}' limit 1");
            if (empty($item)) {
                message('抱歉，您的订单不存或是已经被取消！', $this->createMobileUrl('myorder'), 'error');
            }
            $goodsid = pdo_fetchall("SELECT goodsid,total FROM " . tablename('hc_moreshop_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');

            $goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,o.total,o.optionid FROM " . tablename('hc_moreshop_order_goods') . " o left join " . tablename('hc_moreshop_goods') . " g on o.goodsid=g.id "
            ...
            $dispatch = pdo_fetch("select id,dispatchname from " . tablename('hc_moreshop_dispatch') . " where id=:id limit 1", array(":id" => $item['dispatch']));
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(), 'id');
            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('hc_moreshop_order') . " WHERE weid = '{$_W['uniacid']}' AND from_user = '{$_W['openid']}'");
            $pager = pagination($total, $pindex, $psize);
            ...
            if (!empty($list)) {
                foreach ($list as &$row) {
                    ...
                    foreach ($goods as &$item) {
                        //属性
                        $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("hc_moreshop_goods_option") . " where id=:id limit 1", array(":id" => $item['optionid']));
                        if ($option) {
                    ...
                    $row['goods'] = $goods;
                    $row['total'] = $goodsid;
                    $row['dispatch'] = pdo_fetch("select id,dispatchname from " . tablename('hc_moreshop_dispatch') . " where id=:id limit 1", array(":id" => $row['dispatch']));
                }
            }
    ...
    public function doMobileDetail() {
        ...
        $goodsid = intval($_GPC['id']);
        $goods = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_goods') . " WHERE id = :id", array(':id' => $goodsid));
		if(empty($goods['commission'])){
			$goods['commission'] = $rule['globalCommission'];
		}
		$ccate = intval($goods['ccate']);
		$member = pdo_fetch( " SELECT * FROM ".tablename('hc_moreshop_member')." WHERE from_user='".$_W['openid']."' AND weid=".$_W['uniacid']." " );
		$id = intval($member['id']) ? intval($member['id']) : 0;
        if (empty($goods)) {
            message('抱歉，商品不存在或是已经被删除！');
        }
        ...
        //浏览量
        pdo_query("update " . tablename('hc_moreshop_goods') . " set viewcount=viewcount+1 where id=:id and weid='{$_W['uniacid']}' ", array(":id" => $goodsid));
        ...
        //规格及规格项
           $allspecs = pdo_fetchall("select * from " . tablename('hc_moreshop_spec') . " where goodsid=:id order by displayorder asc", array(':id' => $goodsid));
           foreach ($allspecs as &$s) {
                 $s['items'] = pdo_fetchall("select * from " . tablename('hc_moreshop_spec_item') . " where  `show`=1 and specid=:specid order by displayorder asc", array(":specid" => $s['id']));
           }
           unset($s);
			
           //处理规格项
           $options = pdo_fetchall("select id,title,thumb,marketprice,productprice,costprice, stock,weight,specs from " . tablename('hc_moreshop_goods_option') . " where goodsid=:id order by id asc", array(':id' => $goodsid));
        ...
        if (!empty($goods['hasoption'])) {
            $options = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods_option') . " WHERE goodsid=:goodsid order by thumb asc,displayorder asc", array(":goodsid" => $goods['id']));
            foreach ($options as $o) {
        ...
        $params = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_goods_param') . " WHERE goodsid=:goodsid order by displayorder asc", array(":goodsid" => $goods['id']));
		$carttotal = $this->getCartTotal();
    ...
    public function doMobileAddress() {
        ...
        if ($operation == 'post') {
            ...
            if (!empty($id)) {
                unset($data['weid']);
                unset($data['openid']);
                pdo_update('hc_moreshop_address', $data, array('id' => $id));
                message($id, '', 'ajax');
            } else {
                pdo_update('hc_moreshop_address', array('isdefault' => 0), array('weid' => $_W['uniacid'], 'openid' => $_W['openid']));
                $data['isdefault'] = 1;
                pdo_insert('hc_moreshop_address', $data);
                $id = pdo_insertid();
                if (!empty($id)) {
                    message($id, '', 'ajax');
                } else {
            ...
            $id = intval($_GPC['id']);
            pdo_update('hc_moreshop_address', array('isdefault' => 0), array('weid' => $_W['uniacid'], 'openid' => $_W['openid']));
            pdo_update('hc_moreshop_address', array('isdefault' => 1), array('id' => $id));
            message(1, '', 'ajax');
        } elseif ($operation == 'detail') {
            $id = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id, realname, mobile, province, city, area, address FROM " . tablename('hc_moreshop_address') . " WHERE id = :id", array(':id' => $id));
            message($row, '', 'ajax');
        } elseif ($operation == 'remove') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $address = pdo_fetch("select isdefault from " . tablename('hc_moreshop_address') . " where id='{$id}' and weid='{$_W['uniacid']}' and openid='{$_W['openid']}' limit 1 ");

                if (!empty($address)) {
                    //pdo_delete("hc_moreshop_address",  array('id'=>$id, 'weid' => $_W['uniacid'], 'openid' => $_W['openid']));
                    //修改成不直接删除，而设置deleted=1
                    pdo_update("hc_moreshop_address", array("deleted" => 1, "isdefault" => 0), array('id' => $id, 'weid' => $_W['uniacid'], 'openid' => $_W['openid']));

                    if ($address['isdefault'] == 1) {
                        //如果删除的是默认地址，则设置是新的为默认地址
                        $maxid = pdo_fetchcolumn("select max(id) as maxid from " . tablename('hc_moreshop_address') . " where weid='{$_W['uniacid']}' and openid='{$_W['openid']}' limit 1 ");
                        if (!empty($maxid)) {
                            pdo_update('hc_moreshop_address', array('isdefault' => 1), array('id' => $maxid, 'weid' => $_W['uniacid'], 'openid' => $_W['openid']));
            ...
            $address = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_address') . " WHERE deleted=0 and openid = :openid", array(':openid' => $_W['openid']));
            $carttotal = $this->getCartTotal();
    ...
    private function checkAuth() {
        global $_W;
        $profile = pdo_fetch('SELECT * FROM '.tablename('hc_moreshop_member')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $_W['uniacid'],':from_user' => $_W['openid']));
		$gzurl = pdo_fetchcolumn("select gzurl from ".tablename('hc_moreshop_rules')." where weid = ".$_W['uniacid']);
		$follow = pdo_fetch("select uid, follow from ".tablename('mc_mapping_fans')." where uniacid = ".$_W['uniacid']." and openid = '".$_W['openid']."'");
		if(empty($follow) || $follow['follow']==0){
    ...
    public function payResult($params) {
        ...
        pdo_update('hc_moreshop_order', $data, array('id' => $params['tid']));
        if ($params['from'] == 'return') {
			$order = pdo_fetch("SELECT weid, price, from_user, createtime, paytype, addressid, mycouponid FROM " . tablename('hc_moreshop_order') . " WHERE id = '{$params['tid']}'");
			$ordergoods = pdo_fetchall("SELECT goodsid, total FROM " . tablename('hc_moreshop_order_goods') . " WHERE orderid = '{$params['tid']}'", array(), 'goodsid');
			$goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total FROM " . tablename('hc_moreshop_goods') . " WHERE id IN ('" . implode("','", array_keys($ordergoods)) . "')");
			if (!empty($this->module['config']['noticeemail'])) {
				$address = pdo_fetch("SELECT * FROM " . tablename('hc_moreshop_address') . " WHERE id = :id", array(':id' => $order['addressid']));
				$body = "<h3>购买商品清单</h3> <br />";
				if (!empty($goods)) {
			...
			$istemplate_id = pdo_fetch('SELECT id, istemplate_id FROM '.tablename('hc_moreshop_membernews')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $order['weid'],':from_user' => $order['from_user']));
			if(empty($istemplate_id) || $istemplate_id['istemplate_id']==1){
			...
			if($order['mycouponid']){
				pdo_update('hc_moreshop_mycoupons', array('isuse'=>1), array('id'=>$order['mycouponid']));
			}
	...
	public function doMobileSpec() {
        ...
        include $this->template('host/spec');
    }
	...
	public function doMobileSpecItem() {
        global $_GPC;
        load()->func('tpl');
		$spec = array(
            "id" => $_GPC['specid']
        ...
        include $this->template('host/spec_item');
    }
	...
	public function doMobileParam() {
        $tag = random(32);
        global $_GPC;
        include $this->template('host/param');
    }
	
    public function doWebExpress() {
        global $_W, $_GPC;
        // pdo_query('DROP TABLE ims_hc_moreshop_express');
        //pdo_query("CREATE TABLE IF NOT EXISTS `ims_hc_moreshop_express` (  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,  `weid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '所属帐号',  `express_name` varchar(50) NOT NULL COMMENT '分类名称',  `express_price` varchar(10) NOT NULL DEFAULT '0',  `displayorder` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '排序',  `express_area` varchar(50) NOT NULL COMMENT '配送区域',  `enabled` tinyint(1) NOT NULL,  PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 ");
        //pdo_query("ALTER TABLE  `ims_hc_moreshop_order` ADD  `expressprice` VARCHAR( 10 ) NOT NULL AFTER  `totalnum` ;");
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        if ($operation == 'display') {
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_express') . " WHERE weid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
    ...
    public function doWebDispatch() {
        ...
        if ($operation == 'display') {
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_dispatch') . " WHERE weid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
        } elseif ($operation == 'post') {
            ...
            $express = pdo_fetchall("select * from " . tablename('hc_moreshop_express') . " WHERE weid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
    ...
    public function doWebAdv() {
        ...
        $operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
        if ($operation == 'display') {
            $list = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_adv') . " WHERE weid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
        } elseif ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    pdo_update('hc_moreshop_adv', $data, array('id' => $id));
                } else {
                    pdo_insert('hc_moreshop_adv', $data);
                    $id = pdo_insertid();
                }
                message('更新幻灯片成功！', $this->createWebUrl('adv', array('op' => 'display')), 'success');
            }
            $adv = pdo_fetch("select * from " . tablename('hc_moreshop_adv') . " where id=:id and weid=:weid limit 1", array(":id" => $id, ":weid" => $_W['uniacid']));
        } elseif ($operation == 'delete') {
            $id = intval($_GPC['id']);
            $adv = pdo_fetch("SELECT id  FROM " . tablename('hc_moreshop_adv') . " WHERE id = '$id' AND weid=" . $_W['uniacid'] . "");
            if (empty($adv)) {
                message('抱歉，幻灯片不存在或是已经被删除！', $this->createWebUrl('adv', array('op' => 'display')), 'error');
            }
            pdo_delete('hc_moreshop_adv', array('id' => $id));
	...
	public function doMobileUserinfo() {
		...
		if (isset($_GPC['code'])){
			...
			$member = pdo_fetch("select id from ".tablename('hc_moreshop_member')." where weid = ".$_W['uniacid']." and from_user = '".$_W['openid']."'");
			if(empty($member)){
				pdo_insert('hc_moreshop_member',$data);
			} else {
				if(!empty($headimg)){
					pdo_update('hc_moreshop_member', $data, array('id'=>$member['id']));
				}
			}
			setCookie('hc_moreshop_headimgurl', $info['headimgurl'], time()+3600*240);
...
function sendOrder($openid, $goodsname, $goodsprice, $createtime, $url) {
	global $_W;
	$template_id = pdo_fetchcolumn("select template_id from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
...
function sendGoodsSend($openid, $ordernum, $expresscom, $expresssn, $url) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendGoodsSend from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
...
function sendCommWarm($openid, $commission, $createtime, $url) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendCommWarm from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
...
function sendCheckChange($openid, $commission, $applytime, $checktime) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendCheckChange from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
...
function sendApplyMoneyBack($openid, $money, $goodsname, $order) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendApplyMoneyBack from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
...
function sendMoneyBack($openid, $money, $backtype, $returntime, $goodsname, $ordersn, $refundreason) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendMoneyBack from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
...
//客户下单提醒消息
function sendOpenidNews($openid, $money, $backtype, $returntime, $goodsname, $ordersn, $refundreason) {
	global $_W;
	$template_id = pdo_fetchcolumn("select sendopenidnews from ".tablename('hc_moreshop_templatenews')." where weid = ".$_W['uniacid']);
	$template_id = trim($template_id);
	if (!empty($template_id)) {
		$datas = array(
		   'first'=> array('value'=>'退款申请已核实，请等待商家退款信息。','color'=>'#173177'),
...
//红包函数
function get_red_bag($appid,$mch_id,$openid,$total_amount,$wishing,$act_name,$remark,$key,$send_name){
	global $_W;
	load()->func('communication');
	//随机字符串
	$nonce_str = createNonceStr();
	//随机数字
	$nonce_num = createNoNum();
	//订单号
	...
	//接受者openid
	$re_openid = $openid;
	//$stringA = "nonce_str=$nonce_str&mch_billno=$mch_billno&mch_id=$mch_id&wxappid=$wxappid&send_name=$send_name&re_openid=$re_openid&total_amount=$total_amount&total_num=$total_num&wishing=$wishing&client_ip=$client_ip&act_name=$act_name&remark=$remark";
	...
	$res = curl_post_ssl('https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack',$postarr);
...
function curl_post_ssl($url, $vars, $second=30,$aHeader=array()){
	...
	//以下两种方式需选择一种
	$url = $_SERVER['DOCUMENT_ROOT'].'/payment';
