<?php
	...
	if ($operation == 'display') {
		...
		$children = array();
		$category = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_category') . " WHERE weid = '{$_W['uniacid']}' ORDER BY parentid ASC, displayorder DESC");
		...
		if (checksubmit('submit')) {
			...
			$data = array(
				'weid' => $_W['uniacid'],
				'name' => $_GPC['catename'],				'thumb' => $_GPC['thumb'],				'enabled' => intval($_GPC['enabled']),
		...
		$id = intval($_GPC['id']);
		$category = pdo_fetch("SELECT id, parentid FROM " . tablename('hc_moreshop_category') . " WHERE id = '$id'");
		if (empty($category)) {
			message('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('category', array('op' => 'display')), 'error');
		}
		pdo_delete('hc_moreshop_category', array('id' => $id, 'parentid' => $id), 'OR');
