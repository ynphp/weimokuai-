<?php
	...
	if (!empty($conditions['member'])) {
		$addressids = pdo_fetchall("select id from ".tablename('hc_moreshop_address')." where weid = ".$_W['uniacid']." and realname LIKE '%{$conditions['member']}%' or mobile LIKE '%{$conditions['member']}%'");
		$addressid = 0;
		if(!empty($addressids)){
	...
	$orders = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order') . " WHERE weid = '{$_W['uniacid']}' $condition ");
	if(empty($orders)){
		message('没有需要导出的订单！');
	} else {
		$orderid = '';
	...
	$dispatchs = pdo_fetchall("SELECT id, dispatchname FROM " . tablename('hc_moreshop_dispatch') . " WHERE weid = ".$weid);
	$dispatch = array();
	...
	$address = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_address')." WHERE weid = ".$weid);
	$addres = array();
	...
	$goods = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order_goods')." WHERE weid = ".$_W['uniacid']." and orderid in ".$orderid);
	$list = array();
