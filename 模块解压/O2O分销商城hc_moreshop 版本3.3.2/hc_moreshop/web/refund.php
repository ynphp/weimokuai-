<?php
	...
	if($_GPC['op']=='select'){
		...
		if(!empty($isoutput)){
			...
			$lists = pdo_fetchall("SELECT o.id, o.weid, o.from_user, o.ordersn, o.refundtime, o.price, o.refundreason, m.realname, m.mobile, m.bankcard, m.alipay, m.wxhao FROM ".tablename('hc_moreshop_order')." as o left join ".tablename('hc_moreshop_member')." as m on o.from_user = m.from_user and o.weid = m.weid WHERE o.weid = ".$_W['uniacid']." and o.status = -2 and o.id in (".$ids.")");
			foreach($isoutput as $i){
				pdo_update('hc_moreshop_order', array('isoutput'=>1), array('id'=>$i));
			}
		} else {
			message('没有要导出的订单！');
			exit;
	...
	foreach($lists as $k=>$v){
		$ordergoods = pdo_fetchall("SELECT * FROM " . tablename('hc_moreshop_order_goods')." WHERE weid = ".$_W['uniacid']." and orderid = ".$v['id']);
		$title = '';
		foreach($ordergoods as $o){
			$title = $good[$o['goodsid']].','.$title;
		}
		$title = trim($title, ',');
		$issendMoneyBack = pdo_fetch('SELECT id, issendMoneyBack FROM '.tablename('hc_moreshop_membernews')." WHERE  weid = :weid  AND from_user = :from_user" , array(':weid' => $v['weid'],':from_user' => $v['from_user']));
		if(empty($issendMoneyBack) || $issendMoneyBack['issendMoneyBack']==1){
