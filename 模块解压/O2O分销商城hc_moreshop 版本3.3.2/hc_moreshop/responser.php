...
class QRResponser {
	public function aa($openid, $uniacid, $id){
		...
		if(empty($member['headimg'])){
			$hr_file = IA_ROOT.'/addons/hc_moreshop/qrcode/header.png';
		...
		$qr_file = IA_ROOT."/addons/hc_moreshop/style/images/share/$imgname";
		if(!file_exists($qr_file)){
			require IA_ROOT . '/framework/library/qrcode/phpqrcode.php';
			$value = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=hc_moreshop&do=index&mid=' . $id;
			$errorCorrectionLevel = "L";
			$matrixPointSize = "4";
			QRcode::png($value, $qr_file, $errorCorrectionLevel, $matrixPointSize);
		}
		$qr_file = $_W['siteroot']."/addons/hc_moreshop/style/images/share/$imgname";
		$poster = pdo_fetch('SELECT * FROM ' . tablename('hc_moreshop_poster') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		$target_file = IA_ROOT.'/addons/hc_moreshop/qrcode/qrshare/'.$uniacid."share$id.jpg";
		imagejpeg($target, $target_file);
        imagedestroy($target);
		if(file_exists($target_file)){
			$media_id = $this->uploadImage($target_file);
			if(!empty($media_id)){
				pdo_update('hc_moreshop_member', array('media_id'=>$media_id, 'ischange'=>0, 'mediatime'=>time()), array('id'=>$id));
	...
	public function mergeText($target, $alldata, $text){
		$font = IA_ROOT . '/addons/hc_moreshop/style/font/qrcodefont/msyhbd.ttf';
